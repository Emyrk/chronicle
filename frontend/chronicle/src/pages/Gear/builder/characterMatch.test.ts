...
import { buildCharacterMatch, slotOwned, stageCoverage } from "./characterMatch";
...
describe("buildCharacterMatch", () => {
  it("splits equipped (newest) from owned (union)", () => {
    ...
    expect([...match.ownedIds].sort()).toEqual([100, 101, 200, 300]);
  });
...
describe("slotOwned / stageCoverage", () => {
  ...
  const match = buildCharacterMatch(history(outfit({ 0: 100 }), outfit({ 4: 101 })));

  it("counts primary and alternate ownership", () => {
    expect(slotOwned(stage, SLOT.head, match)).toBe(true); // primary owned
    expect(slotOwned(stage, SLOT.chest, match)).toBe(true); // via alternate
    expect(slotOwned(stage, SLOT.mainHand, match)).toBe(false);
    expect(slotOwned(stage, SLOT.waist, match)).toBe(false); // empty slot
  });
  ...
  it("computes coverage excluding cosmetic slots", () => {
    ...
    expect(coverage).toEqual({
      filled: 3,
      owned: 2,
      equipped: 1, // head item 100 is currently worn
