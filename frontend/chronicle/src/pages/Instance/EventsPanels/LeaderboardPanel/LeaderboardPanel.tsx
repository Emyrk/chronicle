...
export function createLeaderboardPanel(): PanelDefinition<
...
export function LeaderboardContent({
  speedrunOverride,
  ...props
}: PanelRenderProps<LeaderboardPanelResult> & {
  speedrunOverride?: SpeedrunResult | null;
}) {
  if (speedrunOverride !== undefined) {
    return speedrunOverride ? (
      <LeaderboardDetails speedrun={speedrunOverride} />
    ) : (
...
function LeaderboardQueryContent(
  props: PanelRenderProps<LeaderboardPanelResult>,
) {
...
export function LeaderboardDetails({ speedrun }: { speedrun: SpeedrunResult }) {
  ...
  const showRawDuration =
    rankedDurationMs === undefined || rankedDurationMs !== speedrun.duration_ms;

  return (
    <div className="p-3">
      <div
        className="flex items-center justify-between mb-3"
        data-lesson-target="read-proof"
      >
      ...
      {Object.entries(
        speedrun.proof.reduce<
          Record<string, Array<(typeof speedrun.proof)[number]>>
        >((acc, p) => {
          ...
          <div className="grid grid-cols-2 gap-1">
            <div className="flex items-center gap-1.5 text-xs px-1.5 py-0.5 rounded">
              ...
              >
                ...
                <span className="text-zinc-500">
                  {" "}
                  (min {speedrun.version_status.min_parser_version || "none"})
                </span>
              </span>
            </div>
            <div className="flex items-center gap-1.5 text-xs px-1.5 py-0.5 rounded">
              ...
              >
                ...
                <span className="text-zinc-500">
                  {" "}
                  (min {speedrun.version_status.min_addon_version || "none"})
                </span>
              </span>
            </div>
          </div>
