 ...
 * then the cursor switches to the "Healed" tab to show healing by target —
 * with overheal per target.
...
const TAB_FRAME = 210; // "Healed" tab clicked
const TARGET_COL_FRAME = 290; // per-target overheal called out
...
function Content() {
  ...
  // Cursor: rest near the Overheal column, then to the Healed tab.
  const cursorX = interpolate(frame, [26, 100, 160, 200], [1140, 900, 900, HEALED_TAB.x], {
    ...clamp,
    easing: entranceEasing,
  });
  const cursorY = interpolate(frame, [26, 100, 160, 200], [600, 260, 260, HEALED_TAB.y], {
    ...clamp,
    easing: entranceEasing,
  });
  const clickPulse = interpolate(frame, [TAB_FRAME - 4, TAB_FRAME, TAB_FRAME + 10], [0, 1, 0], clamp);

  const overhealColIn = interpolate(frame, [45, 57, TAB_FRAME - 10, TAB_FRAME], [0, 1, 1, 0], clamp);
  ...
  const step = frame >= TARGET_COL_FRAME ? 3 : frame >= TAB_FRAME ? 2 : 1;
