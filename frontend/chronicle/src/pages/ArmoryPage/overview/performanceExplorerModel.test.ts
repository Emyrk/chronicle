...
describe("selectPerformanceEncounterNames", () => {
  const variant = buildPerformanceVariants([
    encounter({ instance_name: "Zul'Gurub", encounter_name: "High Priestess Jeklik" }),
    encounter({ instance_name: "Zul'Gurub", encounter_name: "Hakkar" }),
    encounter({ instance_name: "Zul'Gurub", encounter_name: "Gahz'ranka" }),
  ...
  it("excludes optional Zul'Gurub bosses from the default selection", () => {
    expect(selectPerformanceEncounterNames(variant, [], progressionBosses)).toEqual([
      "Hakkar",
  ...
  it("keeps optional bosses when they are explicitly selected", () => {
    expect(selectPerformanceEncounterNames(
      variant,
      variant.encounters,
      progressionBosses,
    )).toEqual(variant.encounters);
  });
});
