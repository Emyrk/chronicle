...
export function formatCooldown(spell: WoWSpell): string | null {
  // recovery_time and category_recovery_time are nanoseconds (Go time.Duration
  // serializes as its int64 nanosecond value). The effective cooldown is the
  // larger of the spell's own recovery and its shared category recovery — many
  // spells (e.g. Power Word: Shield) store their cooldown only in the category
  // field.
  const ns = Math.max(spell.recovery_time || 0, spell.category_recovery_time || 0);
  if (ns <= 0) return null;
