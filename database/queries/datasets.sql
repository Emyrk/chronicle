...
VALUES (@name, @slug, @wow_version, @build_version, @description, @spell_dbc_storage_key)
RETURNING *;
...
UPDATE datasets SET
    ...
    spell_dbc_storage_key = COALESCE(sqlc.narg('spell_dbc_storage_key'), spell_dbc_storage_key),
    updated_at        = now()
WHERE id = @id
RETURNING *;
