...
SELECT
    ...
    COALESCE(CASE
        WHEN @use_ranked_timing::boolean THEN COALESCE(duplicate_speedrun.boss_to_boss_duration_ms, duplicate_speedrun.ranked_duration_ms)
        ELSE duplicate_speedrun.duration_ms
    END, 0)::bigint AS duration_ms,
    CASE
        WHEN @use_ranked_timing::boolean THEN COALESCE(duplicate_speedrun.boss_to_boss_start_time, duplicate_speedrun.ranked_start_time)
        ELSE duplicate_speedrun.start_time
    END::timestamptz AS start_time,
    CASE
        WHEN @use_ranked_timing::boolean THEN COALESCE(duplicate_speedrun.boss_to_boss_completion_time, duplicate_speedrun.ranked_completion_time)
