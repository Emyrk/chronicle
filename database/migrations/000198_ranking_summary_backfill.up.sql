...
-- This control table stores bounded, resumable backfill state only. Historical
-- source and summary tables are intentionally not scanned or modified during startup.
CREATE TABLE ranking_summary_backfills (
    ...
    cursor_run_id UUID NOT NULL DEFAULT '00000000-0000-0000-0000-000000000000',
    ...
    CONSTRAINT ranking_summary_backfills_preview_counts_check CHECK (
        preview_total_runs >= 0 AND preview_source_rows >= 0 AND preview_missing_runs >= 0 AND
        preview_stale_runs >= 0 AND preview_current_runs >= 0 AND
        preview_dirty_runs >= 0 AND estimated_wal_bytes >= 0 AND
    ...
    CONSTRAINT ranking_summary_backfills_lifecycle_check CHECK (
        ...
        (status IN ('running', 'paused') AND started_at IS NOT NULL AND completed_at IS NULL) OR
