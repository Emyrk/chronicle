...
CREATE TABLE ranking_summary_backfills (
    ...
    CONSTRAINT ranking_summary_backfills_lifecycle_check CHECK ((((status = 'planned'::text) AND (started_at IS NULL) AND (completed_at IS NULL)) OR ((status = 'running'::text) AND (started_at IS NOT NULL) AND (completed_at IS NULL)) OR ((status = ANY (ARRAY['completed'::text, 'failed'::text, 'cancelled'::text])) AND (completed_at IS NOT NULL)))),
    CONSTRAINT ranking_summary_backfills_max_batches_check CHECK (((max_batches >= 1) AND (max_batches <= 100))),
    CONSTRAINT ranking_summary_backfills_preview_counts_check CHECK (((preview_total_runs >= 0) AND (preview_missing_runs >= 0) AND (preview_stale_runs >= 0) AND (preview_current_runs >= 0) AND (preview_dirty_runs >= 0) AND (preview_total_runs = ((preview_missing_runs + preview_stale_runs) + preview_current_runs)) AND (preview_dirty_runs <= preview_total_runs))),
    ...
    CONSTRAINT ranking_summary_backfills_status_check CHECK ((status = ANY (ARRAY['planned'::text, 'running'::text, 'completed'::text, 'failed'::text, 'cancelled'::text]))),
...
ALTER TABLE ONLY ranking_summary_backfills
    ADD CONSTRAINT ranking_summary_backfills_tenant_id_fkey FOREIGN KEY (tenant_id) REFERENCES tenants(id) ON DELETE SET NULL;
