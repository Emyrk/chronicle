...
func (q *sqlQuerier) RankingRunsNeedingRepair(ctx context.Context, queryLimit int32) ([]RankingRunsNeedingRepairRow, error) {
	rows, err := q.db.Query(ctx, rankingRunsNeedingRepair, queryLimit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
