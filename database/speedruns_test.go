...
func TestEncounterKillTimesIncludePartialKills(t *testing.T) {
	...
	require.NoError(t, err)
	require.True(t, speedrun.RankedStartTime.Valid)
	require.True(t, startedAt.Equal(speedrun.RankedStartTime.Time))
	require.True(t, speedrun.RankedCompletionTime.Valid)
	require.True(t, startedAt.Add(time.Hour).Equal(speedrun.RankedCompletionTime.Time))
	require.Equal(t, pgtype.Int8{Int64: int64(time.Hour / time.Millisecond), Valid: true}, speedrun.RankedDurationMs)
