...
func TestFetcherDBOnly_PopulatesModernSpellComponents(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, []int32{0, 2}, []int32{got.ModernEffects[0].EffectIndex, got.ModernEffects[1].EffectIndex})
	require.Equal(t, []int32{20, 22}, []int32{got.ModernEffects[0].Effect, got.ModernEffects[1].Effect})
	require.Equal(t, []int32{101, 102, 103, 104}, got.ModernEffects[0].EffectSpellClassMask)
	require.Equal(t, []int32{0, 1}, []int32{got.ModernPowers[0].OrderIndex, got.ModernPowers[1].OrderIndex})
	require.Equal(t, []int32{1000, 2000}, []int32{got.ModernPowers[0].ManaCost, got.ModernPowers[1].ManaCost})
	require.Equal(t, []int32{0, 2}, []int32{got.ModernVariants[0].DifficultyID, got.ModernVariants[1].DifficultyID})
	require.Equal(t, []int32{1, 2, 3, 4}, got.ModernVariants[0].ClassOptions.SpellClassMask)
	require.NotNil(t, got.ModernVariants[0].Levels)
	require.Nil(t, got.ModernVariants[0].Categories)
	require.Nil(t, got.ModernVariants[1].Levels)
	require.Equal(t, int32(14), got.ModernVariants[1].Categories.Category)
	...
	require.Len(t, byName[0].ModernEffects, 2)
	require.Len(t, byName[0].ModernPowers, 2)
	require.Len(t, byName[0].ModernVariants, 2)

	legacyGot, err := fetcher.Spell(ctx, datasetID, legacy.ID)
	require.NoError(t, err)
	require.Empty(t, legacyGot.ModernEffects)
	require.Empty(t, legacyGot.ModernPowers)
	require.Empty(t, legacyGot.ModernVariants)
	legacyJSON, err := json.Marshal(legacyGot)
	require.NoError(t, err)
	require.NotContains(t, string(legacyJSON), "modern_effects")
	...
	require.NoError(t, err)
	require.Len(t, cached.ModernEffects, 2)
}
