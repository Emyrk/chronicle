...
func (h *Handler) persistWowdata(ctx context.Context, datasetID uuid.UUID, p *wowdata.Import) error {
	...
	if err := store.InTx(ctx, func(tx database.Store) error {
		...
		for i := 0; i < len(p.Spells); i += batchSize {
			...
			if err := spelldb.UpsertBatch(ctx, tx, p.Spells[i:end]); err != nil {
				return fmt.Errorf("upsert spells at %d: %w", i, err)
			}
		}
		if err := persistNormalizedSpells(ctx, tx, datasetID, p); err != nil {
			return err
		}
		if err := upsertItems(ctx, tx, datasetID, p.Items); err != nil {
			return err
		}
		if p.SpellIcons != nil {
			if err := h.upsertWowdataSpellIcons(ctx, tx, datasetID, p.SpellIcons); err != nil {
				return err
			}
		}
		if err := h.upsertWowdataMetadata(ctx, tx, datasetID, p); err != nil {
			return err
		}
		if err := h.upsertWowdataEnchantments(ctx, tx, datasetID, p.Enchantments); err != nil {
			return err
		}
		if err := h.upsertWowdataItemSets(ctx, tx, datasetID, p.ItemSets); err != nil {
			return err
		}
		if len(p.TalentTrees) > 0 {
			if err := tx.UpsertDatasetTalentTrees(ctx, database.UpsertDatasetTalentTreesParams{DatasetID: datasetID, Data: p.TalentTrees}); err != nil {
				return fmt.Errorf("upsert talent trees: %w", err)
			}
		}
		if _, err := tx.Exec(ctx, `UPDATE datasets SET spells_imported_at=now(), spells_count=$2, updated_at=now() WHERE id=$1`, datasetID, len(p.Spells)); err != nil {
...
func (h *Handler) upsertWowdataSpellIcons(ctx context.Context, db database.DBTX, datasetID uuid.UUID, rows []wowdata.SpellIcon) error {
	if _, err := db.Exec(ctx, `DELETE FROM dbc_spell_icons WHERE dataset_id=$1`, datasetID); err != nil {
...
func (h *Handler) upsertWowdataMetadata(ctx context.Context, db database.DBTX, datasetID uuid.UUID, p *wowdata.Import) error {
	statements := []struct {
		clear, insert string
		rows          [][]any
	}{
	...
	for _, s := range statements {
		if _, err := db.Exec(ctx, s.clear, datasetID); err != nil {
			return err
		}
		if err := batchRows(ctx, db, datasetID, s.insert, s.rows); err != nil {
			return err
		}
	}
	return nil
...
func batchRows(ctx context.Context, db database.DBTX, datasetID uuid.UUID, sql string, rows [][]any) error {
	batch := &pgx.Batch{}
	for _, row := range rows {
	...
	if batch.Len() > 0 {
		return flushBatch(ctx, db, batch)
	}
	return nil
}

func (h *Handler) upsertWowdataEnchantments(ctx context.Context, db database.DBTX, datasetID uuid.UUID, rows []wowdata.Enchantment) error {
...
func (h *Handler) upsertWowdataItemSets(ctx context.Context, db database.DBTX, datasetID uuid.UUID, rows []wowdata.ItemSet) error {
	...
	if _, err := db.Exec(ctx, `DELETE FROM dbc_item_set_item WHERE dataset_id=$1`, datasetID); err != nil {
		return err
	}
	b := &pgx.Batch{}
	for _, x := range rows {
