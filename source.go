...
var (
	...
	ReSpellCastAttempt                             = regexp.MustCompile(`(.+[^\s]) begins to (casts|performs) (.+[^\s])\.`)
	...
	ReLoot                                         = regexp.MustCompile(`LOOT: ([^&]+)&(.+[^\s]) receives loot: \|c([a-zA-Z]+)\|Hitem:(\d+):(\d+):(\d+):(\d+)\|h\[([a-zA-Z0-9\s']+)\]\|h\|rx(\d+)\.`)
	// Bug pattern
