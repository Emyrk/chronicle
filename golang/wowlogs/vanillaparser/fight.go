...
func NewFight(logger *slog.Logger, ts time.Time, zone zone.Zone) *Fight {
	f := &Fight{
...
func (f *Fight) Slain(ts time.Time, gs ...guid.GUID) {
	...
	f.Logger.Debug("Participant slain",
		slog.Int("remaining_friendly", len(f.FriendlyAlive)),
		slog.Int("remaining_enemies", len(f.EnemiesAlive)),
		slog.Time("time", ts),
	)
}
...
func (f *Fight) Timeout(now time.Time) {
	for id, ts := range f.Participants {
		if now.Sub(ts) > ParticipantTimeout {
			...
			f.Slain(ts, id)
			f.Logger.Warn("Participant timed out from fight", "id", id, "lastSeen", ts)
		}
	}
}
