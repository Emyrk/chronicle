...
// Code generated by regengo for pattern: (.+[^\s]) falls and loses (\d+) health\.
// DO NOT EDIT.

var reFallDamageStackPool = sync.Pool{New: func() interface{} {
	stack := make([][3]int, 0, 32)
	return &stack
}}

var reFallDamageCaptureStackPool = sync.Pool{New: func() interface{} {
	stack := make([]int, 0, 96)
	return &stack
}}

type ReFallDamage struct{}

var CompiledReFallDamage = ReFallDamage{}

// MinMatchLen is the minimum number of bytes any match can have.
const ReFallDamageMinMatchLen = 28

// MaxMatchLen is the maximum number of bytes any match can have.
// -1 means unbounded (pattern contains * or + quantifiers).
const ReFallDamageMaxMatchLen = -1

func (ReFallDamage) MatchLengthInfo() (minLen, maxLen int) {
	return ReFallDamageMinMatchLen, ReFallDamageMaxMatchLen
}

func (ReFallDamage) MatchString(input string) bool {
	l := len(input)
	offset := 0
	stackPtr := reFallDamageStackPool.Get().(*[][3]int)
	stack := (*stackPtr)[:0]
	defer func() {
		for i := range stack {
			stack[i] = [3]int{0, 0, 0}
		...
		*stackPtr = stack[:0]
		reFallDamageStackPool.Put(stackPtr)
	}()
	nextInstruction := 1
	goto StepSelect
TryFallback:
...
func (ReFallDamage) MatchBytes(input []byte) bool {
	l := len(input)
	offset := 0
	stackPtr := reFallDamageStackPool.Get().(*[][3]int)
	stack := (*stackPtr)[:0]
...
type ReFallDamageResult struct {
	Match  string // Full match
	Group1 string
	Group2 string
}

type ReFallDamageBytesResult struct {
	Match  []byte // Full match
	Group1 []byte
	Group2 []byte
}

func (ReFallDamage) FindStringReuse(input string, r *ReFallDamageResult) (*ReFallDamageResult, bool) {
	l := len(input)
	offset := 0
	var captures [6]int
	captureStackPtr := reFallDamageCaptureStackPool.Get().(*[]int)
	captureStack := (*captureStackPtr)[:0]
	defer func() {
		*captureStackPtr = captureStack[:0]
		reFallDamageCaptureStackPool.Put(captureStackPtr)
	}()
	stackPtr := reFallDamageStackPool.Get().(*[][3]int)
	stack := (*stackPtr)[:0]
	defer func() {
...
func (recv ReFallDamage) FindString(input string) (*ReFallDamageResult, bool) {
	return recv.FindStringReuse(input, nil)
}
func (ReFallDamage) FindAllStringAppend(input string, n int, s []*ReFallDamageResult) []*ReFallDamageResult {
	if n == 0 {
		return s
	}
	result := s
	...
	Ins35:
		{
			captures[1] = offset
			var item *ReFallDamageResult
			if len(result) < cap(result) {
				result = result[:len(result)+1]
				item = result[len(result)-1]
				if item == nil {
					item = &ReFallDamageResult{}
...
func (r ReFallDamage) FindAllString(input string, n int) []*ReFallDamageResult {
	return r.FindAllStringAppend(input, n, nil)
}
func (ReFallDamage) FindBytesReuse(input []byte, r *ReFallDamageBytesResult) (*ReFallDamageBytesResult, bool) {
	l := len(input)
	offset := 0
	var captures [6]int
	captureStackPtr := reFallDamageCaptureStackPool.Get().(*[]int)
...
func (recv ReFallDamage) FindBytes(input []byte) (*ReFallDamageBytesResult, bool) {
	return recv.FindBytesReuse(input, nil)
}
func (ReFallDamage) FindAllBytesAppend(input []byte, n int, s []*ReFallDamageBytesResult) []*ReFallDamageBytesResult {
	if n == 0 {
		return s
	}
	result := s
	...
	Ins35:
		{
			captures[1] = offset
			var item *ReFallDamageBytesResult
			if len(result) < cap(result) {
				result = result[:len(result)+1]
				item = result[len(result)-1]
				if item == nil {
					item = &ReFallDamageBytesResult{}
					result[len(result)-1] = item
				}
			} else {
				item = &ReFallDamageBytesResult{}
...
func (r ReFallDamage) FindAllBytes(input []byte, n int) []*ReFallDamageBytesResult {
...
func (ReFallDamage) DefaultMaxLeftover() int {
	return 1048576
}
...
func (ReFallDamage) FindReader(r io.Reader, cfg stream.Config, onMatch func(stream.Match[*ReFallDamageBytesResult]) bool) error {
	...
	reuseResult := &ReFallDamageBytesResult{}
			...
			if err == io.EOF {
				...
				if leftover > 0 {
					...
					for searchPos < len(chunk) {
						result, ok := ReFallDamage{}.FindBytesReuse(chunk[searchPos:], reuseResult)
						if !ok {
							break
						}
						...
						m := stream.Match[*ReFallDamageBytesResult]{
...
func (ReFallDamage) FindReaderCount(r io.Reader, cfg stream.Config) (int64, error) {
	var count int64
	err := ReFallDamage{}.FindReader(r, cfg, func(_ stream.Match[*ReFallDamageBytesResult]) bool {
		count++
		return true
	})
	return count, err
...
func (ReFallDamage) FindReaderFirst(r io.Reader, cfg stream.Config) (*ReFallDamageBytesResult, int64, error) {
	var result *ReFallDamageBytesResult
	var offset int64
	err := ReFallDamage{}.FindReader(r, cfg, func(m stream.Match[*ReFallDamageBytesResult]) bool {
		// Copy the result since buffer will be reused
		result = ReFallDamage{}.copyBytesResult(m.Result)
...
func (ReFallDamage) copyBytesResult(src *ReFallDamageBytesResult) *ReFallDamageBytesResult {
	if src == nil {
		return nil
	}
	dst := &ReFallDamageBytesResult{}
	// Copy Match slice
...
//	    })
//	io.Copy(os.Stdout, r)
func (ReFallDamage) NewTransformReader(r io.Reader, cfg stream.TransformConfig, onMatch func(match *ReFallDamageBytesResult, emit func([]byte))) io.Reader {
	if cfg.BufferSize == 0 {
	...
	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReFallDamage{}.processTransform(data, isEOF, onMatch, emitOut)
	}
...
func (ReFallDamage) processTransform(data []byte, isEOF bool, onMatch func(match *ReFallDamageBytesResult, emit func([]byte)), emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReFallDamageBytesResult{}

	for {
		result, ok := ReFallDamage{}.FindBytesReuse(data[processed:], reuseResult)
		if !ok {
			// No more matches
			if isEOF {
...
//	io.Copy(os.Stdout, r)
func (ReFallDamage) ReplaceReader(r io.Reader, template string) io.Reader {
	parsed, err := replace.Parse(template)
	if err != nil {
		return &reFallDamageTransformErrReader{err: err}
	}
	...
	return ReFallDamage{}.NewTransformReader(r, stream.DefaultTransformConfig(), func(m *ReFallDamageBytesResult, emit func([]byte)) {
		...
		for _, seg := range resolved {
			...
			case replace.SegmentCaptureIndex:
				capture := (ReFallDamage{}).getCaptureByIndex(m, seg.CaptureIndex)
				if capture != nil {
...
// reFallDamageTransformErrReader is a reader that always returns an error.
type reFallDamageTransformErrReader struct {
	err error
}

func (r *reFallDamageTransformErrReader) Read(p []byte) (int, error) {
	return 0, r.err
}

// getCaptureByIndex returns the capture group value by its index.
func (ReFallDamage) getCaptureByIndex(m *ReFallDamageBytesResult, index int) []byte {
	switch index {
	case 0:
		return m.Match
	default:
...
//	})
func (ReFallDamage) SelectReader(r io.Reader, pred func(*ReFallDamageBytesResult) bool) io.Reader {
	cfg := stream.DefaultTransformConfig()
	cfg.MaxLeftover = 1048576

	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReFallDamage{}.processSelect(data, isEOF, pred, emitOut)
	}
...
func (ReFallDamage) processSelect(data []byte, isEOF bool, pred func(*ReFallDamageBytesResult) bool, emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReFallDamageBytesResult{}
...
//	})
func (ReFallDamage) RejectReader(r io.Reader, pred func(*ReFallDamageBytesResult) bool) io.Reader {
	cfg := stream.DefaultTransformConfig()
	cfg.MaxLeftover = 1048576

	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReFallDamage{}.processReject(data, isEOF, pred, emitOut)
	}
...
func (ReFallDamage) processReject(data []byte, isEOF bool, pred func(*ReFallDamageBytesResult) bool, emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReFallDamageBytesResult{}
...
func (r *ReFallDamageResult) CaptureByIndex(idx int) string {
	switch idx {
	case 0:
		return r.Match
	case 1:
...
// CaptureByIndex returns the capture group value by its 0-based index.
// Index 0 returns the full match, 1+ returns capture groups.
func (r *ReFallDamageBytesResult) CaptureByIndex(idx int) []byte {
	switch idx {
	case 0:
		return r.Match
	case 1:
...
func (ReFallDamage) ReplaceAllString(input string, template string) string {
	...
	lastEnd := 0
	var r ReFallDamageResult

	remaining := input
	offset := 0

	for {
		match, ok := ReFallDamage{}.FindStringReuse(remaining, &r)
		if !ok {
			break
		}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReFallDamage) ReplaceAllBytes(input []byte, template string) []byte {
	return ReFallDamage{}.ReplaceAllBytesAppend(input, template, nil)
}

// ReplaceAllBytesAppend replaces all matches and appends to buf.
// If buf has sufficient capacity, no allocation occurs.
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReFallDamage) ReplaceAllBytesAppend(input []byte, template string, buf []byte) []byte {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	result := buf[:0]
	lastEnd := 0
	var r ReFallDamageBytesResult

	remaining := input
	offset := 0

	for {
		match, ok := ReFallDamage{}.FindBytesReuse(remaining, &r)
		if !ok {
			break
		}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReFallDamage) ReplaceFirstString(input string, template string) string {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	var r ReFallDamageResult
	match, ok := ReFallDamage{}.FindStringReuse(input, &r)
	if !ok {
		return input
	}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReFallDamage) ReplaceFirstBytes(input []byte, template string) []byte {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	var r ReFallDamageBytesResult
	match, ok := ReFallDamage{}.FindBytesReuse(input, &r)
	if !ok {
...
// ReFallDamageReplaceTemplate holds a pre-compiled replace template for the ReFallDamage pattern.
// Use CompileReplaceTemplate to create one, then call its Replace methods.
type ReFallDamageReplaceTemplate struct {
	original string
...
//
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReFallDamage) CompileReplaceTemplate(template string) (*ReFallDamageReplaceTemplate, error) {
	tmpl, err := replace.Parse(template)
	if err != nil {
		return nil, err
	}
	...
	return &ReFallDamageReplaceTemplate{
		original: template,
		segments: resolved,
	}, nil
}
...
func (t *ReFallDamageReplaceTemplate) String() string {
	return t.original
}

// ReplaceAllString replaces all matches in input using this compiled template.
func (t *ReFallDamageReplaceTemplate) ReplaceAllString(input string) string {
	var result strings.Builder
	lastEnd := 0
	var r ReFallDamageResult
...
func (t *ReFallDamageReplaceTemplate) ReplaceAllBytes(input []byte) []byte {
	return t.ReplaceAllBytesAppend(input, nil)
}

// ReplaceAllBytesAppend replaces all matches and appends to buf.
// If buf has sufficient capacity, no allocation occurs.
func (t *ReFallDamageReplaceTemplate) ReplaceAllBytesAppend(input []byte, buf []byte) []byte {
	result := buf[:0]
	lastEnd := 0
	var r ReFallDamageBytesResult
...
func (t *ReFallDamageReplaceTemplate) ReplaceFirstString(input string) string {
	var r ReFallDamageResult
	match, ok := ReFallDamage{}.FindStringReuse(input, &r)
	if !ok {
		return input
...
func (t *ReFallDamageReplaceTemplate) ReplaceFirstBytes(input []byte) []byte {
	var r ReFallDamageBytesResult
	match, ok := ReFallDamage{}.FindBytesReuse(input, &r)
	if !ok {
		return append([]byte{}, input...)
