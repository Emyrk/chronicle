...
// Code generated by regengo for pattern: (.+[^\s])'s (.+[^\s]) reputation has (increased|decreased) by (\d+)\.
// DO NOT EDIT.

var reReputationChangeStackPool = sync.Pool{New: func() interface{} {
	stack := make([][3]int, 0, 32)
	return &stack
}}

var reReputationChangeCaptureStackPool = sync.Pool{New: func() interface{} {
	stack := make([]int, 0, 160)
	return &stack
}}

type ReReputationChange struct{}

var CompiledReReputationChange = ReReputationChange{}

// MinMatchLen is the minimum number of bytes any match can have.
const ReReputationChangeMinMatchLen = 38

// MaxMatchLen is the maximum number of bytes any match can have.
// -1 means unbounded (pattern contains * or + quantifiers).
const ReReputationChangeMaxMatchLen = -1

func (ReReputationChange) MatchLengthInfo() (minLen, maxLen int) {
	return ReReputationChangeMinMatchLen, ReReputationChangeMaxMatchLen
}

func (ReReputationChange) MatchString(input string) bool {
	l := len(input)
	offset := 0
	stackPtr := reReputationChangeStackPool.Get().(*[][3]int)
	stack := (*stackPtr)[:0]
	defer func() {
		for i := range stack {
			stack[i] = [3]int{0, 0, 0}
		...
		*stackPtr = stack[:0]
		reReputationChangeStackPool.Put(stackPtr)
	}()
	nextInstruction := 1
	goto StepSelect
TryFallback:
...
func (ReReputationChange) MatchBytes(input []byte) bool {
	l := len(input)
	offset := 0
	stackPtr := reReputationChangeStackPool.Get().(*[][3]int)
	stack := (*stackPtr)[:0]
...
type ReReputationChangeResult struct {
...
type ReReputationChangeBytesResult struct {
...
func (ReReputationChange) FindStringReuse(input string, r *ReReputationChangeResult) (*ReReputationChangeResult, bool) {
	l := len(input)
	offset := 0
	var captures [10]int
	stackPtr := reReputationChangeStackPool.Get().(*[][3]int)
...
func (recv ReReputationChange) FindString(input string) (*ReReputationChangeResult, bool) {
	return recv.FindStringReuse(input, nil)
}
func (ReReputationChange) FindAllStringAppend(input string, n int, s []*ReReputationChangeResult) []*ReReputationChangeResult {
	if n == 0 {
		return s
	}
	result := s
	...
	Ins60:
		{
			captures[1] = offset
			var item *ReReputationChangeResult
			if len(result) < cap(result) {
				result = result[:len(result)+1]
				item = result[len(result)-1]
				if item == nil {
					item = &ReReputationChangeResult{}
...
func (r ReReputationChange) FindAllString(input string, n int) []*ReReputationChangeResult {
	return r.FindAllStringAppend(input, n, nil)
}
func (ReReputationChange) FindBytesReuse(input []byte, r *ReReputationChangeBytesResult) (*ReReputationChangeBytesResult, bool) {
	l := len(input)
	offset := 0
	var captures [10]int
	stackPtr := reReputationChangeStackPool.Get().(*[][3]int)
...
Ins60:
	{
		captures[1] = offset
		if r == nil {
			r = &ReReputationChangeBytesResult{}
		}
...
func (recv ReReputationChange) FindBytes(input []byte) (*ReReputationChangeBytesResult, bool) {
	return recv.FindBytesReuse(input, nil)
}
func (ReReputationChange) FindAllBytesAppend(input []byte, n int, s []*ReReputationChangeBytesResult) []*ReReputationChangeBytesResult {
	if n == 0 {
		return s
	}
	result := s
	...
	Ins60:
		{
			captures[1] = offset
			var item *ReReputationChangeBytesResult
			if len(result) < cap(result) {
				result = result[:len(result)+1]
				item = result[len(result)-1]
				if item == nil {
					item = &ReReputationChangeBytesResult{}
					result[len(result)-1] = item
				}
			} else {
				item = &ReReputationChangeBytesResult{}
...
func (r ReReputationChange) FindAllBytes(input []byte, n int) []*ReReputationChangeBytesResult {
...
func (ReReputationChange) DefaultMaxLeftover() int {
	return 1048576
}
...
func (ReReputationChange) FindReader(r io.Reader, cfg stream.Config, onMatch func(stream.Match[*ReReputationChangeBytesResult]) bool) error {
	...
	reuseResult := &ReReputationChangeBytesResult{}
			...
			if err == io.EOF {
				...
				if leftover > 0 {
					...
					for searchPos < len(chunk) {
						result, ok := ReReputationChange{}.FindBytesReuse(chunk[searchPos:], reuseResult)
						if !ok {
							break
						}
						...
						m := stream.Match[*ReReputationChangeBytesResult]{
...
func (ReReputationChange) FindReaderCount(r io.Reader, cfg stream.Config) (int64, error) {
	var count int64
	err := ReReputationChange{}.FindReader(r, cfg, func(_ stream.Match[*ReReputationChangeBytesResult]) bool {
		count++
		return true
	})
	return count, err
...
func (ReReputationChange) FindReaderFirst(r io.Reader, cfg stream.Config) (*ReReputationChangeBytesResult, int64, error) {
	var result *ReReputationChangeBytesResult
	var offset int64
	err := ReReputationChange{}.FindReader(r, cfg, func(m stream.Match[*ReReputationChangeBytesResult]) bool {
		// Copy the result since buffer will be reused
		result = ReReputationChange{}.copyBytesResult(m.Result)
...
func (ReReputationChange) copyBytesResult(src *ReReputationChangeBytesResult) *ReReputationChangeBytesResult {
	if src == nil {
		return nil
	}
	dst := &ReReputationChangeBytesResult{}
	// Copy Match slice
...
//	    })
//	io.Copy(os.Stdout, r)
func (ReReputationChange) NewTransformReader(r io.Reader, cfg stream.TransformConfig, onMatch func(match *ReReputationChangeBytesResult, emit func([]byte))) io.Reader {
	if cfg.BufferSize == 0 {
	...
	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReReputationChange{}.processTransform(data, isEOF, onMatch, emitOut)
	}
...
func (ReReputationChange) processTransform(data []byte, isEOF bool, onMatch func(match *ReReputationChangeBytesResult, emit func([]byte)), emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReReputationChangeBytesResult{}

	for {
		result, ok := ReReputationChange{}.FindBytesReuse(data[processed:], reuseResult)
		if !ok {
			// No more matches
			if isEOF {
...
//	io.Copy(os.Stdout, r)
func (ReReputationChange) ReplaceReader(r io.Reader, template string) io.Reader {
	parsed, err := replace.Parse(template)
	if err != nil {
		return &reReputationChangeTransformErrReader{err: err}
	}
	...
	return ReReputationChange{}.NewTransformReader(r, stream.DefaultTransformConfig(), func(m *ReReputationChangeBytesResult, emit func([]byte)) {
		...
		for _, seg := range resolved {
			...
			case replace.SegmentCaptureIndex:
				capture := (ReReputationChange{}).getCaptureByIndex(m, seg.CaptureIndex)
				if capture != nil {
...
// reReputationChangeTransformErrReader is a reader that always returns an error.
type reReputationChangeTransformErrReader struct {
	err error
}

func (r *reReputationChangeTransformErrReader) Read(p []byte) (int, error) {
	return 0, r.err
}

// getCaptureByIndex returns the capture group value by its index.
func (ReReputationChange) getCaptureByIndex(m *ReReputationChangeBytesResult, index int) []byte {
	switch index {
	case 0:
		return m.Match
	default:
...
//	})
func (ReReputationChange) SelectReader(r io.Reader, pred func(*ReReputationChangeBytesResult) bool) io.Reader {
	cfg := stream.DefaultTransformConfig()
	cfg.MaxLeftover = 1048576

	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReReputationChange{}.processSelect(data, isEOF, pred, emitOut)
	}
...
func (ReReputationChange) processSelect(data []byte, isEOF bool, pred func(*ReReputationChangeBytesResult) bool, emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReReputationChangeBytesResult{}
...
//	})
func (ReReputationChange) RejectReader(r io.Reader, pred func(*ReReputationChangeBytesResult) bool) io.Reader {
	cfg := stream.DefaultTransformConfig()
	cfg.MaxLeftover = 1048576

	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReReputationChange{}.processReject(data, isEOF, pred, emitOut)
	}
...
func (ReReputationChange) processReject(data []byte, isEOF bool, pred func(*ReReputationChangeBytesResult) bool, emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReReputationChangeBytesResult{}
...
func (r *ReReputationChangeResult) CaptureByIndex(idx int) string {
	switch idx {
	case 0:
		return r.Match
	case 1:
...
// CaptureByIndex returns the capture group value by its 0-based index.
// Index 0 returns the full match, 1+ returns capture groups.
func (r *ReReputationChangeBytesResult) CaptureByIndex(idx int) []byte {
	switch idx {
	case 0:
		return r.Match
	case 1:
...
func (ReReputationChange) ReplaceAllString(input string, template string) string {
	...
	lastEnd := 0
	var r ReReputationChangeResult

	remaining := input
	offset := 0

	for {
		match, ok := ReReputationChange{}.FindStringReuse(remaining, &r)
		if !ok {
			break
		}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReReputationChange) ReplaceAllBytes(input []byte, template string) []byte {
	return ReReputationChange{}.ReplaceAllBytesAppend(input, template, nil)
}

// ReplaceAllBytesAppend replaces all matches and appends to buf.
// If buf has sufficient capacity, no allocation occurs.
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReReputationChange) ReplaceAllBytesAppend(input []byte, template string, buf []byte) []byte {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	result := buf[:0]
	lastEnd := 0
	var r ReReputationChangeBytesResult

	remaining := input
	offset := 0

	for {
		match, ok := ReReputationChange{}.FindBytesReuse(remaining, &r)
		if !ok {
			break
		}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReReputationChange) ReplaceFirstString(input string, template string) string {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	var r ReReputationChangeResult
	match, ok := ReReputationChange{}.FindStringReuse(input, &r)
	if !ok {
		return input
	}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReReputationChange) ReplaceFirstBytes(input []byte, template string) []byte {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	var r ReReputationChangeBytesResult
	match, ok := ReReputationChange{}.FindBytesReuse(input, &r)
	if !ok {
...
// ReReputationChangeReplaceTemplate holds a pre-compiled replace template for the ReReputationChange pattern.
// Use CompileReplaceTemplate to create one, then call its Replace methods.
type ReReputationChangeReplaceTemplate struct {
	original string
...
//
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReReputationChange) CompileReplaceTemplate(template string) (*ReReputationChangeReplaceTemplate, error) {
	tmpl, err := replace.Parse(template)
	if err != nil {
		return nil, err
	}
	...
	return &ReReputationChangeReplaceTemplate{
		original: template,
		segments: resolved,
	}, nil
}
...
func (t *ReReputationChangeReplaceTemplate) String() string {
	return t.original
}

// ReplaceAllString replaces all matches in input using this compiled template.
func (t *ReReputationChangeReplaceTemplate) ReplaceAllString(input string) string {
	var result strings.Builder
	lastEnd := 0
	var r ReReputationChangeResult
...
func (t *ReReputationChangeReplaceTemplate) ReplaceAllBytes(input []byte) []byte {
	return t.ReplaceAllBytesAppend(input, nil)
}

// ReplaceAllBytesAppend replaces all matches and appends to buf.
// If buf has sufficient capacity, no allocation occurs.
func (t *ReReputationChangeReplaceTemplate) ReplaceAllBytesAppend(input []byte, buf []byte) []byte {
	result := buf[:0]
	lastEnd := 0
	var r ReReputationChangeBytesResult
...
func (t *ReReputationChangeReplaceTemplate) ReplaceFirstString(input string) string {
	var r ReReputationChangeResult
	match, ok := ReReputationChange{}.FindStringReuse(input, &r)
	if !ok {
		return input
...
func (t *ReReputationChangeReplaceTemplate) ReplaceFirstBytes(input []byte) []byte {
	var r ReReputationChangeBytesResult
	match, ok := ReReputationChange{}.FindBytesReuse(input, &r)
	if !ok {
		return append([]byte{}, input...)
