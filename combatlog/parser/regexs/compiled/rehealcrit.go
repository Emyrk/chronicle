...
// Code generated by regengo for pattern: (.+[^\s])'s (.+[^\s]) critically heals (.+[^\s]) for (\d+)\.
// DO NOT EDIT.

var reHealCritStackPool = sync.Pool{New: func() interface{} {
	stack := make([][3]int, 0, 32)
	return &stack
}}

var reHealCritCaptureStackPool = sync.Pool{New: func() interface{} {
	stack := make([]int, 0, 160)
	return &stack
}}

type ReHealCrit struct{}

var CompiledReHealCrit = ReHealCrit{}

// MinMatchLen is the minimum number of bytes any match can have.
const ReHealCritMinMatchLen = 34

// MaxMatchLen is the maximum number of bytes any match can have.
// -1 means unbounded (pattern contains * or + quantifiers).
const ReHealCritMaxMatchLen = -1

func (ReHealCrit) MatchLengthInfo() (minLen, maxLen int) {
	return ReHealCritMinMatchLen, ReHealCritMaxMatchLen
}

func (ReHealCrit) MatchString(input string) bool {
	l := len(input)
	offset := 0
	stackPtr := reHealCritStackPool.Get().(*[][3]int)
	stack := (*stackPtr)[:0]
	defer func() {
		for i := range stack {
			stack[i] = [3]int{0, 0, 0}
		...
		*stackPtr = stack[:0]
		reHealCritStackPool.Put(stackPtr)
	}()
	nextInstruction := 1
	goto StepSelect
TryFallback:
...
func (ReHealCrit) MatchBytes(input []byte) bool {
	l := len(input)
	offset := 0
	stackPtr := reHealCritStackPool.Get().(*[][3]int)
	stack := (*stackPtr)[:0]
...
type ReHealCritBytesResult struct {
...
func (ReHealCrit) FindStringReuse(input string, r *ReHealCritResult) (*ReHealCritResult, bool) {
	l := len(input)
	offset := 0
	var captures [10]int
	stackPtr := reHealCritStackPool.Get().(*[][3]int)
...
func (recv ReHealCrit) FindString(input string) (*ReHealCritResult, bool) {
	return recv.FindStringReuse(input, nil)
}
func (ReHealCrit) FindAllStringAppend(input string, n int, s []*ReHealCritResult) []*ReHealCritResult {
	if n == 0 {
		return s
	}
	result := s
	...
	Ins47:
		{
			captures[1] = offset
			var item *ReHealCritResult
			if len(result) < cap(result) {
				result = result[:len(result)+1]
				item = result[len(result)-1]
				if item == nil {
					item = &ReHealCritResult{}
...
func (r ReHealCrit) FindAllString(input string, n int) []*ReHealCritResult {
	return r.FindAllStringAppend(input, n, nil)
}
func (ReHealCrit) FindBytesReuse(input []byte, r *ReHealCritBytesResult) (*ReHealCritBytesResult, bool) {
	l := len(input)
	offset := 0
	var captures [10]int
	stackPtr := reHealCritStackPool.Get().(*[][3]int)
...
func (recv ReHealCrit) FindBytes(input []byte) (*ReHealCritBytesResult, bool) {
	return recv.FindBytesReuse(input, nil)
}
func (ReHealCrit) FindAllBytesAppend(input []byte, n int, s []*ReHealCritBytesResult) []*ReHealCritBytesResult {
	if n == 0 {
		return s
	}
	result := s
	...
	Ins47:
		{
			captures[1] = offset
			var item *ReHealCritBytesResult
			if len(result) < cap(result) {
				result = result[:len(result)+1]
				item = result[len(result)-1]
				if item == nil {
					item = &ReHealCritBytesResult{}
					result[len(result)-1] = item
				}
			} else {
				item = &ReHealCritBytesResult{}
...
func (r ReHealCrit) FindAllBytes(input []byte, n int) []*ReHealCritBytesResult {
...
func (ReHealCrit) DefaultMaxLeftover() int {
	return 1048576
}
...
func (ReHealCrit) FindReader(r io.Reader, cfg stream.Config, onMatch func(stream.Match[*ReHealCritBytesResult]) bool) error {
	...
	reuseResult := &ReHealCritBytesResult{}
			...
			if err == io.EOF {
				...
				if leftover > 0 {
					...
					for searchPos < len(chunk) {
						result, ok := ReHealCrit{}.FindBytesReuse(chunk[searchPos:], reuseResult)
						if !ok {
							break
						}
						...
						m := stream.Match[*ReHealCritBytesResult]{
...
func (ReHealCrit) FindReaderCount(r io.Reader, cfg stream.Config) (int64, error) {
	var count int64
	err := ReHealCrit{}.FindReader(r, cfg, func(_ stream.Match[*ReHealCritBytesResult]) bool {
		count++
		return true
	})
	return count, err
...
func (ReHealCrit) FindReaderFirst(r io.Reader, cfg stream.Config) (*ReHealCritBytesResult, int64, error) {
	var result *ReHealCritBytesResult
	var offset int64
	err := ReHealCrit{}.FindReader(r, cfg, func(m stream.Match[*ReHealCritBytesResult]) bool {
		// Copy the result since buffer will be reused
		result = ReHealCrit{}.copyBytesResult(m.Result)
...
func (ReHealCrit) copyBytesResult(src *ReHealCritBytesResult) *ReHealCritBytesResult {
	if src == nil {
		return nil
	}
	dst := &ReHealCritBytesResult{}
	// Copy Match slice
...
//	    })
//	io.Copy(os.Stdout, r)
func (ReHealCrit) NewTransformReader(r io.Reader, cfg stream.TransformConfig, onMatch func(match *ReHealCritBytesResult, emit func([]byte))) io.Reader {
	if cfg.BufferSize == 0 {
	...
	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReHealCrit{}.processTransform(data, isEOF, onMatch, emitOut)
	}
...
func (ReHealCrit) processTransform(data []byte, isEOF bool, onMatch func(match *ReHealCritBytesResult, emit func([]byte)), emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReHealCritBytesResult{}

	for {
		result, ok := ReHealCrit{}.FindBytesReuse(data[processed:], reuseResult)
		if !ok {
			// No more matches
			if isEOF {
...
//	io.Copy(os.Stdout, r)
func (ReHealCrit) ReplaceReader(r io.Reader, template string) io.Reader {
	parsed, err := replace.Parse(template)
	if err != nil {
		return &reHealCritTransformErrReader{err: err}
	}
	...
	return ReHealCrit{}.NewTransformReader(r, stream.DefaultTransformConfig(), func(m *ReHealCritBytesResult, emit func([]byte)) {
		...
		for _, seg := range resolved {
			...
			case replace.SegmentCaptureIndex:
				capture := (ReHealCrit{}).getCaptureByIndex(m, seg.CaptureIndex)
				if capture != nil {
...
// reHealCritTransformErrReader is a reader that always returns an error.
type reHealCritTransformErrReader struct {
	err error
}

func (r *reHealCritTransformErrReader) Read(p []byte) (int, error) {
	return 0, r.err
}

// getCaptureByIndex returns the capture group value by its index.
func (ReHealCrit) getCaptureByIndex(m *ReHealCritBytesResult, index int) []byte {
	switch index {
	case 0:
		return m.Match
	default:
...
//	})
func (ReHealCrit) SelectReader(r io.Reader, pred func(*ReHealCritBytesResult) bool) io.Reader {
	cfg := stream.DefaultTransformConfig()
	cfg.MaxLeftover = 1048576

	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReHealCrit{}.processSelect(data, isEOF, pred, emitOut)
	}
...
func (ReHealCrit) processSelect(data []byte, isEOF bool, pred func(*ReHealCritBytesResult) bool, emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReHealCritBytesResult{}
...
//	})
func (ReHealCrit) RejectReader(r io.Reader, pred func(*ReHealCritBytesResult) bool) io.Reader {
	cfg := stream.DefaultTransformConfig()
	cfg.MaxLeftover = 1048576

	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReHealCrit{}.processReject(data, isEOF, pred, emitOut)
	}
...
func (ReHealCrit) processReject(data []byte, isEOF bool, pred func(*ReHealCritBytesResult) bool, emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReHealCritBytesResult{}
...
func (r *ReHealCritResult) CaptureByIndex(idx int) string {
	switch idx {
	case 0:
		return r.Match
	case 1:
...
// CaptureByIndex returns the capture group value by its 0-based index.
// Index 0 returns the full match, 1+ returns capture groups.
func (r *ReHealCritBytesResult) CaptureByIndex(idx int) []byte {
	switch idx {
	case 0:
		return r.Match
	case 1:
...
func (ReHealCrit) ReplaceAllString(input string, template string) string {
	...
	lastEnd := 0
	var r ReHealCritResult

	remaining := input
	offset := 0

	for {
		match, ok := ReHealCrit{}.FindStringReuse(remaining, &r)
		if !ok {
			break
		}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReHealCrit) ReplaceAllBytes(input []byte, template string) []byte {
	return ReHealCrit{}.ReplaceAllBytesAppend(input, template, nil)
}

// ReplaceAllBytesAppend replaces all matches and appends to buf.
// If buf has sufficient capacity, no allocation occurs.
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReHealCrit) ReplaceAllBytesAppend(input []byte, template string, buf []byte) []byte {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	result := buf[:0]
	lastEnd := 0
	var r ReHealCritBytesResult

	remaining := input
	offset := 0

	for {
		match, ok := ReHealCrit{}.FindBytesReuse(remaining, &r)
		if !ok {
			break
		}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReHealCrit) ReplaceFirstString(input string, template string) string {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	var r ReHealCritResult
	match, ok := ReHealCrit{}.FindStringReuse(input, &r)
	if !ok {
		return input
	}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReHealCrit) ReplaceFirstBytes(input []byte, template string) []byte {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	var r ReHealCritBytesResult
	match, ok := ReHealCrit{}.FindBytesReuse(input, &r)
	if !ok {
...
// ReHealCritReplaceTemplate holds a pre-compiled replace template for the ReHealCrit pattern.
// Use CompileReplaceTemplate to create one, then call its Replace methods.
type ReHealCritReplaceTemplate struct {
	original string
...
//
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReHealCrit) CompileReplaceTemplate(template string) (*ReHealCritReplaceTemplate, error) {
	tmpl, err := replace.Parse(template)
	if err != nil {
		return nil, err
	}
	...
	return &ReHealCritReplaceTemplate{
		original: template,
		segments: resolved,
	}, nil
}
...
func (t *ReHealCritReplaceTemplate) String() string {
	return t.original
}

// ReplaceAllString replaces all matches in input using this compiled template.
func (t *ReHealCritReplaceTemplate) ReplaceAllString(input string) string {
	var result strings.Builder
	lastEnd := 0
	var r ReHealCritResult
...
func (t *ReHealCritReplaceTemplate) ReplaceAllBytes(input []byte) []byte {
	return t.ReplaceAllBytesAppend(input, nil)
}

// ReplaceAllBytesAppend replaces all matches and appends to buf.
// If buf has sufficient capacity, no allocation occurs.
func (t *ReHealCritReplaceTemplate) ReplaceAllBytesAppend(input []byte, buf []byte) []byte {
	result := buf[:0]
	lastEnd := 0
	var r ReHealCritBytesResult
...
func (t *ReHealCritReplaceTemplate) ReplaceFirstString(input string) string {
	var r ReHealCritResult
	match, ok := ReHealCrit{}.FindStringReuse(input, &r)
	if !ok {
		return input
...
func (t *ReHealCritReplaceTemplate) ReplaceFirstBytes(input []byte) []byte {
	var r ReHealCritBytesResult
	match, ok := ReHealCrit{}.FindBytesReuse(input, &r)
	if !ok {
		return append([]byte{}, input...)
