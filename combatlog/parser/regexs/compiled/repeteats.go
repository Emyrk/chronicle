...
// Code generated by regengo for pattern: (.+[^\s])'s pet begins eating a (.+[^\s])\.
// DO NOT EDIT.

var rePetEatsStackPool = sync.Pool{New: func() interface{} {
	stack := make([][3]int, 0, 32)
	return &stack
}}

var rePetEatsCaptureStackPool = sync.Pool{New: func() interface{} {
	stack := make([]int, 0, 96)
	return &stack
}}

type RePetEats struct{}

var CompiledRePetEats = RePetEats{}

// MinMatchLen is the minimum number of bytes any match can have.
const RePetEatsMinMatchLen = 28

// MaxMatchLen is the maximum number of bytes any match can have.
// -1 means unbounded (pattern contains * or + quantifiers).
const RePetEatsMaxMatchLen = -1

func (RePetEats) MatchLengthInfo() (minLen, maxLen int) {
	return RePetEatsMinMatchLen, RePetEatsMaxMatchLen
}

func (RePetEats) MatchString(input string) bool {
	l := len(input)
	offset := 0
	stackPtr := rePetEatsStackPool.Get().(*[][3]int)
	stack := (*stackPtr)[:0]
	defer func() {
		for i := range stack {
			stack[i] = [3]int{0, 0, 0}
		...
		*stackPtr = stack[:0]
		rePetEatsStackPool.Put(stackPtr)
	}()
	nextInstruction := 1
	goto StepSelect
TryFallback:
...
func (RePetEats) MatchBytes(input []byte) bool {
	l := len(input)
	offset := 0
	stackPtr := rePetEatsStackPool.Get().(*[][3]int)
	stack := (*stackPtr)[:0]
...
type RePetEatsResult struct {
	Match  string // Full match
	Group1 string
	Group2 string
}

type RePetEatsBytesResult struct {
	Match  []byte // Full match
	Group1 []byte
	Group2 []byte
}

func (RePetEats) FindStringReuse(input string, r *RePetEatsResult) (*RePetEatsResult, bool) {
	l := len(input)
	offset := 0
	var captures [6]int
	captureStackPtr := rePetEatsCaptureStackPool.Get().(*[]int)
	captureStack := (*captureStackPtr)[:0]
	defer func() {
		*captureStackPtr = captureStack[:0]
		rePetEatsCaptureStackPool.Put(captureStackPtr)
	}()
	stackPtr := rePetEatsStackPool.Get().(*[][3]int)
	stack := (*stackPtr)[:0]
	defer func() {
...
Ins31:
	{
		captureStack = append(captureStack, captures[:]...)
		stack = append(stack, [3]int{offset, 32, 1})
		goto Ins30
	}
Ins32:
	{
...
func (recv RePetEats) FindString(input string) (*RePetEatsResult, bool) {
	return recv.FindStringReuse(input, nil)
}
func (RePetEats) FindAllStringAppend(input string, n int, s []*RePetEatsResult) []*RePetEatsResult {
	if n == 0 {
		return s
	}
	result := s
	...
	Ins35:
		{
			captures[1] = offset
			var item *RePetEatsResult
			if len(result) < cap(result) {
				result = result[:len(result)+1]
				item = result[len(result)-1]
				if item == nil {
					item = &RePetEatsResult{}
...
func (r RePetEats) FindAllString(input string, n int) []*RePetEatsResult {
	return r.FindAllStringAppend(input, n, nil)
}
func (RePetEats) FindBytesReuse(input []byte, r *RePetEatsBytesResult) (*RePetEatsBytesResult, bool) {
	l := len(input)
	offset := 0
	var captures [6]int
	captureStackPtr := rePetEatsCaptureStackPool.Get().(*[]int)
...
func (recv RePetEats) FindBytes(input []byte) (*RePetEatsBytesResult, bool) {
	return recv.FindBytesReuse(input, nil)
}
func (RePetEats) FindAllBytesAppend(input []byte, n int, s []*RePetEatsBytesResult) []*RePetEatsBytesResult {
	if n == 0 {
		return s
	}
	result := s
	...
	Ins35:
		{
			captures[1] = offset
			var item *RePetEatsBytesResult
			if len(result) < cap(result) {
				result = result[:len(result)+1]
				item = result[len(result)-1]
				if item == nil {
					item = &RePetEatsBytesResult{}
					result[len(result)-1] = item
				}
			} else {
				item = &RePetEatsBytesResult{}
...
func (r RePetEats) FindAllBytes(input []byte, n int) []*RePetEatsBytesResult {
...
func (RePetEats) DefaultMaxLeftover() int {
	return 1048576
}
...
func (RePetEats) FindReader(r io.Reader, cfg stream.Config, onMatch func(stream.Match[*RePetEatsBytesResult]) bool) error {
	...
	reuseResult := &RePetEatsBytesResult{}
			...
			if err == io.EOF {
				...
				if leftover > 0 {
					...
					for searchPos < len(chunk) {
						result, ok := RePetEats{}.FindBytesReuse(chunk[searchPos:], reuseResult)
						if !ok {
							break
						}
						...
						m := stream.Match[*RePetEatsBytesResult]{
...
func (RePetEats) FindReaderCount(r io.Reader, cfg stream.Config) (int64, error) {
	var count int64
	err := RePetEats{}.FindReader(r, cfg, func(_ stream.Match[*RePetEatsBytesResult]) bool {
		count++
		return true
	})
	return count, err
...
func (RePetEats) FindReaderFirst(r io.Reader, cfg stream.Config) (*RePetEatsBytesResult, int64, error) {
	var result *RePetEatsBytesResult
	var offset int64
	err := RePetEats{}.FindReader(r, cfg, func(m stream.Match[*RePetEatsBytesResult]) bool {
		// Copy the result since buffer will be reused
		result = RePetEats{}.copyBytesResult(m.Result)
...
func (RePetEats) copyBytesResult(src *RePetEatsBytesResult) *RePetEatsBytesResult {
	if src == nil {
		return nil
	}
	dst := &RePetEatsBytesResult{}
	// Copy Match slice
...
//	    })
//	io.Copy(os.Stdout, r)
func (RePetEats) NewTransformReader(r io.Reader, cfg stream.TransformConfig, onMatch func(match *RePetEatsBytesResult, emit func([]byte))) io.Reader {
	if cfg.BufferSize == 0 {
	...
	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return RePetEats{}.processTransform(data, isEOF, onMatch, emitOut)
	}
...
func (RePetEats) processTransform(data []byte, isEOF bool, onMatch func(match *RePetEatsBytesResult, emit func([]byte)), emitOut func([]byte)) int {
	processed := 0
	reuseResult := &RePetEatsBytesResult{}

	for {
		result, ok := RePetEats{}.FindBytesReuse(data[processed:], reuseResult)
		if !ok {
			// No more matches
			if isEOF {
...
//	io.Copy(os.Stdout, r)
func (RePetEats) ReplaceReader(r io.Reader, template string) io.Reader {
	parsed, err := replace.Parse(template)
	if err != nil {
		return &rePetEatsTransformErrReader{err: err}
	}
	...
	return RePetEats{}.NewTransformReader(r, stream.DefaultTransformConfig(), func(m *RePetEatsBytesResult, emit func([]byte)) {
		...
		for _, seg := range resolved {
			...
			case replace.SegmentCaptureIndex:
				capture := (RePetEats{}).getCaptureByIndex(m, seg.CaptureIndex)
				if capture != nil {
...
// rePetEatsTransformErrReader is a reader that always returns an error.
type rePetEatsTransformErrReader struct {
	err error
}

func (r *rePetEatsTransformErrReader) Read(p []byte) (int, error) {
	return 0, r.err
}

// getCaptureByIndex returns the capture group value by its index.
func (RePetEats) getCaptureByIndex(m *RePetEatsBytesResult, index int) []byte {
	switch index {
	case 0:
		return m.Match
	default:
...
//	})
func (RePetEats) SelectReader(r io.Reader, pred func(*RePetEatsBytesResult) bool) io.Reader {
	cfg := stream.DefaultTransformConfig()
	cfg.MaxLeftover = 1048576

	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return RePetEats{}.processSelect(data, isEOF, pred, emitOut)
	}
...
func (RePetEats) processSelect(data []byte, isEOF bool, pred func(*RePetEatsBytesResult) bool, emitOut func([]byte)) int {
	processed := 0
	reuseResult := &RePetEatsBytesResult{}
...
//	})
func (RePetEats) RejectReader(r io.Reader, pred func(*RePetEatsBytesResult) bool) io.Reader {
	cfg := stream.DefaultTransformConfig()
	cfg.MaxLeftover = 1048576

	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return RePetEats{}.processReject(data, isEOF, pred, emitOut)
	}
...
func (RePetEats) processReject(data []byte, isEOF bool, pred func(*RePetEatsBytesResult) bool, emitOut func([]byte)) int {
	processed := 0
	reuseResult := &RePetEatsBytesResult{}
...
func (r *RePetEatsResult) CaptureByIndex(idx int) string {
	switch idx {
	case 0:
		return r.Match
	case 1:
...
// CaptureByIndex returns the capture group value by its 0-based index.
// Index 0 returns the full match, 1+ returns capture groups.
func (r *RePetEatsBytesResult) CaptureByIndex(idx int) []byte {
	switch idx {
	case 0:
		return r.Match
	case 1:
...
func (RePetEats) ReplaceAllString(input string, template string) string {
	...
	lastEnd := 0
	var r RePetEatsResult

	remaining := input
	offset := 0

	for {
		match, ok := RePetEats{}.FindStringReuse(remaining, &r)
		if !ok {
			break
		}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (RePetEats) ReplaceAllBytes(input []byte, template string) []byte {
	return RePetEats{}.ReplaceAllBytesAppend(input, template, nil)
}

// ReplaceAllBytesAppend replaces all matches and appends to buf.
// If buf has sufficient capacity, no allocation occurs.
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (RePetEats) ReplaceAllBytesAppend(input []byte, template string, buf []byte) []byte {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	result := buf[:0]
	lastEnd := 0
	var r RePetEatsBytesResult

	remaining := input
	offset := 0

	for {
		match, ok := RePetEats{}.FindBytesReuse(remaining, &r)
		if !ok {
			break
		}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (RePetEats) ReplaceFirstString(input string, template string) string {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	var r RePetEatsResult
	match, ok := RePetEats{}.FindStringReuse(input, &r)
	if !ok {
		return input
	}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (RePetEats) ReplaceFirstBytes(input []byte, template string) []byte {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	var r RePetEatsBytesResult
	match, ok := RePetEats{}.FindBytesReuse(input, &r)
	if !ok {
...
// RePetEatsReplaceTemplate holds a pre-compiled replace template for the RePetEats pattern.
// Use CompileReplaceTemplate to create one, then call its Replace methods.
type RePetEatsReplaceTemplate struct {
	original string
...
//
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (RePetEats) CompileReplaceTemplate(template string) (*RePetEatsReplaceTemplate, error) {
	tmpl, err := replace.Parse(template)
	if err != nil {
		return nil, err
	}
	...
	return &RePetEatsReplaceTemplate{
		original: template,
		segments: resolved,
	}, nil
}
...
func (t *RePetEatsReplaceTemplate) String() string {
	return t.original
}

// ReplaceAllString replaces all matches in input using this compiled template.
func (t *RePetEatsReplaceTemplate) ReplaceAllString(input string) string {
	var result strings.Builder
	lastEnd := 0
	var r RePetEatsResult
...
func (t *RePetEatsReplaceTemplate) ReplaceAllBytes(input []byte) []byte {
	return t.ReplaceAllBytesAppend(input, nil)
}

// ReplaceAllBytesAppend replaces all matches and appends to buf.
// If buf has sufficient capacity, no allocation occurs.
func (t *RePetEatsReplaceTemplate) ReplaceAllBytesAppend(input []byte, buf []byte) []byte {
	result := buf[:0]
	lastEnd := 0
	var r RePetEatsBytesResult
...
func (t *RePetEatsReplaceTemplate) ReplaceFirstString(input string) string {
	var r RePetEatsResult
	match, ok := RePetEats{}.FindStringReuse(input, &r)
	if !ok {
		return input
...
func (t *RePetEatsReplaceTemplate) ReplaceFirstBytes(input []byte) []byte {
	var r RePetEatsBytesResult
	match, ok := RePetEats{}.FindBytesReuse(input, &r)
	if !ok {
		return append([]byte{}, input...)
