...
// Code generated by regengo for pattern: (.+[^\s]) is immune to (.+[^\s])'s (.+[^\s])\.
// DO NOT EDIT.

var reFullImmuneStackPool = sync.Pool{New: func() interface{} {
	stack := make([][3]int, 0, 32)
	return &stack
}}

var reFullImmuneCaptureStackPool = sync.Pool{New: func() interface{} {
	stack := make([]int, 0, 128)
	return &stack
}}

type ReFullImmune struct{}

var CompiledReFullImmune = ReFullImmune{}

// MinMatchLen is the minimum number of bytes any match can have.
const ReFullImmuneMinMatchLen = 24

// MaxMatchLen is the maximum number of bytes any match can have.
// -1 means unbounded (pattern contains * or + quantifiers).
const ReFullImmuneMaxMatchLen = -1

func (ReFullImmune) MatchLengthInfo() (minLen, maxLen int) {
	return ReFullImmuneMinMatchLen, ReFullImmuneMaxMatchLen
}

func (ReFullImmune) MatchString(input string) bool {
	l := len(input)
	offset := 0
	stackPtr := reFullImmuneStackPool.Get().(*[][3]int)
	stack := (*stackPtr)[:0]
	defer func() {
		for i := range stack {
			stack[i] = [3]int{0, 0, 0}
		...
		*stackPtr = stack[:0]
		reFullImmuneStackPool.Put(stackPtr)
	}()
	nextInstruction := 1
	goto StepSelect
TryFallback:
...
func (ReFullImmune) MatchBytes(input []byte) bool {
	l := len(input)
	offset := 0
	stackPtr := reFullImmuneStackPool.Get().(*[][3]int)
	stack := (*stackPtr)[:0]
...
type ReFullImmuneResult struct {
...
type ReFullImmuneBytesResult struct {
...
func (ReFullImmune) FindStringReuse(input string, r *ReFullImmuneResult) (*ReFullImmuneResult, bool) {
	l := len(input)
	offset := 0
	var captures [8]int
	captureStackPtr := reFullImmuneCaptureStackPool.Get().(*[]int)
	captureStack := (*captureStackPtr)[:0]
	defer func() {
		*captureStackPtr = captureStack[:0]
		reFullImmuneCaptureStackPool.Put(captureStackPtr)
	}()
	stackPtr := reFullImmuneStackPool.Get().(*[][3]int)
	stack := (*stackPtr)[:0]
	defer func() {
...
Ins30:
	{
		captureStack = append(captureStack, captures[:]...)
		stack = append(stack, [3]int{offset, 31, 1})
		goto Ins29
	}
Ins31:
	{
...
func (recv ReFullImmune) FindString(input string) (*ReFullImmuneResult, bool) {
	return recv.FindStringReuse(input, nil)
}
func (ReFullImmune) FindAllStringAppend(input string, n int, s []*ReFullImmuneResult) []*ReFullImmuneResult {
	if n == 0 {
		return s
	}
	result := s
	...
	Ins34:
		{
			captures[1] = offset
			var item *ReFullImmuneResult
			if len(result) < cap(result) {
				result = result[:len(result)+1]
				item = result[len(result)-1]
				if item == nil {
					item = &ReFullImmuneResult{}
...
func (r ReFullImmune) FindAllString(input string, n int) []*ReFullImmuneResult {
	return r.FindAllStringAppend(input, n, nil)
}
func (ReFullImmune) FindBytesReuse(input []byte, r *ReFullImmuneBytesResult) (*ReFullImmuneBytesResult, bool) {
	l := len(input)
	offset := 0
	var captures [8]int
	captureStackPtr := reFullImmuneCaptureStackPool.Get().(*[]int)
...
func (recv ReFullImmune) FindBytes(input []byte) (*ReFullImmuneBytesResult, bool) {
	return recv.FindBytesReuse(input, nil)
}
func (ReFullImmune) FindAllBytesAppend(input []byte, n int, s []*ReFullImmuneBytesResult) []*ReFullImmuneBytesResult {
	if n == 0 {
		return s
	}
	result := s
	...
	Ins34:
		{
			captures[1] = offset
			var item *ReFullImmuneBytesResult
			if len(result) < cap(result) {
				result = result[:len(result)+1]
				item = result[len(result)-1]
				if item == nil {
					item = &ReFullImmuneBytesResult{}
					result[len(result)-1] = item
				}
			} else {
				item = &ReFullImmuneBytesResult{}
...
func (r ReFullImmune) FindAllBytes(input []byte, n int) []*ReFullImmuneBytesResult {
...
func (ReFullImmune) DefaultMaxLeftover() int {
	return 1048576
}
...
func (ReFullImmune) FindReader(r io.Reader, cfg stream.Config, onMatch func(stream.Match[*ReFullImmuneBytesResult]) bool) error {
	...
	reuseResult := &ReFullImmuneBytesResult{}
			...
			if err == io.EOF {
				...
				if leftover > 0 {
					...
					for searchPos < len(chunk) {
						result, ok := ReFullImmune{}.FindBytesReuse(chunk[searchPos:], reuseResult)
						if !ok {
							break
						}
						...
						m := stream.Match[*ReFullImmuneBytesResult]{
...
func (ReFullImmune) FindReaderCount(r io.Reader, cfg stream.Config) (int64, error) {
	var count int64
	err := ReFullImmune{}.FindReader(r, cfg, func(_ stream.Match[*ReFullImmuneBytesResult]) bool {
		count++
		return true
	})
	return count, err
...
func (ReFullImmune) FindReaderFirst(r io.Reader, cfg stream.Config) (*ReFullImmuneBytesResult, int64, error) {
	var result *ReFullImmuneBytesResult
	var offset int64
	err := ReFullImmune{}.FindReader(r, cfg, func(m stream.Match[*ReFullImmuneBytesResult]) bool {
		// Copy the result since buffer will be reused
		result = ReFullImmune{}.copyBytesResult(m.Result)
...
func (ReFullImmune) copyBytesResult(src *ReFullImmuneBytesResult) *ReFullImmuneBytesResult {
	if src == nil {
		return nil
	}
	dst := &ReFullImmuneBytesResult{}
	// Copy Match slice
...
//	    })
//	io.Copy(os.Stdout, r)
func (ReFullImmune) NewTransformReader(r io.Reader, cfg stream.TransformConfig, onMatch func(match *ReFullImmuneBytesResult, emit func([]byte))) io.Reader {
	if cfg.BufferSize == 0 {
	...
	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReFullImmune{}.processTransform(data, isEOF, onMatch, emitOut)
	}
...
func (ReFullImmune) processTransform(data []byte, isEOF bool, onMatch func(match *ReFullImmuneBytesResult, emit func([]byte)), emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReFullImmuneBytesResult{}

	for {
		result, ok := ReFullImmune{}.FindBytesReuse(data[processed:], reuseResult)
		if !ok {
			// No more matches
			if isEOF {
...
//	io.Copy(os.Stdout, r)
func (ReFullImmune) ReplaceReader(r io.Reader, template string) io.Reader {
	parsed, err := replace.Parse(template)
	if err != nil {
		return &reFullImmuneTransformErrReader{err: err}
	}
	...
	return ReFullImmune{}.NewTransformReader(r, stream.DefaultTransformConfig(), func(m *ReFullImmuneBytesResult, emit func([]byte)) {
		...
		for _, seg := range resolved {
			...
			case replace.SegmentCaptureIndex:
				capture := (ReFullImmune{}).getCaptureByIndex(m, seg.CaptureIndex)
				if capture != nil {
...
// reFullImmuneTransformErrReader is a reader that always returns an error.
type reFullImmuneTransformErrReader struct {
	err error
}

func (r *reFullImmuneTransformErrReader) Read(p []byte) (int, error) {
	return 0, r.err
}

// getCaptureByIndex returns the capture group value by its index.
func (ReFullImmune) getCaptureByIndex(m *ReFullImmuneBytesResult, index int) []byte {
	switch index {
	case 0:
		return m.Match
	default:
...
//	})
func (ReFullImmune) SelectReader(r io.Reader, pred func(*ReFullImmuneBytesResult) bool) io.Reader {
	cfg := stream.DefaultTransformConfig()
	cfg.MaxLeftover = 1048576

	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReFullImmune{}.processSelect(data, isEOF, pred, emitOut)
	}
...
func (ReFullImmune) processSelect(data []byte, isEOF bool, pred func(*ReFullImmuneBytesResult) bool, emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReFullImmuneBytesResult{}
...
//	})
func (ReFullImmune) RejectReader(r io.Reader, pred func(*ReFullImmuneBytesResult) bool) io.Reader {
	cfg := stream.DefaultTransformConfig()
	cfg.MaxLeftover = 1048576

	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReFullImmune{}.processReject(data, isEOF, pred, emitOut)
	}
...
func (ReFullImmune) processReject(data []byte, isEOF bool, pred func(*ReFullImmuneBytesResult) bool, emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReFullImmuneBytesResult{}
...
func (r *ReFullImmuneResult) CaptureByIndex(idx int) string {
	switch idx {
	case 0:
		return r.Match
	case 1:
...
// CaptureByIndex returns the capture group value by its 0-based index.
// Index 0 returns the full match, 1+ returns capture groups.
func (r *ReFullImmuneBytesResult) CaptureByIndex(idx int) []byte {
	switch idx {
	case 0:
		return r.Match
	case 1:
...
func (ReFullImmune) ReplaceAllString(input string, template string) string {
	...
	lastEnd := 0
	var r ReFullImmuneResult

	remaining := input
	offset := 0

	for {
		match, ok := ReFullImmune{}.FindStringReuse(remaining, &r)
		if !ok {
			break
		}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReFullImmune) ReplaceAllBytes(input []byte, template string) []byte {
	return ReFullImmune{}.ReplaceAllBytesAppend(input, template, nil)
}

// ReplaceAllBytesAppend replaces all matches and appends to buf.
// If buf has sufficient capacity, no allocation occurs.
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReFullImmune) ReplaceAllBytesAppend(input []byte, template string, buf []byte) []byte {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	result := buf[:0]
	lastEnd := 0
	var r ReFullImmuneBytesResult

	remaining := input
	offset := 0

	for {
		match, ok := ReFullImmune{}.FindBytesReuse(remaining, &r)
		if !ok {
			break
		}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReFullImmune) ReplaceFirstString(input string, template string) string {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	var r ReFullImmuneResult
	match, ok := ReFullImmune{}.FindStringReuse(input, &r)
	if !ok {
		return input
	}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReFullImmune) ReplaceFirstBytes(input []byte, template string) []byte {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	var r ReFullImmuneBytesResult
	match, ok := ReFullImmune{}.FindBytesReuse(input, &r)
	if !ok {
...
// ReFullImmuneReplaceTemplate holds a pre-compiled replace template for the ReFullImmune pattern.
// Use CompileReplaceTemplate to create one, then call its Replace methods.
type ReFullImmuneReplaceTemplate struct {
	original string
...
//
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReFullImmune) CompileReplaceTemplate(template string) (*ReFullImmuneReplaceTemplate, error) {
	tmpl, err := replace.Parse(template)
	if err != nil {
		return nil, err
	}
	...
	return &ReFullImmuneReplaceTemplate{
		original: template,
		segments: resolved,
	}, nil
}
...
func (t *ReFullImmuneReplaceTemplate) String() string {
	return t.original
}

// ReplaceAllString replaces all matches in input using this compiled template.
func (t *ReFullImmuneReplaceTemplate) ReplaceAllString(input string) string {
	var result strings.Builder
	lastEnd := 0
	var r ReFullImmuneResult
...
func (t *ReFullImmuneReplaceTemplate) ReplaceAllBytes(input []byte) []byte {
	return t.ReplaceAllBytesAppend(input, nil)
}

// ReplaceAllBytesAppend replaces all matches and appends to buf.
// If buf has sufficient capacity, no allocation occurs.
func (t *ReFullImmuneReplaceTemplate) ReplaceAllBytesAppend(input []byte, buf []byte) []byte {
	result := buf[:0]
	lastEnd := 0
	var r ReFullImmuneBytesResult
...
func (t *ReFullImmuneReplaceTemplate) ReplaceFirstString(input string) string {
	var r ReFullImmuneResult
	match, ok := ReFullImmune{}.FindStringReuse(input, &r)
	if !ok {
		return input
...
func (t *ReFullImmuneReplaceTemplate) ReplaceFirstBytes(input []byte) []byte {
	var r ReFullImmuneBytesResult
	match, ok := ReFullImmune{}.FindBytesReuse(input, &r)
	if !ok {
		return append([]byte{}, input...)
