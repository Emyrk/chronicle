...
// Code generated by regengo for pattern: (.+[^\s]) interrupts (.+[^\s])'s (.+[^\s])\.
// DO NOT EDIT.

var reAuraInterruptStackPool = sync.Pool{New: func() interface{} {
	stack := make([][3]int, 0, 32)
	return &stack
}}

var reAuraInterruptCaptureStackPool = sync.Pool{New: func() interface{} {
	stack := make([]int, 0, 128)
	return &stack
}}

type ReAuraInterrupt struct{}

var CompiledReAuraInterrupt = ReAuraInterrupt{}

// MinMatchLen is the minimum number of bytes any match can have.
const ReAuraInterruptMinMatchLen = 22

// MaxMatchLen is the maximum number of bytes any match can have.
// -1 means unbounded (pattern contains * or + quantifiers).
const ReAuraInterruptMaxMatchLen = -1

func (ReAuraInterrupt) MatchLengthInfo() (minLen, maxLen int) {
	return ReAuraInterruptMinMatchLen, ReAuraInterruptMaxMatchLen
}

func (ReAuraInterrupt) MatchString(input string) bool {
	l := len(input)
	offset := 0
	stackPtr := reAuraInterruptStackPool.Get().(*[][3]int)
	stack := (*stackPtr)[:0]
	defer func() {
		for i := range stack {
			stack[i] = [3]int{0, 0, 0}
		...
		*stackPtr = stack[:0]
		reAuraInterruptStackPool.Put(stackPtr)
	}()
	nextInstruction := 1
	goto StepSelect
TryFallback:
...
func (ReAuraInterrupt) MatchBytes(input []byte) bool {
	l := len(input)
	offset := 0
	stackPtr := reAuraInterruptStackPool.Get().(*[][3]int)
	stack := (*stackPtr)[:0]
...
type ReAuraInterruptResult struct {
...
type ReAuraInterruptBytesResult struct {
...
func (ReAuraInterrupt) FindStringReuse(input string, r *ReAuraInterruptResult) (*ReAuraInterruptResult, bool) {
	l := len(input)
	offset := 0
	var captures [8]int
	captureStackPtr := reAuraInterruptCaptureStackPool.Get().(*[]int)
	captureStack := (*captureStackPtr)[:0]
	defer func() {
		*captureStackPtr = captureStack[:0]
		reAuraInterruptCaptureStackPool.Put(captureStackPtr)
	}()
	stackPtr := reAuraInterruptStackPool.Get().(*[][3]int)
	stack := (*stackPtr)[:0]
	defer func() {
...
Ins20:
	{
		captureStack = append(captureStack, captures[:]...)
		stack = append(stack, [3]int{offset, 21, 1})
		goto Ins19
	}
Ins21:
	{
...
Ins28:
	{
		captureStack = append(captureStack, captures[:]...)
		stack = append(stack, [3]int{offset, 29, 1})
		goto Ins27
	}
Ins29:
	{
...
func (recv ReAuraInterrupt) FindString(input string) (*ReAuraInterruptResult, bool) {
	return recv.FindStringReuse(input, nil)
}
func (ReAuraInterrupt) FindAllStringAppend(input string, n int, s []*ReAuraInterruptResult) []*ReAuraInterruptResult {
	if n == 0 {
		return s
	}
	result := s
	...
	Ins32:
		{
			captures[1] = offset
			var item *ReAuraInterruptResult
			if len(result) < cap(result) {
				result = result[:len(result)+1]
				item = result[len(result)-1]
				if item == nil {
					item = &ReAuraInterruptResult{}
...
func (r ReAuraInterrupt) FindAllString(input string, n int) []*ReAuraInterruptResult {
	return r.FindAllStringAppend(input, n, nil)
}
func (ReAuraInterrupt) FindBytesReuse(input []byte, r *ReAuraInterruptBytesResult) (*ReAuraInterruptBytesResult, bool) {
	l := len(input)
	offset := 0
	var captures [8]int
	captureStackPtr := reAuraInterruptCaptureStackPool.Get().(*[]int)
...
func (recv ReAuraInterrupt) FindBytes(input []byte) (*ReAuraInterruptBytesResult, bool) {
	return recv.FindBytesReuse(input, nil)
}
func (ReAuraInterrupt) FindAllBytesAppend(input []byte, n int, s []*ReAuraInterruptBytesResult) []*ReAuraInterruptBytesResult {
	if n == 0 {
		return s
	}
	result := s
	...
	Ins32:
		{
			captures[1] = offset
			var item *ReAuraInterruptBytesResult
			if len(result) < cap(result) {
				result = result[:len(result)+1]
				item = result[len(result)-1]
				if item == nil {
					item = &ReAuraInterruptBytesResult{}
					result[len(result)-1] = item
				}
			} else {
				item = &ReAuraInterruptBytesResult{}
...
func (r ReAuraInterrupt) FindAllBytes(input []byte, n int) []*ReAuraInterruptBytesResult {
...
func (ReAuraInterrupt) DefaultMaxLeftover() int {
	return 1048576
}
...
func (ReAuraInterrupt) FindReader(r io.Reader, cfg stream.Config, onMatch func(stream.Match[*ReAuraInterruptBytesResult]) bool) error {
	...
	reuseResult := &ReAuraInterruptBytesResult{}
			...
			if err == io.EOF {
				...
				if leftover > 0 {
					...
					for searchPos < len(chunk) {
						result, ok := ReAuraInterrupt{}.FindBytesReuse(chunk[searchPos:], reuseResult)
						if !ok {
							break
						}
						...
						m := stream.Match[*ReAuraInterruptBytesResult]{
...
func (ReAuraInterrupt) FindReaderCount(r io.Reader, cfg stream.Config) (int64, error) {
	var count int64
	err := ReAuraInterrupt{}.FindReader(r, cfg, func(_ stream.Match[*ReAuraInterruptBytesResult]) bool {
		count++
		return true
	})
	return count, err
...
func (ReAuraInterrupt) FindReaderFirst(r io.Reader, cfg stream.Config) (*ReAuraInterruptBytesResult, int64, error) {
	var result *ReAuraInterruptBytesResult
	var offset int64
	err := ReAuraInterrupt{}.FindReader(r, cfg, func(m stream.Match[*ReAuraInterruptBytesResult]) bool {
		// Copy the result since buffer will be reused
		result = ReAuraInterrupt{}.copyBytesResult(m.Result)
...
func (ReAuraInterrupt) copyBytesResult(src *ReAuraInterruptBytesResult) *ReAuraInterruptBytesResult {
	if src == nil {
		return nil
	}
	dst := &ReAuraInterruptBytesResult{}
	// Copy Match slice
...
//	    })
//	io.Copy(os.Stdout, r)
func (ReAuraInterrupt) NewTransformReader(r io.Reader, cfg stream.TransformConfig, onMatch func(match *ReAuraInterruptBytesResult, emit func([]byte))) io.Reader {
	if cfg.BufferSize == 0 {
	...
	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReAuraInterrupt{}.processTransform(data, isEOF, onMatch, emitOut)
	}
...
func (ReAuraInterrupt) processTransform(data []byte, isEOF bool, onMatch func(match *ReAuraInterruptBytesResult, emit func([]byte)), emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReAuraInterruptBytesResult{}

	for {
		result, ok := ReAuraInterrupt{}.FindBytesReuse(data[processed:], reuseResult)
		if !ok {
			// No more matches
			if isEOF {
...
//	io.Copy(os.Stdout, r)
func (ReAuraInterrupt) ReplaceReader(r io.Reader, template string) io.Reader {
	parsed, err := replace.Parse(template)
	if err != nil {
		return &reAuraInterruptTransformErrReader{err: err}
	}
	...
	return ReAuraInterrupt{}.NewTransformReader(r, stream.DefaultTransformConfig(), func(m *ReAuraInterruptBytesResult, emit func([]byte)) {
		...
		for _, seg := range resolved {
			...
			case replace.SegmentCaptureIndex:
				capture := (ReAuraInterrupt{}).getCaptureByIndex(m, seg.CaptureIndex)
				if capture != nil {
...
// reAuraInterruptTransformErrReader is a reader that always returns an error.
type reAuraInterruptTransformErrReader struct {
	err error
}

func (r *reAuraInterruptTransformErrReader) Read(p []byte) (int, error) {
	return 0, r.err
}

// getCaptureByIndex returns the capture group value by its index.
func (ReAuraInterrupt) getCaptureByIndex(m *ReAuraInterruptBytesResult, index int) []byte {
	switch index {
	case 0:
		return m.Match
	default:
...
//	})
func (ReAuraInterrupt) SelectReader(r io.Reader, pred func(*ReAuraInterruptBytesResult) bool) io.Reader {
	cfg := stream.DefaultTransformConfig()
	cfg.MaxLeftover = 1048576

	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReAuraInterrupt{}.processSelect(data, isEOF, pred, emitOut)
	}
...
func (ReAuraInterrupt) processSelect(data []byte, isEOF bool, pred func(*ReAuraInterruptBytesResult) bool, emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReAuraInterruptBytesResult{}
...
//	})
func (ReAuraInterrupt) RejectReader(r io.Reader, pred func(*ReAuraInterruptBytesResult) bool) io.Reader {
	cfg := stream.DefaultTransformConfig()
	cfg.MaxLeftover = 1048576

	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReAuraInterrupt{}.processReject(data, isEOF, pred, emitOut)
	}
...
func (ReAuraInterrupt) processReject(data []byte, isEOF bool, pred func(*ReAuraInterruptBytesResult) bool, emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReAuraInterruptBytesResult{}
...
func (r *ReAuraInterruptResult) CaptureByIndex(idx int) string {
	switch idx {
	case 0:
		return r.Match
	case 1:
...
// CaptureByIndex returns the capture group value by its 0-based index.
// Index 0 returns the full match, 1+ returns capture groups.
func (r *ReAuraInterruptBytesResult) CaptureByIndex(idx int) []byte {
	switch idx {
	case 0:
		return r.Match
	case 1:
...
func (ReAuraInterrupt) ReplaceAllString(input string, template string) string {
	...
	lastEnd := 0
	var r ReAuraInterruptResult

	remaining := input
	offset := 0

	for {
		match, ok := ReAuraInterrupt{}.FindStringReuse(remaining, &r)
		if !ok {
			break
		}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReAuraInterrupt) ReplaceAllBytes(input []byte, template string) []byte {
	return ReAuraInterrupt{}.ReplaceAllBytesAppend(input, template, nil)
}

// ReplaceAllBytesAppend replaces all matches and appends to buf.
// If buf has sufficient capacity, no allocation occurs.
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReAuraInterrupt) ReplaceAllBytesAppend(input []byte, template string, buf []byte) []byte {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	result := buf[:0]
	lastEnd := 0
	var r ReAuraInterruptBytesResult

	remaining := input
	offset := 0

	for {
		match, ok := ReAuraInterrupt{}.FindBytesReuse(remaining, &r)
		if !ok {
			break
		}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReAuraInterrupt) ReplaceFirstString(input string, template string) string {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	var r ReAuraInterruptResult
	match, ok := ReAuraInterrupt{}.FindStringReuse(input, &r)
	if !ok {
		return input
	}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReAuraInterrupt) ReplaceFirstBytes(input []byte, template string) []byte {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	var r ReAuraInterruptBytesResult
	match, ok := ReAuraInterrupt{}.FindBytesReuse(input, &r)
	if !ok {
...
// ReAuraInterruptReplaceTemplate holds a pre-compiled replace template for the ReAuraInterrupt pattern.
// Use CompileReplaceTemplate to create one, then call its Replace methods.
type ReAuraInterruptReplaceTemplate struct {
	original string
...
//
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReAuraInterrupt) CompileReplaceTemplate(template string) (*ReAuraInterruptReplaceTemplate, error) {
	tmpl, err := replace.Parse(template)
	if err != nil {
		return nil, err
	}
	...
	return &ReAuraInterruptReplaceTemplate{
		original: template,
		segments: resolved,
	}, nil
}
...
func (t *ReAuraInterruptReplaceTemplate) String() string {
	return t.original
}

// ReplaceAllString replaces all matches in input using this compiled template.
func (t *ReAuraInterruptReplaceTemplate) ReplaceAllString(input string) string {
	var result strings.Builder
	lastEnd := 0
	var r ReAuraInterruptResult
...
func (t *ReAuraInterruptReplaceTemplate) ReplaceAllBytes(input []byte) []byte {
	return t.ReplaceAllBytesAppend(input, nil)
}

// ReplaceAllBytesAppend replaces all matches and appends to buf.
// If buf has sufficient capacity, no allocation occurs.
func (t *ReAuraInterruptReplaceTemplate) ReplaceAllBytesAppend(input []byte, buf []byte) []byte {
	result := buf[:0]
	lastEnd := 0
	var r ReAuraInterruptBytesResult
...
func (t *ReAuraInterruptReplaceTemplate) ReplaceFirstString(input string) string {
	var r ReAuraInterruptResult
	match, ok := ReAuraInterrupt{}.FindStringReuse(input, &r)
	if !ok {
		return input
...
func (t *ReAuraInterruptReplaceTemplate) ReplaceFirstBytes(input []byte) []byte {
	var r ReAuraInterruptBytesResult
	match, ok := ReAuraInterrupt{}.FindBytesReuse(input, &r)
	if !ok {
		return append([]byte{}, input...)
