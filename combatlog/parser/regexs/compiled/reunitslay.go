...
// Code generated by regengo for pattern: (.+[^\s]) is slain by (.+[^\s])(!|\.)
// DO NOT EDIT.

var reUnitSlayStackPool = sync.Pool{New: func() interface{} {
	stack := make([][3]int, 0, 32)
	return &stack
}}

var reUnitSlayCaptureStackPool = sync.Pool{New: func() interface{} {
	stack := make([]int, 0, 128)
	return &stack
}}

type ReUnitSlay struct{}

var CompiledReUnitSlay = ReUnitSlay{}

// MinMatchLen is the minimum number of bytes any match can have.
const ReUnitSlayMinMatchLen = 18

// MaxMatchLen is the maximum number of bytes any match can have.
// -1 means unbounded (pattern contains * or + quantifiers).
const ReUnitSlayMaxMatchLen = -1

func (ReUnitSlay) MatchLengthInfo() (minLen, maxLen int) {
	return ReUnitSlayMinMatchLen, ReUnitSlayMaxMatchLen
}

func (ReUnitSlay) MatchString(input string) bool {
	l := len(input)
	offset := 0
	stackPtr := reUnitSlayStackPool.Get().(*[][3]int)
	stack := (*stackPtr)[:0]
	defer func() {
		for i := range stack {
			stack[i] = [3]int{0, 0, 0}
		...
		*stackPtr = stack[:0]
		reUnitSlayStackPool.Put(stackPtr)
	}()
	nextInstruction := 1
	goto StepSelect
TryFallback:
...
Ins25:
	...
	{
		if input[offset] != uint8(0x21) && input[offset] != uint8(0x2e) {
			goto TryFallback
		}
		offset++
		goto Ins26
...
func (ReUnitSlay) MatchBytes(input []byte) bool {
	l := len(input)
	offset := 0
	stackPtr := reUnitSlayStackPool.Get().(*[][3]int)
	stack := (*stackPtr)[:0]
...
type ReUnitSlayBytesResult struct {
...
func (ReUnitSlay) FindStringReuse(input string, r *ReUnitSlayResult) (*ReUnitSlayResult, bool) {
	l := len(input)
	offset := 0
	var captures [8]int
	captureStackPtr := reUnitSlayCaptureStackPool.Get().(*[]int)
	captureStack := (*captureStackPtr)[:0]
	defer func() {
		*captureStackPtr = captureStack[:0]
		reUnitSlayCaptureStackPool.Put(captureStackPtr)
	}()
	stackPtr := reUnitSlayStackPool.Get().(*[][3]int)
	stack := (*stackPtr)[:0]
	defer func() {
...
func (recv ReUnitSlay) FindString(input string) (*ReUnitSlayResult, bool) {
	return recv.FindStringReuse(input, nil)
}
func (ReUnitSlay) FindAllStringAppend(input string, n int, s []*ReUnitSlayResult) []*ReUnitSlayResult {
	if n == 0 {
		return s
	}
	result := s
	...
	Ins27:
		{
			captures[1] = offset
			var item *ReUnitSlayResult
			if len(result) < cap(result) {
				result = result[:len(result)+1]
				item = result[len(result)-1]
				if item == nil {
					item = &ReUnitSlayResult{}
...
func (r ReUnitSlay) FindAllString(input string, n int) []*ReUnitSlayResult {
	return r.FindAllStringAppend(input, n, nil)
}
func (ReUnitSlay) FindBytesReuse(input []byte, r *ReUnitSlayBytesResult) (*ReUnitSlayBytesResult, bool) {
	l := len(input)
	offset := 0
	var captures [8]int
	captureStackPtr := reUnitSlayCaptureStackPool.Get().(*[]int)
...
func (recv ReUnitSlay) FindBytes(input []byte) (*ReUnitSlayBytesResult, bool) {
	return recv.FindBytesReuse(input, nil)
}
func (ReUnitSlay) FindAllBytesAppend(input []byte, n int, s []*ReUnitSlayBytesResult) []*ReUnitSlayBytesResult {
	if n == 0 {
		return s
	}
	result := s
	...
	Ins27:
		{
			captures[1] = offset
			var item *ReUnitSlayBytesResult
			if len(result) < cap(result) {
				result = result[:len(result)+1]
				item = result[len(result)-1]
				if item == nil {
					item = &ReUnitSlayBytesResult{}
					result[len(result)-1] = item
				}
			} else {
				item = &ReUnitSlayBytesResult{}
...
func (r ReUnitSlay) FindAllBytes(input []byte, n int) []*ReUnitSlayBytesResult {
...
func (ReUnitSlay) DefaultMaxLeftover() int {
	return 1048576
}
...
func (ReUnitSlay) FindReader(r io.Reader, cfg stream.Config, onMatch func(stream.Match[*ReUnitSlayBytesResult]) bool) error {
	...
	reuseResult := &ReUnitSlayBytesResult{}
			...
			if err == io.EOF {
				...
				if leftover > 0 {
					...
					for searchPos < len(chunk) {
						result, ok := ReUnitSlay{}.FindBytesReuse(chunk[searchPos:], reuseResult)
						if !ok {
							break
						}
						...
						m := stream.Match[*ReUnitSlayBytesResult]{
...
func (ReUnitSlay) FindReaderCount(r io.Reader, cfg stream.Config) (int64, error) {
	var count int64
	err := ReUnitSlay{}.FindReader(r, cfg, func(_ stream.Match[*ReUnitSlayBytesResult]) bool {
		count++
		return true
	})
	return count, err
...
func (ReUnitSlay) FindReaderFirst(r io.Reader, cfg stream.Config) (*ReUnitSlayBytesResult, int64, error) {
	var result *ReUnitSlayBytesResult
	var offset int64
	err := ReUnitSlay{}.FindReader(r, cfg, func(m stream.Match[*ReUnitSlayBytesResult]) bool {
		// Copy the result since buffer will be reused
		result = ReUnitSlay{}.copyBytesResult(m.Result)
...
func (ReUnitSlay) copyBytesResult(src *ReUnitSlayBytesResult) *ReUnitSlayBytesResult {
	if src == nil {
		return nil
	}
	dst := &ReUnitSlayBytesResult{}
	// Copy Match slice
...
//	    })
//	io.Copy(os.Stdout, r)
func (ReUnitSlay) NewTransformReader(r io.Reader, cfg stream.TransformConfig, onMatch func(match *ReUnitSlayBytesResult, emit func([]byte))) io.Reader {
	if cfg.BufferSize == 0 {
	...
	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReUnitSlay{}.processTransform(data, isEOF, onMatch, emitOut)
	}
...
func (ReUnitSlay) processTransform(data []byte, isEOF bool, onMatch func(match *ReUnitSlayBytesResult, emit func([]byte)), emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReUnitSlayBytesResult{}

	for {
		result, ok := ReUnitSlay{}.FindBytesReuse(data[processed:], reuseResult)
		if !ok {
			// No more matches
			if isEOF {
...
//	io.Copy(os.Stdout, r)
func (ReUnitSlay) ReplaceReader(r io.Reader, template string) io.Reader {
	parsed, err := replace.Parse(template)
	if err != nil {
		return &reUnitSlayTransformErrReader{err: err}
	}
	...
	return ReUnitSlay{}.NewTransformReader(r, stream.DefaultTransformConfig(), func(m *ReUnitSlayBytesResult, emit func([]byte)) {
		...
		for _, seg := range resolved {
			...
			case replace.SegmentCaptureIndex:
				capture := (ReUnitSlay{}).getCaptureByIndex(m, seg.CaptureIndex)
				if capture != nil {
...
// reUnitSlayTransformErrReader is a reader that always returns an error.
type reUnitSlayTransformErrReader struct {
	err error
}

func (r *reUnitSlayTransformErrReader) Read(p []byte) (int, error) {
	return 0, r.err
}

// getCaptureByIndex returns the capture group value by its index.
func (ReUnitSlay) getCaptureByIndex(m *ReUnitSlayBytesResult, index int) []byte {
	switch index {
	case 0:
		return m.Match
	default:
...
//	})
func (ReUnitSlay) SelectReader(r io.Reader, pred func(*ReUnitSlayBytesResult) bool) io.Reader {
	cfg := stream.DefaultTransformConfig()
	cfg.MaxLeftover = 1048576

	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReUnitSlay{}.processSelect(data, isEOF, pred, emitOut)
	}
...
func (ReUnitSlay) processSelect(data []byte, isEOF bool, pred func(*ReUnitSlayBytesResult) bool, emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReUnitSlayBytesResult{}
...
//	})
func (ReUnitSlay) RejectReader(r io.Reader, pred func(*ReUnitSlayBytesResult) bool) io.Reader {
	cfg := stream.DefaultTransformConfig()
	cfg.MaxLeftover = 1048576

	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReUnitSlay{}.processReject(data, isEOF, pred, emitOut)
	}
...
func (ReUnitSlay) processReject(data []byte, isEOF bool, pred func(*ReUnitSlayBytesResult) bool, emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReUnitSlayBytesResult{}
...
func (r *ReUnitSlayResult) CaptureByIndex(idx int) string {
	switch idx {
	case 0:
		return r.Match
	case 1:
...
// CaptureByIndex returns the capture group value by its 0-based index.
// Index 0 returns the full match, 1+ returns capture groups.
func (r *ReUnitSlayBytesResult) CaptureByIndex(idx int) []byte {
	switch idx {
	case 0:
		return r.Match
	case 1:
...
func (ReUnitSlay) ReplaceAllString(input string, template string) string {
	...
	lastEnd := 0
	var r ReUnitSlayResult

	remaining := input
	offset := 0

	for {
		match, ok := ReUnitSlay{}.FindStringReuse(remaining, &r)
		if !ok {
			break
		}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReUnitSlay) ReplaceAllBytes(input []byte, template string) []byte {
	return ReUnitSlay{}.ReplaceAllBytesAppend(input, template, nil)
}

// ReplaceAllBytesAppend replaces all matches and appends to buf.
// If buf has sufficient capacity, no allocation occurs.
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReUnitSlay) ReplaceAllBytesAppend(input []byte, template string, buf []byte) []byte {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	result := buf[:0]
	lastEnd := 0
	var r ReUnitSlayBytesResult

	remaining := input
	offset := 0

	for {
		match, ok := ReUnitSlay{}.FindBytesReuse(remaining, &r)
		if !ok {
			break
		}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReUnitSlay) ReplaceFirstString(input string, template string) string {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	var r ReUnitSlayResult
	match, ok := ReUnitSlay{}.FindStringReuse(input, &r)
	if !ok {
		return input
	}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReUnitSlay) ReplaceFirstBytes(input []byte, template string) []byte {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	var r ReUnitSlayBytesResult
	match, ok := ReUnitSlay{}.FindBytesReuse(input, &r)
	if !ok {
...
// ReUnitSlayReplaceTemplate holds a pre-compiled replace template for the ReUnitSlay pattern.
// Use CompileReplaceTemplate to create one, then call its Replace methods.
type ReUnitSlayReplaceTemplate struct {
	original string
...
//
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReUnitSlay) CompileReplaceTemplate(template string) (*ReUnitSlayReplaceTemplate, error) {
	tmpl, err := replace.Parse(template)
	if err != nil {
		return nil, err
	}
	...
	return &ReUnitSlayReplaceTemplate{
		original: template,
		segments: resolved,
	}, nil
}
...
func (t *ReUnitSlayReplaceTemplate) String() string {
	return t.original
}

// ReplaceAllString replaces all matches in input using this compiled template.
func (t *ReUnitSlayReplaceTemplate) ReplaceAllString(input string) string {
	var result strings.Builder
	lastEnd := 0
	var r ReUnitSlayResult
...
func (t *ReUnitSlayReplaceTemplate) ReplaceAllBytes(input []byte) []byte {
	return t.ReplaceAllBytesAppend(input, nil)
}

// ReplaceAllBytesAppend replaces all matches and appends to buf.
// If buf has sufficient capacity, no allocation occurs.
func (t *ReUnitSlayReplaceTemplate) ReplaceAllBytesAppend(input []byte, buf []byte) []byte {
	result := buf[:0]
	lastEnd := 0
	var r ReUnitSlayBytesResult
...
func (t *ReUnitSlayReplaceTemplate) ReplaceFirstString(input string) string {
	var r ReUnitSlayResult
	match, ok := ReUnitSlay{}.FindStringReuse(input, &r)
	if !ok {
		return input
...
func (t *ReUnitSlayReplaceTemplate) ReplaceFirstBytes(input []byte) []byte {
	var r ReUnitSlayBytesResult
	match, ok := ReUnitSlay{}.FindBytesReuse(input, &r)
	if !ok {
		return append([]byte{}, input...)
