...
// Code generated by regengo for pattern: (.+[^\s])'s (.+?) (critically )?heals (.+[^\s]) for (\d+)\.
// DO NOT EDIT.

var reHealStackPool = sync.Pool{New: func() interface{} {
	stack := make([][3]int, 0, 32)
	return &stack
}}

var reHealCaptureStackPool = sync.Pool{New: func() interface{} {
...
var CompiledReHeal = ReHeal{}

// MinMatchLen is the minimum number of bytes any match can have.
const ReHealMinMatchLen = 22

// MaxMatchLen is the maximum number of bytes any match can have.
// -1 means unbounded (pattern contains * or + quantifiers).
const ReHealMaxMatchLen = -1

func (ReHeal) MatchLengthInfo() (minLen, maxLen int) {
	return ReHealMinMatchLen, ReHealMaxMatchLen
}

func (ReHeal) MatchString(input string) bool {
	l := len(input)
	offset := 0
	stackPtr := reHealStackPool.Get().(*[][3]int)
	stack := (*stackPtr)[:0]
	defer func() {
		for i := range stack {
			stack[i] = [3]int{0, 0, 0}
...
func (ReHeal) MatchBytes(input []byte) bool {
	l := len(input)
	offset := 0
	stackPtr := reHealStackPool.Get().(*[][3]int)
	stack := (*stackPtr)[:0]
...
func (ReHeal) FindStringReuse(input string, r *ReHealResult) (*ReHealResult, bool) {
	l := len(input)
	offset := 0
	var captures [12]int
	stackPtr := reHealStackPool.Get().(*[][3]int)
...
func (recv ReHeal) FindString(input string) (*ReHealResult, bool) {
	return recv.FindStringReuse(input, nil)
}
func (ReHeal) FindAllStringAppend(input string, n int, s []*ReHealResult) []*ReHealResult {
	if n == 0 {
		return s
	}
	result := s
...
func (r ReHeal) FindAllString(input string, n int) []*ReHealResult {
	return r.FindAllStringAppend(input, n, nil)
}
func (ReHeal) FindBytesReuse(input []byte, r *ReHealBytesResult) (*ReHealBytesResult, bool) {
	l := len(input)
	offset := 0
	var captures [12]int
	stackPtr := reHealStackPool.Get().(*[][3]int)
...
func (recv ReHeal) FindBytes(input []byte) (*ReHealBytesResult, bool) {
	return recv.FindBytesReuse(input, nil)
}
func (ReHeal) FindAllBytesAppend(input []byte, n int, s []*ReHealBytesResult) []*ReHealBytesResult {
	if n == 0 {
		return s
	}
	result := s
	...
	Ins49:
		{
			captures[1] = offset
			var item *ReHealBytesResult
			if len(result) < cap(result) {
				result = result[:len(result)+1]
				item = result[len(result)-1]
				if item == nil {
					item = &ReHealBytesResult{}
					result[len(result)-1] = item
				}
			} else {
				item = &ReHealBytesResult{}
...
func (r ReHeal) FindAllBytes(input []byte, n int) []*ReHealBytesResult {
...
func (ReHeal) DefaultMaxLeftover() int {
	return 1048576
}
...
func (ReHeal) FindReader(r io.Reader, cfg stream.Config, onMatch func(stream.Match[*ReHealBytesResult]) bool) error {
			...
			if err == io.EOF {
				...
				if leftover > 0 {
					...
					for searchPos < len(chunk) {
						result, ok := ReHeal{}.FindBytesReuse(chunk[searchPos:], reuseResult)
						if !ok {
							break
						}
						...
						m := stream.Match[*ReHealBytesResult]{
...
func (ReHeal) FindReaderCount(r io.Reader, cfg stream.Config) (int64, error) {
	var count int64
	err := ReHeal{}.FindReader(r, cfg, func(_ stream.Match[*ReHealBytesResult]) bool {
		count++
		return true
	})
	return count, err
...
func (ReHeal) FindReaderFirst(r io.Reader, cfg stream.Config) (*ReHealBytesResult, int64, error) {
	var result *ReHealBytesResult
	var offset int64
	err := ReHeal{}.FindReader(r, cfg, func(m stream.Match[*ReHealBytesResult]) bool {
		// Copy the result since buffer will be reused
		result = ReHeal{}.copyBytesResult(m.Result)
...
func (ReHeal) copyBytesResult(src *ReHealBytesResult) *ReHealBytesResult {
	if src == nil {
		return nil
	}
	dst := &ReHealBytesResult{}
	// Copy Match slice
...
//	    })
//	io.Copy(os.Stdout, r)
func (ReHeal) NewTransformReader(r io.Reader, cfg stream.TransformConfig, onMatch func(match *ReHealBytesResult, emit func([]byte))) io.Reader {
	if cfg.BufferSize == 0 {
	...
	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReHeal{}.processTransform(data, isEOF, onMatch, emitOut)
	}
...
func (ReHeal) processTransform(data []byte, isEOF bool, onMatch func(match *ReHealBytesResult, emit func([]byte)), emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReHealBytesResult{}

	for {
		result, ok := ReHeal{}.FindBytesReuse(data[processed:], reuseResult)
		if !ok {
			// No more matches
			if isEOF {
...
//	io.Copy(os.Stdout, r)
func (ReHeal) ReplaceReader(r io.Reader, template string) io.Reader {
	parsed, err := replace.Parse(template)
	if err != nil {
		return &reHealTransformErrReader{err: err}
	}
	...
	return ReHeal{}.NewTransformReader(r, stream.DefaultTransformConfig(), func(m *ReHealBytesResult, emit func([]byte)) {
		...
		for _, seg := range resolved {
			...
			case replace.SegmentCaptureIndex:
				capture := (ReHeal{}).getCaptureByIndex(m, seg.CaptureIndex)
				if capture != nil {
...
// reHealTransformErrReader is a reader that always returns an error.
type reHealTransformErrReader struct {
	err error
}

func (r *reHealTransformErrReader) Read(p []byte) (int, error) {
	return 0, r.err
}

// getCaptureByIndex returns the capture group value by its index.
func (ReHeal) getCaptureByIndex(m *ReHealBytesResult, index int) []byte {
	switch index {
	case 0:
		return m.Match
	default:
...
//	})
func (ReHeal) SelectReader(r io.Reader, pred func(*ReHealBytesResult) bool) io.Reader {
	cfg := stream.DefaultTransformConfig()
	cfg.MaxLeftover = 1048576

	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReHeal{}.processSelect(data, isEOF, pred, emitOut)
	}
...
func (ReHeal) processSelect(data []byte, isEOF bool, pred func(*ReHealBytesResult) bool, emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReHealBytesResult{}
...
//	})
func (ReHeal) RejectReader(r io.Reader, pred func(*ReHealBytesResult) bool) io.Reader {
	cfg := stream.DefaultTransformConfig()
	cfg.MaxLeftover = 1048576

	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReHeal{}.processReject(data, isEOF, pred, emitOut)
	}
...
func (ReHeal) processReject(data []byte, isEOF bool, pred func(*ReHealBytesResult) bool, emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReHealBytesResult{}
...
func (r *ReHealResult) CaptureByIndex(idx int) string {
	switch idx {
	case 0:
		return r.Match
	case 1:
...
// CaptureByIndex returns the capture group value by its 0-based index.
// Index 0 returns the full match, 1+ returns capture groups.
func (r *ReHealBytesResult) CaptureByIndex(idx int) []byte {
	switch idx {
	case 0:
		return r.Match
	case 1:
...
func (ReHeal) ReplaceAllString(input string, template string) string {
	...
	for {
		match, ok := ReHeal{}.FindStringReuse(remaining, &r)
		if !ok {
			break
		}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReHeal) ReplaceAllBytes(input []byte, template string) []byte {
	return ReHeal{}.ReplaceAllBytesAppend(input, template, nil)
}

// ReplaceAllBytesAppend replaces all matches and appends to buf.
// If buf has sufficient capacity, no allocation occurs.
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReHeal) ReplaceAllBytesAppend(input []byte, template string, buf []byte) []byte {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	result := buf[:0]
	lastEnd := 0
	var r ReHealBytesResult

	remaining := input
	offset := 0

	for {
		match, ok := ReHeal{}.FindBytesReuse(remaining, &r)
		if !ok {
			break
		}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReHeal) ReplaceFirstString(input string, template string) string {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	var r ReHealResult
	match, ok := ReHeal{}.FindStringReuse(input, &r)
	if !ok {
		return input
	}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReHeal) ReplaceFirstBytes(input []byte, template string) []byte {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	var r ReHealBytesResult
	match, ok := ReHeal{}.FindBytesReuse(input, &r)
	if !ok {
...
// ReHealReplaceTemplate holds a pre-compiled replace template for the ReHeal pattern.
// Use CompileReplaceTemplate to create one, then call its Replace methods.
type ReHealReplaceTemplate struct {
	original string
...
//
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReHeal) CompileReplaceTemplate(template string) (*ReHealReplaceTemplate, error) {
	tmpl, err := replace.Parse(template)
	if err != nil {
		return nil, err
	}
...
func (t *ReHealReplaceTemplate) String() string {
	return t.original
}

// ReplaceAllString replaces all matches in input using this compiled template.
func (t *ReHealReplaceTemplate) ReplaceAllString(input string) string {
	var result strings.Builder
	lastEnd := 0
	var r ReHealResult
...
func (t *ReHealReplaceTemplate) ReplaceAllBytes(input []byte) []byte {
	return t.ReplaceAllBytesAppend(input, nil)
}

// ReplaceAllBytesAppend replaces all matches and appends to buf.
// If buf has sufficient capacity, no allocation occurs.
func (t *ReHealReplaceTemplate) ReplaceAllBytesAppend(input []byte, buf []byte) []byte {
	result := buf[:0]
	lastEnd := 0
	var r ReHealBytesResult
...
func (t *ReHealReplaceTemplate) ReplaceFirstString(input string) string {
	var r ReHealResult
	match, ok := ReHeal{}.FindStringReuse(input, &r)
	if !ok {
		return input
...
func (t *ReHealReplaceTemplate) ReplaceFirstBytes(input []byte) []byte {
	var r ReHealBytesResult
	match, ok := ReHeal{}.FindBytesReuse(input, &r)
	if !ok {
		return append([]byte{}, input...)
