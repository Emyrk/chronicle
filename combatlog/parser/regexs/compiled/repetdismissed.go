...
// Code generated by regengo for pattern: (.+[^\s])'s (.+[^\s]) is dismissed\.
// DO NOT EDIT.

var rePetDismissedStackPool = sync.Pool{New: func() interface{} {
	stack := make([][3]int, 0, 32)
	return &stack
}}

var rePetDismissedCaptureStackPool = sync.Pool{New: func() interface{} {
	stack := make([]int, 0, 96)
	return &stack
}}

type RePetDismissed struct{}

var CompiledRePetDismissed = RePetDismissed{}

// MinMatchLen is the minimum number of bytes any match can have.
const RePetDismissedMinMatchLen = 21

// MaxMatchLen is the maximum number of bytes any match can have.
// -1 means unbounded (pattern contains * or + quantifiers).
const RePetDismissedMaxMatchLen = -1

func (RePetDismissed) MatchLengthInfo() (minLen, maxLen int) {
	return RePetDismissedMinMatchLen, RePetDismissedMaxMatchLen
}

func (RePetDismissed) MatchString(input string) bool {
	l := len(input)
	offset := 0
	stackPtr := rePetDismissedStackPool.Get().(*[][3]int)
	stack := (*stackPtr)[:0]
	defer func() {
		for i := range stack {
			stack[i] = [3]int{0, 0, 0}
		...
		*stackPtr = stack[:0]
		rePetDismissedStackPool.Put(stackPtr)
	}()
	nextInstruction := 1
	goto StepSelect
TryFallback:
...
func (RePetDismissed) MatchBytes(input []byte) bool {
	l := len(input)
	offset := 0
	stackPtr := rePetDismissedStackPool.Get().(*[][3]int)
	stack := (*stackPtr)[:0]
...
type RePetDismissedResult struct {
	Match  string // Full match
	Group1 string
	Group2 string
}

type RePetDismissedBytesResult struct {
	Match  []byte // Full match
	Group1 []byte
	Group2 []byte
}

func (RePetDismissed) FindStringReuse(input string, r *RePetDismissedResult) (*RePetDismissedResult, bool) {
	l := len(input)
	offset := 0
	var captures [6]int
	captureStackPtr := rePetDismissedCaptureStackPool.Get().(*[]int)
	captureStack := (*captureStackPtr)[:0]
	defer func() {
		*captureStackPtr = captureStack[:0]
		rePetDismissedCaptureStackPool.Put(captureStackPtr)
	}()
	stackPtr := rePetDismissedStackPool.Get().(*[][3]int)
	stack := (*stackPtr)[:0]
	defer func() {
...
func (recv RePetDismissed) FindString(input string) (*RePetDismissedResult, bool) {
	return recv.FindStringReuse(input, nil)
}
func (RePetDismissed) FindAllStringAppend(input string, n int, s []*RePetDismissedResult) []*RePetDismissedResult {
	if n == 0 {
		return s
	}
	result := s
	...
	Ins28:
		{
			captures[1] = offset
			var item *RePetDismissedResult
			if len(result) < cap(result) {
				result = result[:len(result)+1]
				item = result[len(result)-1]
				if item == nil {
					item = &RePetDismissedResult{}
...
func (r RePetDismissed) FindAllString(input string, n int) []*RePetDismissedResult {
	return r.FindAllStringAppend(input, n, nil)
}
func (RePetDismissed) FindBytesReuse(input []byte, r *RePetDismissedBytesResult) (*RePetDismissedBytesResult, bool) {
	l := len(input)
	offset := 0
	var captures [6]int
	captureStackPtr := rePetDismissedCaptureStackPool.Get().(*[]int)
...
func (recv RePetDismissed) FindBytes(input []byte) (*RePetDismissedBytesResult, bool) {
	return recv.FindBytesReuse(input, nil)
}
func (RePetDismissed) FindAllBytesAppend(input []byte, n int, s []*RePetDismissedBytesResult) []*RePetDismissedBytesResult {
	if n == 0 {
		return s
	}
	result := s
	...
	Ins28:
		{
			captures[1] = offset
			var item *RePetDismissedBytesResult
			if len(result) < cap(result) {
				result = result[:len(result)+1]
				item = result[len(result)-1]
				if item == nil {
					item = &RePetDismissedBytesResult{}
					result[len(result)-1] = item
				}
			} else {
				item = &RePetDismissedBytesResult{}
...
func (r RePetDismissed) FindAllBytes(input []byte, n int) []*RePetDismissedBytesResult {
...
func (RePetDismissed) DefaultMaxLeftover() int {
	return 1048576
}
...
func (RePetDismissed) FindReader(r io.Reader, cfg stream.Config, onMatch func(stream.Match[*RePetDismissedBytesResult]) bool) error {
	...
	reuseResult := &RePetDismissedBytesResult{}
			...
			if err == io.EOF {
				...
				if leftover > 0 {
					...
					for searchPos < len(chunk) {
						result, ok := RePetDismissed{}.FindBytesReuse(chunk[searchPos:], reuseResult)
						if !ok {
							break
						}
						...
						m := stream.Match[*RePetDismissedBytesResult]{
...
func (RePetDismissed) FindReaderCount(r io.Reader, cfg stream.Config) (int64, error) {
	var count int64
	err := RePetDismissed{}.FindReader(r, cfg, func(_ stream.Match[*RePetDismissedBytesResult]) bool {
		count++
		return true
	})
	return count, err
...
func (RePetDismissed) FindReaderFirst(r io.Reader, cfg stream.Config) (*RePetDismissedBytesResult, int64, error) {
	var result *RePetDismissedBytesResult
	var offset int64
	err := RePetDismissed{}.FindReader(r, cfg, func(m stream.Match[*RePetDismissedBytesResult]) bool {
		// Copy the result since buffer will be reused
		result = RePetDismissed{}.copyBytesResult(m.Result)
...
func (RePetDismissed) copyBytesResult(src *RePetDismissedBytesResult) *RePetDismissedBytesResult {
	if src == nil {
		return nil
	}
	dst := &RePetDismissedBytesResult{}
	// Copy Match slice
...
//	    })
//	io.Copy(os.Stdout, r)
func (RePetDismissed) NewTransformReader(r io.Reader, cfg stream.TransformConfig, onMatch func(match *RePetDismissedBytesResult, emit func([]byte))) io.Reader {
	if cfg.BufferSize == 0 {
	...
	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return RePetDismissed{}.processTransform(data, isEOF, onMatch, emitOut)
	}
...
func (RePetDismissed) processTransform(data []byte, isEOF bool, onMatch func(match *RePetDismissedBytesResult, emit func([]byte)), emitOut func([]byte)) int {
	processed := 0
	reuseResult := &RePetDismissedBytesResult{}

	for {
		result, ok := RePetDismissed{}.FindBytesReuse(data[processed:], reuseResult)
		if !ok {
			// No more matches
			if isEOF {
...
//	io.Copy(os.Stdout, r)
func (RePetDismissed) ReplaceReader(r io.Reader, template string) io.Reader {
	parsed, err := replace.Parse(template)
	if err != nil {
		return &rePetDismissedTransformErrReader{err: err}
	}
	...
	return RePetDismissed{}.NewTransformReader(r, stream.DefaultTransformConfig(), func(m *RePetDismissedBytesResult, emit func([]byte)) {
		...
		for _, seg := range resolved {
			...
			case replace.SegmentCaptureIndex:
				capture := (RePetDismissed{}).getCaptureByIndex(m, seg.CaptureIndex)
				if capture != nil {
...
// rePetDismissedTransformErrReader is a reader that always returns an error.
type rePetDismissedTransformErrReader struct {
	err error
}

func (r *rePetDismissedTransformErrReader) Read(p []byte) (int, error) {
	return 0, r.err
}

// getCaptureByIndex returns the capture group value by its index.
func (RePetDismissed) getCaptureByIndex(m *RePetDismissedBytesResult, index int) []byte {
	switch index {
	case 0:
		return m.Match
	default:
...
//	})
func (RePetDismissed) SelectReader(r io.Reader, pred func(*RePetDismissedBytesResult) bool) io.Reader {
	cfg := stream.DefaultTransformConfig()
	cfg.MaxLeftover = 1048576

	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return RePetDismissed{}.processSelect(data, isEOF, pred, emitOut)
	}
...
func (RePetDismissed) processSelect(data []byte, isEOF bool, pred func(*RePetDismissedBytesResult) bool, emitOut func([]byte)) int {
	processed := 0
	reuseResult := &RePetDismissedBytesResult{}
...
//	})
func (RePetDismissed) RejectReader(r io.Reader, pred func(*RePetDismissedBytesResult) bool) io.Reader {
	cfg := stream.DefaultTransformConfig()
	cfg.MaxLeftover = 1048576

	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return RePetDismissed{}.processReject(data, isEOF, pred, emitOut)
	}
...
func (RePetDismissed) processReject(data []byte, isEOF bool, pred func(*RePetDismissedBytesResult) bool, emitOut func([]byte)) int {
	processed := 0
	reuseResult := &RePetDismissedBytesResult{}
...
func (r *RePetDismissedResult) CaptureByIndex(idx int) string {
	switch idx {
	case 0:
		return r.Match
	case 1:
...
// CaptureByIndex returns the capture group value by its 0-based index.
// Index 0 returns the full match, 1+ returns capture groups.
func (r *RePetDismissedBytesResult) CaptureByIndex(idx int) []byte {
	switch idx {
	case 0:
		return r.Match
	case 1:
...
func (RePetDismissed) ReplaceAllString(input string, template string) string {
	...
	lastEnd := 0
	var r RePetDismissedResult

	remaining := input
	offset := 0

	for {
		match, ok := RePetDismissed{}.FindStringReuse(remaining, &r)
		if !ok {
			break
		}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (RePetDismissed) ReplaceAllBytes(input []byte, template string) []byte {
	return RePetDismissed{}.ReplaceAllBytesAppend(input, template, nil)
}

// ReplaceAllBytesAppend replaces all matches and appends to buf.
// If buf has sufficient capacity, no allocation occurs.
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (RePetDismissed) ReplaceAllBytesAppend(input []byte, template string, buf []byte) []byte {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	result := buf[:0]
	lastEnd := 0
	var r RePetDismissedBytesResult

	remaining := input
	offset := 0

	for {
		match, ok := RePetDismissed{}.FindBytesReuse(remaining, &r)
		if !ok {
			break
		}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (RePetDismissed) ReplaceFirstString(input string, template string) string {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	var r RePetDismissedResult
	match, ok := RePetDismissed{}.FindStringReuse(input, &r)
	if !ok {
		return input
	}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (RePetDismissed) ReplaceFirstBytes(input []byte, template string) []byte {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	var r RePetDismissedBytesResult
	match, ok := RePetDismissed{}.FindBytesReuse(input, &r)
	if !ok {
...
// RePetDismissedReplaceTemplate holds a pre-compiled replace template for the RePetDismissed pattern.
// Use CompileReplaceTemplate to create one, then call its Replace methods.
type RePetDismissedReplaceTemplate struct {
	original string
...
//
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (RePetDismissed) CompileReplaceTemplate(template string) (*RePetDismissedReplaceTemplate, error) {
	tmpl, err := replace.Parse(template)
	if err != nil {
		return nil, err
	}
	...
	return &RePetDismissedReplaceTemplate{
		original: template,
		segments: resolved,
	}, nil
}
...
func (t *RePetDismissedReplaceTemplate) String() string {
	return t.original
}

// ReplaceAllString replaces all matches in input using this compiled template.
func (t *RePetDismissedReplaceTemplate) ReplaceAllString(input string) string {
	var result strings.Builder
	lastEnd := 0
	var r RePetDismissedResult
...
func (t *RePetDismissedReplaceTemplate) ReplaceAllBytes(input []byte) []byte {
	return t.ReplaceAllBytesAppend(input, nil)
}

// ReplaceAllBytesAppend replaces all matches and appends to buf.
// If buf has sufficient capacity, no allocation occurs.
func (t *RePetDismissedReplaceTemplate) ReplaceAllBytesAppend(input []byte, buf []byte) []byte {
	result := buf[:0]
	lastEnd := 0
	var r RePetDismissedBytesResult
...
func (t *RePetDismissedReplaceTemplate) ReplaceFirstString(input string) string {
	var r RePetDismissedResult
	match, ok := RePetDismissed{}.FindStringReuse(input, &r)
	if !ok {
		return input
...
func (t *RePetDismissedReplaceTemplate) ReplaceFirstBytes(input []byte) []byte {
	var r RePetDismissedBytesResult
	match, ok := RePetDismissed{}.FindBytesReuse(input, &r)
	if !ok {
		return append([]byte{}, input...)
