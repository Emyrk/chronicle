...
// Code generated by regengo for pattern: (.+[^\s])'s (.+[^\s]) is removed\.
// DO NOT EDIT.

var reAuraDispelStackPool = sync.Pool{New: func() interface{} {
	stack := make([][3]int, 0, 32)
	return &stack
}}

var reAuraDispelCaptureStackPool = sync.Pool{New: func() interface{} {
	stack := make([]int, 0, 96)
	return &stack
}}

type ReAuraDispel struct{}

var CompiledReAuraDispel = ReAuraDispel{}

// MinMatchLen is the minimum number of bytes any match can have.
const ReAuraDispelMinMatchLen = 19

// MaxMatchLen is the maximum number of bytes any match can have.
// -1 means unbounded (pattern contains * or + quantifiers).
const ReAuraDispelMaxMatchLen = -1

func (ReAuraDispel) MatchLengthInfo() (minLen, maxLen int) {
	return ReAuraDispelMinMatchLen, ReAuraDispelMaxMatchLen
}

func (ReAuraDispel) MatchString(input string) bool {
	l := len(input)
	offset := 0
	stackPtr := reAuraDispelStackPool.Get().(*[][3]int)
	stack := (*stackPtr)[:0]
	defer func() {
		for i := range stack {
			stack[i] = [3]int{0, 0, 0}
		...
		*stackPtr = stack[:0]
		reAuraDispelStackPool.Put(stackPtr)
	}()
	nextInstruction := 1
	goto StepSelect
TryFallback:
...
func (ReAuraDispel) MatchBytes(input []byte) bool {
	l := len(input)
	offset := 0
	stackPtr := reAuraDispelStackPool.Get().(*[][3]int)
	stack := (*stackPtr)[:0]
...
type ReAuraDispelResult struct {
	Match  string // Full match
	Group1 string
	Group2 string
}

type ReAuraDispelBytesResult struct {
	Match  []byte // Full match
	Group1 []byte
	Group2 []byte
}

func (ReAuraDispel) FindStringReuse(input string, r *ReAuraDispelResult) (*ReAuraDispelResult, bool) {
	l := len(input)
	offset := 0
	var captures [6]int
	captureStackPtr := reAuraDispelCaptureStackPool.Get().(*[]int)
	captureStack := (*captureStackPtr)[:0]
	defer func() {
		*captureStackPtr = captureStack[:0]
		reAuraDispelCaptureStackPool.Put(captureStackPtr)
	}()
	stackPtr := reAuraDispelStackPool.Get().(*[][3]int)
	stack := (*stackPtr)[:0]
	defer func() {
...
func (recv ReAuraDispel) FindString(input string) (*ReAuraDispelResult, bool) {
	return recv.FindStringReuse(input, nil)
}
func (ReAuraDispel) FindAllStringAppend(input string, n int, s []*ReAuraDispelResult) []*ReAuraDispelResult {
	if n == 0 {
		return s
	}
	result := s
	...
	Ins26:
		{
			captures[1] = offset
			var item *ReAuraDispelResult
			if len(result) < cap(result) {
				result = result[:len(result)+1]
				item = result[len(result)-1]
				if item == nil {
					item = &ReAuraDispelResult{}
...
func (r ReAuraDispel) FindAllString(input string, n int) []*ReAuraDispelResult {
	return r.FindAllStringAppend(input, n, nil)
}
func (ReAuraDispel) FindBytesReuse(input []byte, r *ReAuraDispelBytesResult) (*ReAuraDispelBytesResult, bool) {
	l := len(input)
	offset := 0
	var captures [6]int
	captureStackPtr := reAuraDispelCaptureStackPool.Get().(*[]int)
...
func (recv ReAuraDispel) FindBytes(input []byte) (*ReAuraDispelBytesResult, bool) {
	return recv.FindBytesReuse(input, nil)
}
func (ReAuraDispel) FindAllBytesAppend(input []byte, n int, s []*ReAuraDispelBytesResult) []*ReAuraDispelBytesResult {
	if n == 0 {
		return s
	}
	result := s
	...
	Ins26:
		{
			captures[1] = offset
			var item *ReAuraDispelBytesResult
			if len(result) < cap(result) {
				result = result[:len(result)+1]
				item = result[len(result)-1]
				if item == nil {
					item = &ReAuraDispelBytesResult{}
					result[len(result)-1] = item
				}
			} else {
				item = &ReAuraDispelBytesResult{}
...
func (r ReAuraDispel) FindAllBytes(input []byte, n int) []*ReAuraDispelBytesResult {
...
func (ReAuraDispel) DefaultMaxLeftover() int {
	return 1048576
}
...
func (ReAuraDispel) FindReader(r io.Reader, cfg stream.Config, onMatch func(stream.Match[*ReAuraDispelBytesResult]) bool) error {
	...
	reuseResult := &ReAuraDispelBytesResult{}
			...
			if err == io.EOF {
				...
				if leftover > 0 {
					...
					for searchPos < len(chunk) {
						result, ok := ReAuraDispel{}.FindBytesReuse(chunk[searchPos:], reuseResult)
						if !ok {
							break
						}
						...
						m := stream.Match[*ReAuraDispelBytesResult]{
...
func (ReAuraDispel) FindReaderCount(r io.Reader, cfg stream.Config) (int64, error) {
	var count int64
	err := ReAuraDispel{}.FindReader(r, cfg, func(_ stream.Match[*ReAuraDispelBytesResult]) bool {
		count++
		return true
	})
	return count, err
...
func (ReAuraDispel) FindReaderFirst(r io.Reader, cfg stream.Config) (*ReAuraDispelBytesResult, int64, error) {
	var result *ReAuraDispelBytesResult
	var offset int64
	err := ReAuraDispel{}.FindReader(r, cfg, func(m stream.Match[*ReAuraDispelBytesResult]) bool {
		// Copy the result since buffer will be reused
		result = ReAuraDispel{}.copyBytesResult(m.Result)
...
func (ReAuraDispel) copyBytesResult(src *ReAuraDispelBytesResult) *ReAuraDispelBytesResult {
	if src == nil {
		return nil
	}
	dst := &ReAuraDispelBytesResult{}
	// Copy Match slice
...
//	    })
//	io.Copy(os.Stdout, r)
func (ReAuraDispel) NewTransformReader(r io.Reader, cfg stream.TransformConfig, onMatch func(match *ReAuraDispelBytesResult, emit func([]byte))) io.Reader {
	if cfg.BufferSize == 0 {
	...
	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReAuraDispel{}.processTransform(data, isEOF, onMatch, emitOut)
	}
...
func (ReAuraDispel) processTransform(data []byte, isEOF bool, onMatch func(match *ReAuraDispelBytesResult, emit func([]byte)), emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReAuraDispelBytesResult{}

	for {
		result, ok := ReAuraDispel{}.FindBytesReuse(data[processed:], reuseResult)
		if !ok {
			// No more matches
			if isEOF {
...
//	io.Copy(os.Stdout, r)
func (ReAuraDispel) ReplaceReader(r io.Reader, template string) io.Reader {
	parsed, err := replace.Parse(template)
	if err != nil {
		return &reAuraDispelTransformErrReader{err: err}
	}
	...
	return ReAuraDispel{}.NewTransformReader(r, stream.DefaultTransformConfig(), func(m *ReAuraDispelBytesResult, emit func([]byte)) {
		...
		for _, seg := range resolved {
			...
			case replace.SegmentCaptureIndex:
				capture := (ReAuraDispel{}).getCaptureByIndex(m, seg.CaptureIndex)
				if capture != nil {
...
// reAuraDispelTransformErrReader is a reader that always returns an error.
type reAuraDispelTransformErrReader struct {
	err error
}

func (r *reAuraDispelTransformErrReader) Read(p []byte) (int, error) {
	return 0, r.err
}

// getCaptureByIndex returns the capture group value by its index.
func (ReAuraDispel) getCaptureByIndex(m *ReAuraDispelBytesResult, index int) []byte {
	switch index {
	case 0:
		return m.Match
	default:
...
//	})
func (ReAuraDispel) SelectReader(r io.Reader, pred func(*ReAuraDispelBytesResult) bool) io.Reader {
	cfg := stream.DefaultTransformConfig()
	cfg.MaxLeftover = 1048576

	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReAuraDispel{}.processSelect(data, isEOF, pred, emitOut)
	}
...
func (ReAuraDispel) processSelect(data []byte, isEOF bool, pred func(*ReAuraDispelBytesResult) bool, emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReAuraDispelBytesResult{}
...
//	})
func (ReAuraDispel) RejectReader(r io.Reader, pred func(*ReAuraDispelBytesResult) bool) io.Reader {
	cfg := stream.DefaultTransformConfig()
	cfg.MaxLeftover = 1048576

	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReAuraDispel{}.processReject(data, isEOF, pred, emitOut)
	}
...
func (ReAuraDispel) processReject(data []byte, isEOF bool, pred func(*ReAuraDispelBytesResult) bool, emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReAuraDispelBytesResult{}
...
func (r *ReAuraDispelResult) CaptureByIndex(idx int) string {
	switch idx {
	case 0:
		return r.Match
	case 1:
...
// CaptureByIndex returns the capture group value by its 0-based index.
// Index 0 returns the full match, 1+ returns capture groups.
func (r *ReAuraDispelBytesResult) CaptureByIndex(idx int) []byte {
	switch idx {
	case 0:
		return r.Match
	case 1:
...
func (ReAuraDispel) ReplaceAllString(input string, template string) string {
	...
	lastEnd := 0
	var r ReAuraDispelResult

	remaining := input
	offset := 0

	for {
		match, ok := ReAuraDispel{}.FindStringReuse(remaining, &r)
		if !ok {
			break
		}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReAuraDispel) ReplaceAllBytes(input []byte, template string) []byte {
	return ReAuraDispel{}.ReplaceAllBytesAppend(input, template, nil)
}

// ReplaceAllBytesAppend replaces all matches and appends to buf.
// If buf has sufficient capacity, no allocation occurs.
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReAuraDispel) ReplaceAllBytesAppend(input []byte, template string, buf []byte) []byte {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	result := buf[:0]
	lastEnd := 0
	var r ReAuraDispelBytesResult

	remaining := input
	offset := 0

	for {
		match, ok := ReAuraDispel{}.FindBytesReuse(remaining, &r)
		if !ok {
			break
		}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReAuraDispel) ReplaceFirstString(input string, template string) string {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	var r ReAuraDispelResult
	match, ok := ReAuraDispel{}.FindStringReuse(input, &r)
	if !ok {
		return input
	}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReAuraDispel) ReplaceFirstBytes(input []byte, template string) []byte {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	var r ReAuraDispelBytesResult
	match, ok := ReAuraDispel{}.FindBytesReuse(input, &r)
	if !ok {
...
// ReAuraDispelReplaceTemplate holds a pre-compiled replace template for the ReAuraDispel pattern.
// Use CompileReplaceTemplate to create one, then call its Replace methods.
type ReAuraDispelReplaceTemplate struct {
	original string
...
//
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReAuraDispel) CompileReplaceTemplate(template string) (*ReAuraDispelReplaceTemplate, error) {
	tmpl, err := replace.Parse(template)
	if err != nil {
		return nil, err
	}
	...
	return &ReAuraDispelReplaceTemplate{
		original: template,
		segments: resolved,
	}, nil
}
...
func (t *ReAuraDispelReplaceTemplate) String() string {
	return t.original
}

// ReplaceAllString replaces all matches in input using this compiled template.
func (t *ReAuraDispelReplaceTemplate) ReplaceAllString(input string) string {
	var result strings.Builder
	lastEnd := 0
	var r ReAuraDispelResult
...
func (t *ReAuraDispelReplaceTemplate) ReplaceAllBytes(input []byte) []byte {
	return t.ReplaceAllBytesAppend(input, nil)
}

// ReplaceAllBytesAppend replaces all matches and appends to buf.
// If buf has sufficient capacity, no allocation occurs.
func (t *ReAuraDispelReplaceTemplate) ReplaceAllBytesAppend(input []byte, buf []byte) []byte {
	result := buf[:0]
	lastEnd := 0
	var r ReAuraDispelBytesResult
...
func (t *ReAuraDispelReplaceTemplate) ReplaceFirstString(input string) string {
	var r ReAuraDispelResult
	match, ok := ReAuraDispel{}.FindStringReuse(input, &r)
	if !ok {
		return input
...
func (t *ReAuraDispelReplaceTemplate) ReplaceFirstBytes(input []byte) []byte {
	var r ReAuraDispelBytesResult
	match, ok := ReAuraDispel{}.FindBytesReuse(input, &r)
	if !ok {
		return append([]byte{}, input...)
