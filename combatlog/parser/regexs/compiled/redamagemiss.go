...
// Code generated by regengo for pattern: (.+[^\s]) misses (.+[^\s])\.
// DO NOT EDIT.

var reDamageMissStackPool = sync.Pool{New: func() interface{} {
	stack := make([][3]int, 0, 32)
	return &stack
}}

var reDamageMissCaptureStackPool = sync.Pool{New: func() interface{} {
	stack := make([]int, 0, 96)
	return &stack
}}

type ReDamageMiss struct{}

var CompiledReDamageMiss = ReDamageMiss{}

// MinMatchLen is the minimum number of bytes any match can have.
const ReDamageMissMinMatchLen = 13

// MaxMatchLen is the maximum number of bytes any match can have.
// -1 means unbounded (pattern contains * or + quantifiers).
const ReDamageMissMaxMatchLen = -1

func (ReDamageMiss) MatchLengthInfo() (minLen, maxLen int) {
	return ReDamageMissMinMatchLen, ReDamageMissMaxMatchLen
}

func (ReDamageMiss) MatchString(input string) bool {
	l := len(input)
	offset := 0
	stackPtr := reDamageMissStackPool.Get().(*[][3]int)
	stack := (*stackPtr)[:0]
	defer func() {
		for i := range stack {
			stack[i] = [3]int{0, 0, 0}
		...
		*stackPtr = stack[:0]
		reDamageMissStackPool.Put(stackPtr)
	}()
	nextInstruction := 1
	goto StepSelect
TryFallback:
...
func (ReDamageMiss) MatchBytes(input []byte) bool {
	l := len(input)
	offset := 0
	stackPtr := reDamageMissStackPool.Get().(*[][3]int)
	stack := (*stackPtr)[:0]
...
type ReDamageMissResult struct {
	Match  string // Full match
	Group1 string
	Group2 string
}

type ReDamageMissBytesResult struct {
	Match  []byte // Full match
	Group1 []byte
	Group2 []byte
}

func (ReDamageMiss) FindStringReuse(input string, r *ReDamageMissResult) (*ReDamageMissResult, bool) {
	l := len(input)
	offset := 0
	var captures [6]int
	captureStackPtr := reDamageMissCaptureStackPool.Get().(*[]int)
	captureStack := (*captureStackPtr)[:0]
	defer func() {
		*captureStackPtr = captureStack[:0]
		reDamageMissCaptureStackPool.Put(captureStackPtr)
	}()
	stackPtr := reDamageMissStackPool.Get().(*[][3]int)
	stack := (*stackPtr)[:0]
	defer func() {
...
Ins16:
	{
		captureStack = append(captureStack, captures[:]...)
		stack = append(stack, [3]int{offset, 17, 1})
		goto Ins15
	}
Ins17:
	{
...
func (recv ReDamageMiss) FindString(input string) (*ReDamageMissResult, bool) {
	return recv.FindStringReuse(input, nil)
}
func (ReDamageMiss) FindAllStringAppend(input string, n int, s []*ReDamageMissResult) []*ReDamageMissResult {
	if n == 0 {
		return s
	}
	result := s
	...
	Ins20:
		{
			captures[1] = offset
			var item *ReDamageMissResult
			if len(result) < cap(result) {
				result = result[:len(result)+1]
				item = result[len(result)-1]
				if item == nil {
					item = &ReDamageMissResult{}
...
func (r ReDamageMiss) FindAllString(input string, n int) []*ReDamageMissResult {
	return r.FindAllStringAppend(input, n, nil)
}
func (ReDamageMiss) FindBytesReuse(input []byte, r *ReDamageMissBytesResult) (*ReDamageMissBytesResult, bool) {
	l := len(input)
	offset := 0
	var captures [6]int
	captureStackPtr := reDamageMissCaptureStackPool.Get().(*[]int)
...
func (recv ReDamageMiss) FindBytes(input []byte) (*ReDamageMissBytesResult, bool) {
	return recv.FindBytesReuse(input, nil)
}
func (ReDamageMiss) FindAllBytesAppend(input []byte, n int, s []*ReDamageMissBytesResult) []*ReDamageMissBytesResult {
	if n == 0 {
		return s
	}
	result := s
	...
	Ins20:
		{
			captures[1] = offset
			var item *ReDamageMissBytesResult
			if len(result) < cap(result) {
				result = result[:len(result)+1]
				item = result[len(result)-1]
				if item == nil {
					item = &ReDamageMissBytesResult{}
					result[len(result)-1] = item
				}
			} else {
				item = &ReDamageMissBytesResult{}
...
func (r ReDamageMiss) FindAllBytes(input []byte, n int) []*ReDamageMissBytesResult {
...
func (ReDamageMiss) DefaultMaxLeftover() int {
	return 1048576
}
...
func (ReDamageMiss) FindReader(r io.Reader, cfg stream.Config, onMatch func(stream.Match[*ReDamageMissBytesResult]) bool) error {
	...
	reuseResult := &ReDamageMissBytesResult{}
			...
			if err == io.EOF {
				...
				if leftover > 0 {
					...
					for searchPos < len(chunk) {
						result, ok := ReDamageMiss{}.FindBytesReuse(chunk[searchPos:], reuseResult)
						if !ok {
							break
						}
						...
						m := stream.Match[*ReDamageMissBytesResult]{
...
func (ReDamageMiss) FindReaderCount(r io.Reader, cfg stream.Config) (int64, error) {
	var count int64
	err := ReDamageMiss{}.FindReader(r, cfg, func(_ stream.Match[*ReDamageMissBytesResult]) bool {
		count++
		return true
	})
	return count, err
...
func (ReDamageMiss) FindReaderFirst(r io.Reader, cfg stream.Config) (*ReDamageMissBytesResult, int64, error) {
	var result *ReDamageMissBytesResult
	var offset int64
	err := ReDamageMiss{}.FindReader(r, cfg, func(m stream.Match[*ReDamageMissBytesResult]) bool {
		// Copy the result since buffer will be reused
		result = ReDamageMiss{}.copyBytesResult(m.Result)
...
func (ReDamageMiss) copyBytesResult(src *ReDamageMissBytesResult) *ReDamageMissBytesResult {
	if src == nil {
		return nil
	}
	dst := &ReDamageMissBytesResult{}
	// Copy Match slice
...
//	    })
//	io.Copy(os.Stdout, r)
func (ReDamageMiss) NewTransformReader(r io.Reader, cfg stream.TransformConfig, onMatch func(match *ReDamageMissBytesResult, emit func([]byte))) io.Reader {
	if cfg.BufferSize == 0 {
	...
	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReDamageMiss{}.processTransform(data, isEOF, onMatch, emitOut)
	}
...
func (ReDamageMiss) processTransform(data []byte, isEOF bool, onMatch func(match *ReDamageMissBytesResult, emit func([]byte)), emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReDamageMissBytesResult{}

	for {
		result, ok := ReDamageMiss{}.FindBytesReuse(data[processed:], reuseResult)
		if !ok {
			// No more matches
			if isEOF {
...
//	io.Copy(os.Stdout, r)
func (ReDamageMiss) ReplaceReader(r io.Reader, template string) io.Reader {
	parsed, err := replace.Parse(template)
	if err != nil {
		return &reDamageMissTransformErrReader{err: err}
	}
	...
	return ReDamageMiss{}.NewTransformReader(r, stream.DefaultTransformConfig(), func(m *ReDamageMissBytesResult, emit func([]byte)) {
		...
		for _, seg := range resolved {
			...
			case replace.SegmentCaptureIndex:
				capture := (ReDamageMiss{}).getCaptureByIndex(m, seg.CaptureIndex)
				if capture != nil {
...
// reDamageMissTransformErrReader is a reader that always returns an error.
type reDamageMissTransformErrReader struct {
	err error
}

func (r *reDamageMissTransformErrReader) Read(p []byte) (int, error) {
	return 0, r.err
}

// getCaptureByIndex returns the capture group value by its index.
func (ReDamageMiss) getCaptureByIndex(m *ReDamageMissBytesResult, index int) []byte {
	switch index {
	case 0:
		return m.Match
	default:
...
//	})
func (ReDamageMiss) SelectReader(r io.Reader, pred func(*ReDamageMissBytesResult) bool) io.Reader {
	cfg := stream.DefaultTransformConfig()
	cfg.MaxLeftover = 1048576

	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReDamageMiss{}.processSelect(data, isEOF, pred, emitOut)
	}
...
func (ReDamageMiss) processSelect(data []byte, isEOF bool, pred func(*ReDamageMissBytesResult) bool, emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReDamageMissBytesResult{}
...
//	})
func (ReDamageMiss) RejectReader(r io.Reader, pred func(*ReDamageMissBytesResult) bool) io.Reader {
	cfg := stream.DefaultTransformConfig()
	cfg.MaxLeftover = 1048576

	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReDamageMiss{}.processReject(data, isEOF, pred, emitOut)
	}
...
func (ReDamageMiss) processReject(data []byte, isEOF bool, pred func(*ReDamageMissBytesResult) bool, emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReDamageMissBytesResult{}
...
func (r *ReDamageMissResult) CaptureByIndex(idx int) string {
	switch idx {
	case 0:
		return r.Match
	case 1:
...
// CaptureByIndex returns the capture group value by its 0-based index.
// Index 0 returns the full match, 1+ returns capture groups.
func (r *ReDamageMissBytesResult) CaptureByIndex(idx int) []byte {
	switch idx {
	case 0:
		return r.Match
	case 1:
...
func (ReDamageMiss) ReplaceAllString(input string, template string) string {
	...
	lastEnd := 0
	var r ReDamageMissResult

	remaining := input
	offset := 0

	for {
		match, ok := ReDamageMiss{}.FindStringReuse(remaining, &r)
		if !ok {
			break
		}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReDamageMiss) ReplaceAllBytes(input []byte, template string) []byte {
	return ReDamageMiss{}.ReplaceAllBytesAppend(input, template, nil)
}

// ReplaceAllBytesAppend replaces all matches and appends to buf.
// If buf has sufficient capacity, no allocation occurs.
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReDamageMiss) ReplaceAllBytesAppend(input []byte, template string, buf []byte) []byte {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	result := buf[:0]
	lastEnd := 0
	var r ReDamageMissBytesResult

	remaining := input
	offset := 0

	for {
		match, ok := ReDamageMiss{}.FindBytesReuse(remaining, &r)
		if !ok {
			break
		}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReDamageMiss) ReplaceFirstString(input string, template string) string {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	var r ReDamageMissResult
	match, ok := ReDamageMiss{}.FindStringReuse(input, &r)
	if !ok {
		return input
	}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReDamageMiss) ReplaceFirstBytes(input []byte, template string) []byte {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	var r ReDamageMissBytesResult
	match, ok := ReDamageMiss{}.FindBytesReuse(input, &r)
	if !ok {
...
// ReDamageMissReplaceTemplate holds a pre-compiled replace template for the ReDamageMiss pattern.
// Use CompileReplaceTemplate to create one, then call its Replace methods.
type ReDamageMissReplaceTemplate struct {
	original string
...
//
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReDamageMiss) CompileReplaceTemplate(template string) (*ReDamageMissReplaceTemplate, error) {
	tmpl, err := replace.Parse(template)
	if err != nil {
		return nil, err
	}
	...
	return &ReDamageMissReplaceTemplate{
		original: template,
		segments: resolved,
	}, nil
}
...
func (t *ReDamageMissReplaceTemplate) String() string {
	return t.original
}

// ReplaceAllString replaces all matches in input using this compiled template.
func (t *ReDamageMissReplaceTemplate) ReplaceAllString(input string) string {
	var result strings.Builder
	lastEnd := 0
	var r ReDamageMissResult
...
func (t *ReDamageMissReplaceTemplate) ReplaceAllBytes(input []byte) []byte {
	return t.ReplaceAllBytesAppend(input, nil)
}

// ReplaceAllBytesAppend replaces all matches and appends to buf.
// If buf has sufficient capacity, no allocation occurs.
func (t *ReDamageMissReplaceTemplate) ReplaceAllBytesAppend(input []byte, buf []byte) []byte {
	result := buf[:0]
	lastEnd := 0
	var r ReDamageMissBytesResult
...
func (t *ReDamageMissReplaceTemplate) ReplaceFirstString(input string) string {
	var r ReDamageMissResult
	match, ok := ReDamageMiss{}.FindStringReuse(input, &r)
	if !ok {
		return input
...
func (t *ReDamageMissReplaceTemplate) ReplaceFirstBytes(input []byte) []byte {
	var r ReDamageMissBytesResult
	match, ok := ReDamageMiss{}.FindBytesReuse(input, &r)
	if !ok {
		return append([]byte{}, input...)
