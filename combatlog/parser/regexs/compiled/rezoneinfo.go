...
// Code generated by regengo for pattern: ZONE_INFO: ([^&]+)&(.+[^\s])\&(\d+)
// DO NOT EDIT.

var reZoneInfoStackPool = sync.Pool{New: func() interface{} {
	stack := make([][3]int, 0, 32)
	return &stack
}}

var reZoneInfoCaptureStackPool = sync.Pool{New: func() interface{} {
	stack := make([]int, 0, 128)
	return &stack
}}

type ReZoneInfo struct{}

var CompiledReZoneInfo = ReZoneInfo{}

// MinMatchLen is the minimum number of bytes any match can have.
const ReZoneInfoMinMatchLen = 17

// MaxMatchLen is the maximum number of bytes any match can have.
// -1 means unbounded (pattern contains * or + quantifiers).
const ReZoneInfoMaxMatchLen = -1

func (ReZoneInfo) MatchLengthInfo() (minLen, maxLen int) {
	return ReZoneInfoMinMatchLen, ReZoneInfoMaxMatchLen
}

func (ReZoneInfo) MatchString(input string) bool {
	l := len(input)
	idx := strings.IndexByte(input, uint8(0x5a))
	if idx == -1 {
		return false
	}
	offset := idx
	stackPtr := reZoneInfoStackPool.Get().(*[][3]int)
	stack := (*stackPtr)[:0]
	defer func() {
		for i := range stack {
			stack[i] = [3]int{0, 0, 0}
		...
		*stackPtr = stack[:0]
		reZoneInfoStackPool.Put(stackPtr)
	}()
	nextInstruction := 1
	goto StepSelect
TryFallback:
	if len(stack) > 0 {
		...
		offset++
		if l > offset {
			idx := strings.IndexByte(input[offset:], uint8(0x5a))
			if idx == -1 {
				return false
			}
			offset += idx
...
func (ReZoneInfo) MatchBytes(input []byte) bool {
	l := len(input)
	idx := bytes.IndexByte(input, uint8(0x5a))
	if idx == -1 {
		return false
	}
	offset := idx
...
TryFallback:
	if len(stack) > 0 {
		...
		offset++
		if l > offset {
			idx := bytes.IndexByte(input[offset:], uint8(0x5a))
			if idx == -1 {
				return false
			}
			offset += idx
...
type ReZoneInfoBytesResult struct {
...
func (ReZoneInfo) FindStringReuse(input string, r *ReZoneInfoResult) (*ReZoneInfoResult, bool) {
	l := len(input)
	offset := 0
	var captures [8]int
	captureStackPtr := reZoneInfoCaptureStackPool.Get().(*[]int)
	captureStack := (*captureStackPtr)[:0]
	defer func() {
		*captureStackPtr = captureStack[:0]
		reZoneInfoCaptureStackPool.Put(captureStackPtr)
	}()
	stackPtr := reZoneInfoStackPool.Get().(*[][3]int)
	stack := (*stackPtr)[:0]
	defer func() {
...
Ins19:
	{
		captureStack = append(captureStack, captures[:]...)
		stack = append(stack, [3]int{offset, 20, 1})
		goto Ins18
	}
Ins20:
	{
...
Ins25:
	{
		captureStack = append(captureStack, captures[:]...)
		stack = append(stack, [3]int{offset, 26, 1})
		goto Ins24
	}
Ins26:
	{
...
func (recv ReZoneInfo) FindString(input string) (*ReZoneInfoResult, bool) {
	return recv.FindStringReuse(input, nil)
}
func (ReZoneInfo) FindAllStringAppend(input string, n int, s []*ReZoneInfoResult) []*ReZoneInfoResult {
	if n == 0 {
		return s
	}
	result := s
	...
	Ins27:
		{
			captures[1] = offset
			var item *ReZoneInfoResult
			if len(result) < cap(result) {
				result = result[:len(result)+1]
				item = result[len(result)-1]
				if item == nil {
					item = &ReZoneInfoResult{}
...
func (r ReZoneInfo) FindAllString(input string, n int) []*ReZoneInfoResult {
	return r.FindAllStringAppend(input, n, nil)
}
func (ReZoneInfo) FindBytesReuse(input []byte, r *ReZoneInfoBytesResult) (*ReZoneInfoBytesResult, bool) {
	l := len(input)
	offset := 0
	var captures [8]int
	captureStackPtr := reZoneInfoCaptureStackPool.Get().(*[]int)
...
func (recv ReZoneInfo) FindBytes(input []byte) (*ReZoneInfoBytesResult, bool) {
	return recv.FindBytesReuse(input, nil)
}
func (ReZoneInfo) FindAllBytesAppend(input []byte, n int, s []*ReZoneInfoBytesResult) []*ReZoneInfoBytesResult {
	if n == 0 {
		return s
	}
	result := s
	...
	Ins27:
		{
			captures[1] = offset
			var item *ReZoneInfoBytesResult
			if len(result) < cap(result) {
				result = result[:len(result)+1]
				item = result[len(result)-1]
				if item == nil {
					item = &ReZoneInfoBytesResult{}
					result[len(result)-1] = item
				}
			} else {
				item = &ReZoneInfoBytesResult{}
...
func (r ReZoneInfo) FindAllBytes(input []byte, n int) []*ReZoneInfoBytesResult {
...
func (ReZoneInfo) DefaultMaxLeftover() int {
	return 1048576
}
...
func (ReZoneInfo) FindReader(r io.Reader, cfg stream.Config, onMatch func(stream.Match[*ReZoneInfoBytesResult]) bool) error {
	...
	reuseResult := &ReZoneInfoBytesResult{}
			...
			if err == io.EOF {
				...
				if leftover > 0 {
					...
					for searchPos < len(chunk) {
						result, ok := ReZoneInfo{}.FindBytesReuse(chunk[searchPos:], reuseResult)
						if !ok {
							break
						}
						...
						m := stream.Match[*ReZoneInfoBytesResult]{
...
func (ReZoneInfo) FindReaderCount(r io.Reader, cfg stream.Config) (int64, error) {
	var count int64
	err := ReZoneInfo{}.FindReader(r, cfg, func(_ stream.Match[*ReZoneInfoBytesResult]) bool {
		count++
		return true
	})
	return count, err
...
func (ReZoneInfo) FindReaderFirst(r io.Reader, cfg stream.Config) (*ReZoneInfoBytesResult, int64, error) {
	var result *ReZoneInfoBytesResult
	var offset int64
	err := ReZoneInfo{}.FindReader(r, cfg, func(m stream.Match[*ReZoneInfoBytesResult]) bool {
		// Copy the result since buffer will be reused
		result = ReZoneInfo{}.copyBytesResult(m.Result)
...
func (ReZoneInfo) copyBytesResult(src *ReZoneInfoBytesResult) *ReZoneInfoBytesResult {
	if src == nil {
		return nil
	}
	dst := &ReZoneInfoBytesResult{}
	// Copy Match slice
...
//	    })
//	io.Copy(os.Stdout, r)
func (ReZoneInfo) NewTransformReader(r io.Reader, cfg stream.TransformConfig, onMatch func(match *ReZoneInfoBytesResult, emit func([]byte))) io.Reader {
	if cfg.BufferSize == 0 {
	...
	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReZoneInfo{}.processTransform(data, isEOF, onMatch, emitOut)
	}
...
func (ReZoneInfo) processTransform(data []byte, isEOF bool, onMatch func(match *ReZoneInfoBytesResult, emit func([]byte)), emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReZoneInfoBytesResult{}

	for {
		result, ok := ReZoneInfo{}.FindBytesReuse(data[processed:], reuseResult)
		if !ok {
			// No more matches
			if isEOF {
...
//	io.Copy(os.Stdout, r)
func (ReZoneInfo) ReplaceReader(r io.Reader, template string) io.Reader {
	parsed, err := replace.Parse(template)
	if err != nil {
		return &reZoneInfoTransformErrReader{err: err}
	}
	...
	return ReZoneInfo{}.NewTransformReader(r, stream.DefaultTransformConfig(), func(m *ReZoneInfoBytesResult, emit func([]byte)) {
		...
		for _, seg := range resolved {
			...
			case replace.SegmentCaptureIndex:
				capture := (ReZoneInfo{}).getCaptureByIndex(m, seg.CaptureIndex)
				if capture != nil {
...
// reZoneInfoTransformErrReader is a reader that always returns an error.
type reZoneInfoTransformErrReader struct {
	err error
}

func (r *reZoneInfoTransformErrReader) Read(p []byte) (int, error) {
	return 0, r.err
}

// getCaptureByIndex returns the capture group value by its index.
func (ReZoneInfo) getCaptureByIndex(m *ReZoneInfoBytesResult, index int) []byte {
	switch index {
	case 0:
		return m.Match
	default:
...
//	})
func (ReZoneInfo) SelectReader(r io.Reader, pred func(*ReZoneInfoBytesResult) bool) io.Reader {
	cfg := stream.DefaultTransformConfig()
	cfg.MaxLeftover = 1048576

	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReZoneInfo{}.processSelect(data, isEOF, pred, emitOut)
	}
...
func (ReZoneInfo) processSelect(data []byte, isEOF bool, pred func(*ReZoneInfoBytesResult) bool, emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReZoneInfoBytesResult{}
...
//	})
func (ReZoneInfo) RejectReader(r io.Reader, pred func(*ReZoneInfoBytesResult) bool) io.Reader {
	cfg := stream.DefaultTransformConfig()
	cfg.MaxLeftover = 1048576

	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReZoneInfo{}.processReject(data, isEOF, pred, emitOut)
	}
...
func (ReZoneInfo) processReject(data []byte, isEOF bool, pred func(*ReZoneInfoBytesResult) bool, emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReZoneInfoBytesResult{}
...
func (r *ReZoneInfoResult) CaptureByIndex(idx int) string {
	switch idx {
	case 0:
		return r.Match
	case 1:
...
// CaptureByIndex returns the capture group value by its 0-based index.
// Index 0 returns the full match, 1+ returns capture groups.
func (r *ReZoneInfoBytesResult) CaptureByIndex(idx int) []byte {
	switch idx {
	case 0:
		return r.Match
	case 1:
...
func (ReZoneInfo) ReplaceAllString(input string, template string) string {
	...
	lastEnd := 0
	var r ReZoneInfoResult

	remaining := input
	offset := 0

	for {
		match, ok := ReZoneInfo{}.FindStringReuse(remaining, &r)
		if !ok {
			break
		}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReZoneInfo) ReplaceAllBytes(input []byte, template string) []byte {
	return ReZoneInfo{}.ReplaceAllBytesAppend(input, template, nil)
}

// ReplaceAllBytesAppend replaces all matches and appends to buf.
// If buf has sufficient capacity, no allocation occurs.
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReZoneInfo) ReplaceAllBytesAppend(input []byte, template string, buf []byte) []byte {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	result := buf[:0]
	lastEnd := 0
	var r ReZoneInfoBytesResult

	remaining := input
	offset := 0

	for {
		match, ok := ReZoneInfo{}.FindBytesReuse(remaining, &r)
		if !ok {
			break
		}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReZoneInfo) ReplaceFirstString(input string, template string) string {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	var r ReZoneInfoResult
	match, ok := ReZoneInfo{}.FindStringReuse(input, &r)
	if !ok {
		return input
	}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReZoneInfo) ReplaceFirstBytes(input []byte, template string) []byte {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	var r ReZoneInfoBytesResult
	match, ok := ReZoneInfo{}.FindBytesReuse(input, &r)
	if !ok {
...
// ReZoneInfoReplaceTemplate holds a pre-compiled replace template for the ReZoneInfo pattern.
// Use CompileReplaceTemplate to create one, then call its Replace methods.
type ReZoneInfoReplaceTemplate struct {
	original string
...
//
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReZoneInfo) CompileReplaceTemplate(template string) (*ReZoneInfoReplaceTemplate, error) {
	tmpl, err := replace.Parse(template)
	if err != nil {
		return nil, err
	}
	...
	return &ReZoneInfoReplaceTemplate{
		original: template,
		segments: resolved,
	}, nil
}
...
func (t *ReZoneInfoReplaceTemplate) String() string {
	return t.original
}

// ReplaceAllString replaces all matches in input using this compiled template.
func (t *ReZoneInfoReplaceTemplate) ReplaceAllString(input string) string {
	var result strings.Builder
	lastEnd := 0
	var r ReZoneInfoResult
...
func (t *ReZoneInfoReplaceTemplate) ReplaceAllBytes(input []byte) []byte {
	return t.ReplaceAllBytesAppend(input, nil)
}

// ReplaceAllBytesAppend replaces all matches and appends to buf.
// If buf has sufficient capacity, no allocation occurs.
func (t *ReZoneInfoReplaceTemplate) ReplaceAllBytesAppend(input []byte, buf []byte) []byte {
	result := buf[:0]
	lastEnd := 0
	var r ReZoneInfoBytesResult
...
func (t *ReZoneInfoReplaceTemplate) ReplaceFirstString(input string) string {
	var r ReZoneInfoResult
	match, ok := ReZoneInfo{}.FindStringReuse(input, &r)
	if !ok {
		return input
...
func (t *ReZoneInfoReplaceTemplate) ReplaceFirstBytes(input []byte) []byte {
	var r ReZoneInfoBytesResult
	match, ok := ReZoneInfo{}.FindBytesReuse(input, &r)
	if !ok {
		return append([]byte{}, input...)
