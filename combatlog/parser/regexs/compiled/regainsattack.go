...
// Code generated by regengo for pattern: (.+[^\s]) gains (\d+) extra attacks? through (.+[^\s])\.
// DO NOT EDIT.

var reGainsAttackStackPool = sync.Pool{New: func() interface{} {
	stack := make([][3]int, 0, 32)
	return &stack
}}

var reGainsAttackCaptureStackPool = sync.Pool{New: func() interface{} {
	stack := make([]int, 0, 128)
	return &stack
}}

type ReGainsAttack struct{}

var CompiledReGainsAttack = ReGainsAttack{}

// MinMatchLen is the minimum number of bytes any match can have.
const ReGainsAttackMinMatchLen = 35

// MaxMatchLen is the maximum number of bytes any match can have.
// -1 means unbounded (pattern contains * or + quantifiers).
const ReGainsAttackMaxMatchLen = -1

func (ReGainsAttack) MatchLengthInfo() (minLen, maxLen int) {
	return ReGainsAttackMinMatchLen, ReGainsAttackMaxMatchLen
}

func (ReGainsAttack) MatchString(input string) bool {
	l := len(input)
	offset := 0
	stackPtr := reGainsAttackStackPool.Get().(*[][3]int)
	stack := (*stackPtr)[:0]
	defer func() {
		for i := range stack {
			stack[i] = [3]int{0, 0, 0}
		...
		*stackPtr = stack[:0]
		reGainsAttackStackPool.Put(stackPtr)
	}()
	nextInstruction := 1
	goto StepSelect
TryFallback:
...
func (ReGainsAttack) MatchBytes(input []byte) bool {
	l := len(input)
	offset := 0
	stackPtr := reGainsAttackStackPool.Get().(*[][3]int)
	stack := (*stackPtr)[:0]
...
type ReGainsAttackResult struct {
...
type ReGainsAttackBytesResult struct {
...
func (ReGainsAttack) FindStringReuse(input string, r *ReGainsAttackResult) (*ReGainsAttackResult, bool) {
	l := len(input)
	offset := 0
	var captures [8]int
	stackPtr := reGainsAttackStackPool.Get().(*[][3]int)
...
func (recv ReGainsAttack) FindString(input string) (*ReGainsAttackResult, bool) {
	return recv.FindStringReuse(input, nil)
}
func (ReGainsAttack) FindAllStringAppend(input string, n int, s []*ReGainsAttackResult) []*ReGainsAttackResult {
	if n == 0 {
		return s
	}
	result := s
	...
	Ins47:
		{
			captures[1] = offset
			var item *ReGainsAttackResult
			if len(result) < cap(result) {
				result = result[:len(result)+1]
				item = result[len(result)-1]
				if item == nil {
					item = &ReGainsAttackResult{}
...
func (r ReGainsAttack) FindAllString(input string, n int) []*ReGainsAttackResult {
	return r.FindAllStringAppend(input, n, nil)
}
func (ReGainsAttack) FindBytesReuse(input []byte, r *ReGainsAttackBytesResult) (*ReGainsAttackBytesResult, bool) {
	l := len(input)
	offset := 0
	var captures [8]int
	stackPtr := reGainsAttackStackPool.Get().(*[][3]int)
...
func (recv ReGainsAttack) FindBytes(input []byte) (*ReGainsAttackBytesResult, bool) {
	return recv.FindBytesReuse(input, nil)
}
func (ReGainsAttack) FindAllBytesAppend(input []byte, n int, s []*ReGainsAttackBytesResult) []*ReGainsAttackBytesResult {
	if n == 0 {
		return s
	}
	result := s
	...
	Ins47:
		{
			captures[1] = offset
			var item *ReGainsAttackBytesResult
			if len(result) < cap(result) {
				result = result[:len(result)+1]
				item = result[len(result)-1]
				if item == nil {
					item = &ReGainsAttackBytesResult{}
					result[len(result)-1] = item
				}
			} else {
				item = &ReGainsAttackBytesResult{}
...
func (r ReGainsAttack) FindAllBytes(input []byte, n int) []*ReGainsAttackBytesResult {
...
func (ReGainsAttack) DefaultMaxLeftover() int {
	return 1048576
}
...
func (ReGainsAttack) FindReader(r io.Reader, cfg stream.Config, onMatch func(stream.Match[*ReGainsAttackBytesResult]) bool) error {
	...
	reuseResult := &ReGainsAttackBytesResult{}
			...
			if err == io.EOF {
				...
				if leftover > 0 {
					...
					for searchPos < len(chunk) {
						result, ok := ReGainsAttack{}.FindBytesReuse(chunk[searchPos:], reuseResult)
						if !ok {
							break
						}
						...
						m := stream.Match[*ReGainsAttackBytesResult]{
...
func (ReGainsAttack) FindReaderCount(r io.Reader, cfg stream.Config) (int64, error) {
	var count int64
	err := ReGainsAttack{}.FindReader(r, cfg, func(_ stream.Match[*ReGainsAttackBytesResult]) bool {
		count++
		return true
	})
	return count, err
...
func (ReGainsAttack) FindReaderFirst(r io.Reader, cfg stream.Config) (*ReGainsAttackBytesResult, int64, error) {
	var result *ReGainsAttackBytesResult
	var offset int64
	err := ReGainsAttack{}.FindReader(r, cfg, func(m stream.Match[*ReGainsAttackBytesResult]) bool {
		// Copy the result since buffer will be reused
		result = ReGainsAttack{}.copyBytesResult(m.Result)
...
func (ReGainsAttack) copyBytesResult(src *ReGainsAttackBytesResult) *ReGainsAttackBytesResult {
	if src == nil {
		return nil
	}
	dst := &ReGainsAttackBytesResult{}
	// Copy Match slice
...
//	    })
//	io.Copy(os.Stdout, r)
func (ReGainsAttack) NewTransformReader(r io.Reader, cfg stream.TransformConfig, onMatch func(match *ReGainsAttackBytesResult, emit func([]byte))) io.Reader {
	if cfg.BufferSize == 0 {
	...
	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReGainsAttack{}.processTransform(data, isEOF, onMatch, emitOut)
	}
...
func (ReGainsAttack) processTransform(data []byte, isEOF bool, onMatch func(match *ReGainsAttackBytesResult, emit func([]byte)), emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReGainsAttackBytesResult{}

	for {
		result, ok := ReGainsAttack{}.FindBytesReuse(data[processed:], reuseResult)
		if !ok {
			// No more matches
			if isEOF {
...
//	io.Copy(os.Stdout, r)
func (ReGainsAttack) ReplaceReader(r io.Reader, template string) io.Reader {
	parsed, err := replace.Parse(template)
	if err != nil {
		return &reGainsAttackTransformErrReader{err: err}
	}
	...
	return ReGainsAttack{}.NewTransformReader(r, stream.DefaultTransformConfig(), func(m *ReGainsAttackBytesResult, emit func([]byte)) {
		...
		for _, seg := range resolved {
			...
			case replace.SegmentCaptureIndex:
				capture := (ReGainsAttack{}).getCaptureByIndex(m, seg.CaptureIndex)
				if capture != nil {
...
// reGainsAttackTransformErrReader is a reader that always returns an error.
type reGainsAttackTransformErrReader struct {
	err error
}

func (r *reGainsAttackTransformErrReader) Read(p []byte) (int, error) {
	return 0, r.err
}

// getCaptureByIndex returns the capture group value by its index.
func (ReGainsAttack) getCaptureByIndex(m *ReGainsAttackBytesResult, index int) []byte {
	switch index {
	case 0:
		return m.Match
	default:
...
//	})
func (ReGainsAttack) SelectReader(r io.Reader, pred func(*ReGainsAttackBytesResult) bool) io.Reader {
	cfg := stream.DefaultTransformConfig()
	cfg.MaxLeftover = 1048576

	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReGainsAttack{}.processSelect(data, isEOF, pred, emitOut)
	}
...
func (ReGainsAttack) processSelect(data []byte, isEOF bool, pred func(*ReGainsAttackBytesResult) bool, emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReGainsAttackBytesResult{}
...
//	})
func (ReGainsAttack) RejectReader(r io.Reader, pred func(*ReGainsAttackBytesResult) bool) io.Reader {
	cfg := stream.DefaultTransformConfig()
	cfg.MaxLeftover = 1048576

	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReGainsAttack{}.processReject(data, isEOF, pred, emitOut)
	}
...
func (ReGainsAttack) processReject(data []byte, isEOF bool, pred func(*ReGainsAttackBytesResult) bool, emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReGainsAttackBytesResult{}
...
func (r *ReGainsAttackResult) CaptureByIndex(idx int) string {
	switch idx {
	case 0:
		return r.Match
	case 1:
...
// CaptureByIndex returns the capture group value by its 0-based index.
// Index 0 returns the full match, 1+ returns capture groups.
func (r *ReGainsAttackBytesResult) CaptureByIndex(idx int) []byte {
	switch idx {
	case 0:
		return r.Match
	case 1:
...
func (ReGainsAttack) ReplaceAllString(input string, template string) string {
	...
	lastEnd := 0
	var r ReGainsAttackResult

	remaining := input
	offset := 0

	for {
		match, ok := ReGainsAttack{}.FindStringReuse(remaining, &r)
		if !ok {
			break
		}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReGainsAttack) ReplaceAllBytes(input []byte, template string) []byte {
	return ReGainsAttack{}.ReplaceAllBytesAppend(input, template, nil)
}

// ReplaceAllBytesAppend replaces all matches and appends to buf.
// If buf has sufficient capacity, no allocation occurs.
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReGainsAttack) ReplaceAllBytesAppend(input []byte, template string, buf []byte) []byte {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	result := buf[:0]
	lastEnd := 0
	var r ReGainsAttackBytesResult

	remaining := input
	offset := 0

	for {
		match, ok := ReGainsAttack{}.FindBytesReuse(remaining, &r)
		if !ok {
			break
		}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReGainsAttack) ReplaceFirstString(input string, template string) string {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	var r ReGainsAttackResult
	match, ok := ReGainsAttack{}.FindStringReuse(input, &r)
	if !ok {
		return input
	}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReGainsAttack) ReplaceFirstBytes(input []byte, template string) []byte {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	var r ReGainsAttackBytesResult
	match, ok := ReGainsAttack{}.FindBytesReuse(input, &r)
	if !ok {
...
// ReGainsAttackReplaceTemplate holds a pre-compiled replace template for the ReGainsAttack pattern.
// Use CompileReplaceTemplate to create one, then call its Replace methods.
type ReGainsAttackReplaceTemplate struct {
	original string
...
//
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReGainsAttack) CompileReplaceTemplate(template string) (*ReGainsAttackReplaceTemplate, error) {
	tmpl, err := replace.Parse(template)
	if err != nil {
		return nil, err
	}
	...
	return &ReGainsAttackReplaceTemplate{
		original: template,
		segments: resolved,
	}, nil
}
...
func (t *ReGainsAttackReplaceTemplate) String() string {
	return t.original
}

// ReplaceAllString replaces all matches in input using this compiled template.
func (t *ReGainsAttackReplaceTemplate) ReplaceAllString(input string) string {
	var result strings.Builder
	lastEnd := 0
	var r ReGainsAttackResult
...
func (t *ReGainsAttackReplaceTemplate) ReplaceAllBytes(input []byte) []byte {
	return t.ReplaceAllBytesAppend(input, nil)
}

// ReplaceAllBytesAppend replaces all matches and appends to buf.
// If buf has sufficient capacity, no allocation occurs.
func (t *ReGainsAttackReplaceTemplate) ReplaceAllBytesAppend(input []byte, buf []byte) []byte {
	result := buf[:0]
	lastEnd := 0
	var r ReGainsAttackBytesResult
...
func (t *ReGainsAttackReplaceTemplate) ReplaceFirstString(input string) string {
	var r ReGainsAttackResult
	match, ok := ReGainsAttack{}.FindStringReuse(input, &r)
	if !ok {
		return input
...
func (t *ReGainsAttackReplaceTemplate) ReplaceFirstBytes(input []byte) []byte {
	var r ReGainsAttackBytesResult
	match, ok := ReGainsAttack{}.FindBytesReuse(input, &r)
	if !ok {
		return append([]byte{}, input...)
