...
// Code generated by regengo for pattern: (.+[^\s])'s (.+[^\s]) heals (.+[^\s]) for (\d+)\.
// DO NOT EDIT.

var reHealHitStackPool = sync.Pool{New: func() interface{} {
	stack := make([][3]int, 0, 32)
	return &stack
}}

var reHealHitCaptureStackPool = sync.Pool{New: func() interface{} {
	stack := make([]int, 0, 160)
	return &stack
}}

type ReHealHit struct{}

var CompiledReHealHit = ReHealHit{}

// MinMatchLen is the minimum number of bytes any match can have.
const ReHealHitMinMatchLen = 23

// MaxMatchLen is the maximum number of bytes any match can have.
// -1 means unbounded (pattern contains * or + quantifiers).
const ReHealHitMaxMatchLen = -1

func (ReHealHit) MatchLengthInfo() (minLen, maxLen int) {
	return ReHealHitMinMatchLen, ReHealHitMaxMatchLen
}

func (ReHealHit) MatchString(input string) bool {
	l := len(input)
	offset := 0
	stackPtr := reHealHitStackPool.Get().(*[][3]int)
	stack := (*stackPtr)[:0]
	defer func() {
		for i := range stack {
			stack[i] = [3]int{0, 0, 0}
		...
		*stackPtr = stack[:0]
		reHealHitStackPool.Put(stackPtr)
	}()
	nextInstruction := 1
	goto StepSelect
TryFallback:
...
func (ReHealHit) MatchBytes(input []byte) bool {
	l := len(input)
	offset := 0
	stackPtr := reHealHitStackPool.Get().(*[][3]int)
	stack := (*stackPtr)[:0]
...
type ReHealHitBytesResult struct {
...
func (ReHealHit) FindStringReuse(input string, r *ReHealHitResult) (*ReHealHitResult, bool) {
	l := len(input)
	offset := 0
	var captures [10]int
	stackPtr := reHealHitStackPool.Get().(*[][3]int)
...
func (recv ReHealHit) FindString(input string) (*ReHealHitResult, bool) {
	return recv.FindStringReuse(input, nil)
}
func (ReHealHit) FindAllStringAppend(input string, n int, s []*ReHealHitResult) []*ReHealHitResult {
	if n == 0 {
		return s
	}
	result := s
	...
	Ins36:
		{
			captures[1] = offset
			var item *ReHealHitResult
			if len(result) < cap(result) {
				result = result[:len(result)+1]
				item = result[len(result)-1]
				if item == nil {
					item = &ReHealHitResult{}
...
func (r ReHealHit) FindAllString(input string, n int) []*ReHealHitResult {
	return r.FindAllStringAppend(input, n, nil)
}
func (ReHealHit) FindBytesReuse(input []byte, r *ReHealHitBytesResult) (*ReHealHitBytesResult, bool) {
	l := len(input)
	offset := 0
	var captures [10]int
	stackPtr := reHealHitStackPool.Get().(*[][3]int)
...
func (recv ReHealHit) FindBytes(input []byte) (*ReHealHitBytesResult, bool) {
	return recv.FindBytesReuse(input, nil)
}
func (ReHealHit) FindAllBytesAppend(input []byte, n int, s []*ReHealHitBytesResult) []*ReHealHitBytesResult {
	if n == 0 {
		return s
	}
	result := s
	...
	Ins36:
		{
			captures[1] = offset
			var item *ReHealHitBytesResult
			if len(result) < cap(result) {
				result = result[:len(result)+1]
				item = result[len(result)-1]
				if item == nil {
					item = &ReHealHitBytesResult{}
					result[len(result)-1] = item
				}
			} else {
				item = &ReHealHitBytesResult{}
...
func (r ReHealHit) FindAllBytes(input []byte, n int) []*ReHealHitBytesResult {
...
func (ReHealHit) DefaultMaxLeftover() int {
	return 1048576
}
...
func (ReHealHit) FindReader(r io.Reader, cfg stream.Config, onMatch func(stream.Match[*ReHealHitBytesResult]) bool) error {
	...
	reuseResult := &ReHealHitBytesResult{}
			...
			if err == io.EOF {
				...
				if leftover > 0 {
					...
					for searchPos < len(chunk) {
						result, ok := ReHealHit{}.FindBytesReuse(chunk[searchPos:], reuseResult)
						if !ok {
							break
						}
						...
						m := stream.Match[*ReHealHitBytesResult]{
...
func (ReHealHit) FindReaderCount(r io.Reader, cfg stream.Config) (int64, error) {
	var count int64
	err := ReHealHit{}.FindReader(r, cfg, func(_ stream.Match[*ReHealHitBytesResult]) bool {
		count++
		return true
	})
	return count, err
...
func (ReHealHit) FindReaderFirst(r io.Reader, cfg stream.Config) (*ReHealHitBytesResult, int64, error) {
	var result *ReHealHitBytesResult
	var offset int64
	err := ReHealHit{}.FindReader(r, cfg, func(m stream.Match[*ReHealHitBytesResult]) bool {
		// Copy the result since buffer will be reused
		result = ReHealHit{}.copyBytesResult(m.Result)
...
func (ReHealHit) copyBytesResult(src *ReHealHitBytesResult) *ReHealHitBytesResult {
	if src == nil {
		return nil
	}
	dst := &ReHealHitBytesResult{}
	// Copy Match slice
...
//	    })
//	io.Copy(os.Stdout, r)
func (ReHealHit) NewTransformReader(r io.Reader, cfg stream.TransformConfig, onMatch func(match *ReHealHitBytesResult, emit func([]byte))) io.Reader {
	if cfg.BufferSize == 0 {
	...
	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReHealHit{}.processTransform(data, isEOF, onMatch, emitOut)
	}
...
func (ReHealHit) processTransform(data []byte, isEOF bool, onMatch func(match *ReHealHitBytesResult, emit func([]byte)), emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReHealHitBytesResult{}

	for {
		result, ok := ReHealHit{}.FindBytesReuse(data[processed:], reuseResult)
		if !ok {
			// No more matches
			if isEOF {
...
//	io.Copy(os.Stdout, r)
func (ReHealHit) ReplaceReader(r io.Reader, template string) io.Reader {
	parsed, err := replace.Parse(template)
	if err != nil {
		return &reHealHitTransformErrReader{err: err}
	}
	...
	return ReHealHit{}.NewTransformReader(r, stream.DefaultTransformConfig(), func(m *ReHealHitBytesResult, emit func([]byte)) {
		...
		for _, seg := range resolved {
			...
			case replace.SegmentCaptureIndex:
				capture := (ReHealHit{}).getCaptureByIndex(m, seg.CaptureIndex)
				if capture != nil {
...
// reHealHitTransformErrReader is a reader that always returns an error.
type reHealHitTransformErrReader struct {
	err error
}

func (r *reHealHitTransformErrReader) Read(p []byte) (int, error) {
	return 0, r.err
}

// getCaptureByIndex returns the capture group value by its index.
func (ReHealHit) getCaptureByIndex(m *ReHealHitBytesResult, index int) []byte {
	switch index {
	case 0:
		return m.Match
	default:
...
//	})
func (ReHealHit) SelectReader(r io.Reader, pred func(*ReHealHitBytesResult) bool) io.Reader {
	cfg := stream.DefaultTransformConfig()
	cfg.MaxLeftover = 1048576

	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReHealHit{}.processSelect(data, isEOF, pred, emitOut)
	}
...
func (ReHealHit) processSelect(data []byte, isEOF bool, pred func(*ReHealHitBytesResult) bool, emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReHealHitBytesResult{}
...
//	})
func (ReHealHit) RejectReader(r io.Reader, pred func(*ReHealHitBytesResult) bool) io.Reader {
	cfg := stream.DefaultTransformConfig()
	cfg.MaxLeftover = 1048576

	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReHealHit{}.processReject(data, isEOF, pred, emitOut)
	}
...
func (ReHealHit) processReject(data []byte, isEOF bool, pred func(*ReHealHitBytesResult) bool, emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReHealHitBytesResult{}
...
func (r *ReHealHitResult) CaptureByIndex(idx int) string {
	switch idx {
	case 0:
		return r.Match
	case 1:
...
// CaptureByIndex returns the capture group value by its 0-based index.
// Index 0 returns the full match, 1+ returns capture groups.
func (r *ReHealHitBytesResult) CaptureByIndex(idx int) []byte {
	switch idx {
	case 0:
		return r.Match
	case 1:
...
func (ReHealHit) ReplaceAllString(input string, template string) string {
	...
	lastEnd := 0
	var r ReHealHitResult

	remaining := input
	offset := 0

	for {
		match, ok := ReHealHit{}.FindStringReuse(remaining, &r)
		if !ok {
			break
		}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReHealHit) ReplaceAllBytes(input []byte, template string) []byte {
	return ReHealHit{}.ReplaceAllBytesAppend(input, template, nil)
}

// ReplaceAllBytesAppend replaces all matches and appends to buf.
// If buf has sufficient capacity, no allocation occurs.
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReHealHit) ReplaceAllBytesAppend(input []byte, template string, buf []byte) []byte {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	result := buf[:0]
	lastEnd := 0
	var r ReHealHitBytesResult

	remaining := input
	offset := 0

	for {
		match, ok := ReHealHit{}.FindBytesReuse(remaining, &r)
		if !ok {
			break
		}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReHealHit) ReplaceFirstString(input string, template string) string {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	var r ReHealHitResult
	match, ok := ReHealHit{}.FindStringReuse(input, &r)
	if !ok {
		return input
	}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReHealHit) ReplaceFirstBytes(input []byte, template string) []byte {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	var r ReHealHitBytesResult
	match, ok := ReHealHit{}.FindBytesReuse(input, &r)
	if !ok {
...
// ReHealHitReplaceTemplate holds a pre-compiled replace template for the ReHealHit pattern.
// Use CompileReplaceTemplate to create one, then call its Replace methods.
type ReHealHitReplaceTemplate struct {
	original string
...
//
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReHealHit) CompileReplaceTemplate(template string) (*ReHealHitReplaceTemplate, error) {
	tmpl, err := replace.Parse(template)
	if err != nil {
		return nil, err
	}
	...
	return &ReHealHitReplaceTemplate{
		original: template,
		segments: resolved,
	}, nil
}
...
func (t *ReHealHitReplaceTemplate) String() string {
	return t.original
}

// ReplaceAllString replaces all matches in input using this compiled template.
func (t *ReHealHitReplaceTemplate) ReplaceAllString(input string) string {
	var result strings.Builder
	lastEnd := 0
	var r ReHealHitResult
...
func (t *ReHealHitReplaceTemplate) ReplaceAllBytes(input []byte) []byte {
	return t.ReplaceAllBytesAppend(input, nil)
}

// ReplaceAllBytesAppend replaces all matches and appends to buf.
// If buf has sufficient capacity, no allocation occurs.
func (t *ReHealHitReplaceTemplate) ReplaceAllBytesAppend(input []byte, buf []byte) []byte {
	result := buf[:0]
	lastEnd := 0
	var r ReHealHitBytesResult
...
func (t *ReHealHitReplaceTemplate) ReplaceFirstString(input string) string {
	var r ReHealHitResult
	match, ok := ReHealHit{}.FindStringReuse(input, &r)
	if !ok {
		return input
...
func (t *ReHealHitReplaceTemplate) ReplaceFirstBytes(input []byte) []byte {
	var r ReHealHitBytesResult
	match, ok := ReHealHit{}.FindBytesReuse(input, &r)
	if !ok {
		return append([]byte{}, input...)
