...
// Code generated by regengo for pattern: (.+[^\s]) resists (.+[^\s])\s's (.+[^\s])\.
// DO NOT EDIT.

var reDamageProcResistStackPool = sync.Pool{New: func() interface{} {
	stack := make([][3]int, 0, 32)
	return &stack
}}

var reDamageProcResistCaptureStackPool = sync.Pool{New: func() interface{} {
	stack := make([]int, 0, 128)
	return &stack
}}

type ReDamageProcResist struct{}

var CompiledReDamageProcResist = ReDamageProcResist{}

// MinMatchLen is the minimum number of bytes any match can have.
const ReDamageProcResistMinMatchLen = 20

// MaxMatchLen is the maximum number of bytes any match can have.
// -1 means unbounded (pattern contains * or + quantifiers).
const ReDamageProcResistMaxMatchLen = -1

func (ReDamageProcResist) MatchLengthInfo() (minLen, maxLen int) {
	return ReDamageProcResistMinMatchLen, ReDamageProcResistMaxMatchLen
}

func (ReDamageProcResist) MatchString(input string) bool {
	l := len(input)
	offset := 0
	stackPtr := reDamageProcResistStackPool.Get().(*[][3]int)
	stack := (*stackPtr)[:0]
	defer func() {
		for i := range stack {
			stack[i] = [3]int{0, 0, 0}
		...
		*stackPtr = stack[:0]
		reDamageProcResistStackPool.Put(stackPtr)
	}()
	nextInstruction := 1
	goto StepSelect
TryFallback:
...
func (ReDamageProcResist) MatchBytes(input []byte) bool {
	l := len(input)
	offset := 0
	stackPtr := reDamageProcResistStackPool.Get().(*[][3]int)
	stack := (*stackPtr)[:0]
...
type ReDamageProcResistResult struct {
...
type ReDamageProcResistBytesResult struct {
...
func (ReDamageProcResist) FindStringReuse(input string, r *ReDamageProcResistResult) (*ReDamageProcResistResult, bool) {
	l := len(input)
	offset := 0
	var captures [8]int
	captureStackPtr := reDamageProcResistCaptureStackPool.Get().(*[]int)
	captureStack := (*captureStackPtr)[:0]
	defer func() {
		*captureStackPtr = captureStack[:0]
		reDamageProcResistCaptureStackPool.Put(captureStackPtr)
	}()
	stackPtr := reDamageProcResistStackPool.Get().(*[][3]int)
	stack := (*stackPtr)[:0]
	defer func() {
...
func (recv ReDamageProcResist) FindString(input string) (*ReDamageProcResistResult, bool) {
	return recv.FindStringReuse(input, nil)
}
func (ReDamageProcResist) FindAllStringAppend(input string, n int, s []*ReDamageProcResistResult) []*ReDamageProcResistResult {
	if n == 0 {
		return s
	}
	result := s
	...
	Ins30:
		{
			captures[1] = offset
			var item *ReDamageProcResistResult
			if len(result) < cap(result) {
				result = result[:len(result)+1]
				item = result[len(result)-1]
				if item == nil {
					item = &ReDamageProcResistResult{}
...
func (r ReDamageProcResist) FindAllString(input string, n int) []*ReDamageProcResistResult {
	return r.FindAllStringAppend(input, n, nil)
}
func (ReDamageProcResist) FindBytesReuse(input []byte, r *ReDamageProcResistBytesResult) (*ReDamageProcResistBytesResult, bool) {
	l := len(input)
	offset := 0
	var captures [8]int
	captureStackPtr := reDamageProcResistCaptureStackPool.Get().(*[]int)
...
Ins30:
	{
		captures[1] = offset
		if r == nil {
			r = &ReDamageProcResistBytesResult{}
		}
...
func (recv ReDamageProcResist) FindBytes(input []byte) (*ReDamageProcResistBytesResult, bool) {
	return recv.FindBytesReuse(input, nil)
}
func (ReDamageProcResist) FindAllBytesAppend(input []byte, n int, s []*ReDamageProcResistBytesResult) []*ReDamageProcResistBytesResult {
	if n == 0 {
		return s
	}
	result := s
	...
	Ins30:
		{
			captures[1] = offset
			var item *ReDamageProcResistBytesResult
			if len(result) < cap(result) {
				result = result[:len(result)+1]
				item = result[len(result)-1]
				if item == nil {
					item = &ReDamageProcResistBytesResult{}
					result[len(result)-1] = item
				}
			} else {
				item = &ReDamageProcResistBytesResult{}
...
func (r ReDamageProcResist) FindAllBytes(input []byte, n int) []*ReDamageProcResistBytesResult {
...
func (ReDamageProcResist) DefaultMaxLeftover() int {
	return 1048576
}
...
func (ReDamageProcResist) FindReader(r io.Reader, cfg stream.Config, onMatch func(stream.Match[*ReDamageProcResistBytesResult]) bool) error {
	...
	reuseResult := &ReDamageProcResistBytesResult{}
			...
			if err == io.EOF {
				...
				if leftover > 0 {
					...
					for searchPos < len(chunk) {
						result, ok := ReDamageProcResist{}.FindBytesReuse(chunk[searchPos:], reuseResult)
						if !ok {
							break
						}
						...
						m := stream.Match[*ReDamageProcResistBytesResult]{
...
func (ReDamageProcResist) FindReaderCount(r io.Reader, cfg stream.Config) (int64, error) {
	var count int64
	err := ReDamageProcResist{}.FindReader(r, cfg, func(_ stream.Match[*ReDamageProcResistBytesResult]) bool {
		count++
		return true
	})
	return count, err
...
func (ReDamageProcResist) FindReaderFirst(r io.Reader, cfg stream.Config) (*ReDamageProcResistBytesResult, int64, error) {
	var result *ReDamageProcResistBytesResult
	var offset int64
	err := ReDamageProcResist{}.FindReader(r, cfg, func(m stream.Match[*ReDamageProcResistBytesResult]) bool {
		// Copy the result since buffer will be reused
		result = ReDamageProcResist{}.copyBytesResult(m.Result)
...
func (ReDamageProcResist) copyBytesResult(src *ReDamageProcResistBytesResult) *ReDamageProcResistBytesResult {
	if src == nil {
		return nil
	}
	dst := &ReDamageProcResistBytesResult{}
	// Copy Match slice
...
//	    })
//	io.Copy(os.Stdout, r)
func (ReDamageProcResist) NewTransformReader(r io.Reader, cfg stream.TransformConfig, onMatch func(match *ReDamageProcResistBytesResult, emit func([]byte))) io.Reader {
	if cfg.BufferSize == 0 {
	...
	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReDamageProcResist{}.processTransform(data, isEOF, onMatch, emitOut)
	}
...
func (ReDamageProcResist) processTransform(data []byte, isEOF bool, onMatch func(match *ReDamageProcResistBytesResult, emit func([]byte)), emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReDamageProcResistBytesResult{}

	for {
		result, ok := ReDamageProcResist{}.FindBytesReuse(data[processed:], reuseResult)
		if !ok {
			// No more matches
			if isEOF {
...
//	io.Copy(os.Stdout, r)
func (ReDamageProcResist) ReplaceReader(r io.Reader, template string) io.Reader {
	parsed, err := replace.Parse(template)
	if err != nil {
		return &reDamageProcResistTransformErrReader{err: err}
	}
	...
	return ReDamageProcResist{}.NewTransformReader(r, stream.DefaultTransformConfig(), func(m *ReDamageProcResistBytesResult, emit func([]byte)) {
		...
		for _, seg := range resolved {
			...
			case replace.SegmentCaptureIndex:
				capture := (ReDamageProcResist{}).getCaptureByIndex(m, seg.CaptureIndex)
				if capture != nil {
...
// reDamageProcResistTransformErrReader is a reader that always returns an error.
type reDamageProcResistTransformErrReader struct {
	err error
}

func (r *reDamageProcResistTransformErrReader) Read(p []byte) (int, error) {
	return 0, r.err
}

// getCaptureByIndex returns the capture group value by its index.
func (ReDamageProcResist) getCaptureByIndex(m *ReDamageProcResistBytesResult, index int) []byte {
	switch index {
	case 0:
		return m.Match
	default:
...
//	})
func (ReDamageProcResist) SelectReader(r io.Reader, pred func(*ReDamageProcResistBytesResult) bool) io.Reader {
	cfg := stream.DefaultTransformConfig()
	cfg.MaxLeftover = 1048576

	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReDamageProcResist{}.processSelect(data, isEOF, pred, emitOut)
	}
...
func (ReDamageProcResist) processSelect(data []byte, isEOF bool, pred func(*ReDamageProcResistBytesResult) bool, emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReDamageProcResistBytesResult{}
...
//	})
func (ReDamageProcResist) RejectReader(r io.Reader, pred func(*ReDamageProcResistBytesResult) bool) io.Reader {
	cfg := stream.DefaultTransformConfig()
	cfg.MaxLeftover = 1048576

	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReDamageProcResist{}.processReject(data, isEOF, pred, emitOut)
	}
...
func (ReDamageProcResist) processReject(data []byte, isEOF bool, pred func(*ReDamageProcResistBytesResult) bool, emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReDamageProcResistBytesResult{}
...
func (r *ReDamageProcResistResult) CaptureByIndex(idx int) string {
	switch idx {
	case 0:
		return r.Match
	case 1:
...
// CaptureByIndex returns the capture group value by its 0-based index.
// Index 0 returns the full match, 1+ returns capture groups.
func (r *ReDamageProcResistBytesResult) CaptureByIndex(idx int) []byte {
	switch idx {
	case 0:
		return r.Match
	case 1:
...
func (ReDamageProcResist) ReplaceAllString(input string, template string) string {
	...
	lastEnd := 0
	var r ReDamageProcResistResult

	remaining := input
	offset := 0

	for {
		match, ok := ReDamageProcResist{}.FindStringReuse(remaining, &r)
		if !ok {
			break
		}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReDamageProcResist) ReplaceAllBytes(input []byte, template string) []byte {
	return ReDamageProcResist{}.ReplaceAllBytesAppend(input, template, nil)
}

// ReplaceAllBytesAppend replaces all matches and appends to buf.
// If buf has sufficient capacity, no allocation occurs.
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReDamageProcResist) ReplaceAllBytesAppend(input []byte, template string, buf []byte) []byte {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	result := buf[:0]
	lastEnd := 0
	var r ReDamageProcResistBytesResult

	remaining := input
	offset := 0

	for {
		match, ok := ReDamageProcResist{}.FindBytesReuse(remaining, &r)
		if !ok {
			break
		}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReDamageProcResist) ReplaceFirstString(input string, template string) string {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	var r ReDamageProcResistResult
	match, ok := ReDamageProcResist{}.FindStringReuse(input, &r)
	if !ok {
		return input
	}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReDamageProcResist) ReplaceFirstBytes(input []byte, template string) []byte {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	var r ReDamageProcResistBytesResult
	match, ok := ReDamageProcResist{}.FindBytesReuse(input, &r)
	if !ok {
...
// ReDamageProcResistReplaceTemplate holds a pre-compiled replace template for the ReDamageProcResist pattern.
// Use CompileReplaceTemplate to create one, then call its Replace methods.
type ReDamageProcResistReplaceTemplate struct {
	original string
...
//
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReDamageProcResist) CompileReplaceTemplate(template string) (*ReDamageProcResistReplaceTemplate, error) {
	tmpl, err := replace.Parse(template)
	if err != nil {
		return nil, err
	}
	...
	return &ReDamageProcResistReplaceTemplate{
		original: template,
		segments: resolved,
	}, nil
}
...
func (t *ReDamageProcResistReplaceTemplate) String() string {
	return t.original
}

// ReplaceAllString replaces all matches in input using this compiled template.
func (t *ReDamageProcResistReplaceTemplate) ReplaceAllString(input string) string {
	var result strings.Builder
	lastEnd := 0
	var r ReDamageProcResistResult
...
func (t *ReDamageProcResistReplaceTemplate) ReplaceAllBytes(input []byte) []byte {
	return t.ReplaceAllBytesAppend(input, nil)
}

// ReplaceAllBytesAppend replaces all matches and appends to buf.
// If buf has sufficient capacity, no allocation occurs.
func (t *ReDamageProcResistReplaceTemplate) ReplaceAllBytesAppend(input []byte, buf []byte) []byte {
	result := buf[:0]
	lastEnd := 0
	var r ReDamageProcResistBytesResult
...
func (t *ReDamageProcResistReplaceTemplate) ReplaceFirstString(input string) string {
	var r ReDamageProcResistResult
	match, ok := ReDamageProcResist{}.FindStringReuse(input, &r)
	if !ok {
		return input
...
func (t *ReDamageProcResistReplaceTemplate) ReplaceFirstBytes(input []byte) []byte {
	var r ReDamageProcResistBytesResult
	match, ok := ReDamageProcResist{}.FindBytesReuse(input, &r)
	if !ok {
		return append([]byte{}, input...)
