...
// Code generated by regengo for pattern: (.+[^\s])'s (.+[^\s]) drains (\d+) (Mana) from (.+[^\s])\.
// DO NOT EDIT.

var reResourceDrainStackPool = sync.Pool{New: func() interface{} {
	stack := make([][3]int, 0, 32)
	return &stack
}}

var reResourceDrainCaptureStackPool = sync.Pool{New: func() interface{} {
	stack := make([]int, 0, 192)
	return &stack
}}

type ReResourceDrain struct{}

var CompiledReResourceDrain = ReResourceDrain{}

// MinMatchLen is the minimum number of bytes any match can have.
const ReResourceDrainMinMatchLen = 30

// MaxMatchLen is the maximum number of bytes any match can have.
// -1 means unbounded (pattern contains * or + quantifiers).
const ReResourceDrainMaxMatchLen = -1

func (ReResourceDrain) MatchLengthInfo() (minLen, maxLen int) {
	return ReResourceDrainMinMatchLen, ReResourceDrainMaxMatchLen
}

func (ReResourceDrain) MatchString(input string) bool {
	l := len(input)
	offset := 0
	stackPtr := reResourceDrainStackPool.Get().(*[][3]int)
	stack := (*stackPtr)[:0]
	defer func() {
		for i := range stack {
			stack[i] = [3]int{0, 0, 0}
		...
		*stackPtr = stack[:0]
		reResourceDrainStackPool.Put(stackPtr)
	}()
	nextInstruction := 1
	goto StepSelect
TryFallback:
...
func (ReResourceDrain) MatchBytes(input []byte) bool {
	l := len(input)
	offset := 0
	stackPtr := reResourceDrainStackPool.Get().(*[][3]int)
	stack := (*stackPtr)[:0]
...
type ReResourceDrainResult struct {
...
type ReResourceDrainBytesResult struct {
...
func (ReResourceDrain) FindStringReuse(input string, r *ReResourceDrainResult) (*ReResourceDrainResult, bool) {
	l := len(input)
	offset := 0
	var captures [12]int
	stackPtr := reResourceDrainStackPool.Get().(*[][3]int)
...
func (recv ReResourceDrain) FindString(input string) (*ReResourceDrainResult, bool) {
	return recv.FindStringReuse(input, nil)
}
func (ReResourceDrain) FindAllStringAppend(input string, n int, s []*ReResourceDrainResult) []*ReResourceDrainResult {
	if n == 0 {
		return s
	}
	result := s
	...
	Ins45:
		{
			captures[1] = offset
			var item *ReResourceDrainResult
			if len(result) < cap(result) {
				result = result[:len(result)+1]
				item = result[len(result)-1]
				if item == nil {
					item = &ReResourceDrainResult{}
...
func (r ReResourceDrain) FindAllString(input string, n int) []*ReResourceDrainResult {
	return r.FindAllStringAppend(input, n, nil)
}
func (ReResourceDrain) FindBytesReuse(input []byte, r *ReResourceDrainBytesResult) (*ReResourceDrainBytesResult, bool) {
	l := len(input)
	offset := 0
	var captures [12]int
	stackPtr := reResourceDrainStackPool.Get().(*[][3]int)
...
func (recv ReResourceDrain) FindBytes(input []byte) (*ReResourceDrainBytesResult, bool) {
	return recv.FindBytesReuse(input, nil)
}
func (ReResourceDrain) FindAllBytesAppend(input []byte, n int, s []*ReResourceDrainBytesResult) []*ReResourceDrainBytesResult {
	if n == 0 {
		return s
	}
	result := s
	...
	Ins45:
		{
			captures[1] = offset
			var item *ReResourceDrainBytesResult
			if len(result) < cap(result) {
				result = result[:len(result)+1]
				item = result[len(result)-1]
				if item == nil {
					item = &ReResourceDrainBytesResult{}
					result[len(result)-1] = item
				}
			} else {
				item = &ReResourceDrainBytesResult{}
...
func (r ReResourceDrain) FindAllBytes(input []byte, n int) []*ReResourceDrainBytesResult {
...
func (ReResourceDrain) DefaultMaxLeftover() int {
	return 1048576
}
...
func (ReResourceDrain) FindReader(r io.Reader, cfg stream.Config, onMatch func(stream.Match[*ReResourceDrainBytesResult]) bool) error {
	...
	reuseResult := &ReResourceDrainBytesResult{}
			...
			if err == io.EOF {
				...
				if leftover > 0 {
					...
					for searchPos < len(chunk) {
						result, ok := ReResourceDrain{}.FindBytesReuse(chunk[searchPos:], reuseResult)
						if !ok {
							break
						}
						...
						m := stream.Match[*ReResourceDrainBytesResult]{
...
func (ReResourceDrain) FindReaderCount(r io.Reader, cfg stream.Config) (int64, error) {
	var count int64
	err := ReResourceDrain{}.FindReader(r, cfg, func(_ stream.Match[*ReResourceDrainBytesResult]) bool {
		count++
		return true
	})
	return count, err
...
func (ReResourceDrain) FindReaderFirst(r io.Reader, cfg stream.Config) (*ReResourceDrainBytesResult, int64, error) {
	var result *ReResourceDrainBytesResult
	var offset int64
	err := ReResourceDrain{}.FindReader(r, cfg, func(m stream.Match[*ReResourceDrainBytesResult]) bool {
		// Copy the result since buffer will be reused
		result = ReResourceDrain{}.copyBytesResult(m.Result)
...
func (ReResourceDrain) copyBytesResult(src *ReResourceDrainBytesResult) *ReResourceDrainBytesResult {
	if src == nil {
		return nil
	}
	dst := &ReResourceDrainBytesResult{}
	// Copy Match slice
...
//	    })
//	io.Copy(os.Stdout, r)
func (ReResourceDrain) NewTransformReader(r io.Reader, cfg stream.TransformConfig, onMatch func(match *ReResourceDrainBytesResult, emit func([]byte))) io.Reader {
	if cfg.BufferSize == 0 {
	...
	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReResourceDrain{}.processTransform(data, isEOF, onMatch, emitOut)
	}
...
func (ReResourceDrain) processTransform(data []byte, isEOF bool, onMatch func(match *ReResourceDrainBytesResult, emit func([]byte)), emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReResourceDrainBytesResult{}

	for {
		result, ok := ReResourceDrain{}.FindBytesReuse(data[processed:], reuseResult)
		if !ok {
			// No more matches
			if isEOF {
...
//	io.Copy(os.Stdout, r)
func (ReResourceDrain) ReplaceReader(r io.Reader, template string) io.Reader {
	parsed, err := replace.Parse(template)
	if err != nil {
		return &reResourceDrainTransformErrReader{err: err}
	}
	...
	return ReResourceDrain{}.NewTransformReader(r, stream.DefaultTransformConfig(), func(m *ReResourceDrainBytesResult, emit func([]byte)) {
		...
		for _, seg := range resolved {
			...
			case replace.SegmentCaptureIndex:
				capture := (ReResourceDrain{}).getCaptureByIndex(m, seg.CaptureIndex)
				if capture != nil {
...
// reResourceDrainTransformErrReader is a reader that always returns an error.
type reResourceDrainTransformErrReader struct {
	err error
}

func (r *reResourceDrainTransformErrReader) Read(p []byte) (int, error) {
	return 0, r.err
}

// getCaptureByIndex returns the capture group value by its index.
func (ReResourceDrain) getCaptureByIndex(m *ReResourceDrainBytesResult, index int) []byte {
	switch index {
	case 0:
		return m.Match
	default:
...
//	})
func (ReResourceDrain) SelectReader(r io.Reader, pred func(*ReResourceDrainBytesResult) bool) io.Reader {
	cfg := stream.DefaultTransformConfig()
	cfg.MaxLeftover = 1048576

	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReResourceDrain{}.processSelect(data, isEOF, pred, emitOut)
	}
...
func (ReResourceDrain) processSelect(data []byte, isEOF bool, pred func(*ReResourceDrainBytesResult) bool, emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReResourceDrainBytesResult{}
...
//	})
func (ReResourceDrain) RejectReader(r io.Reader, pred func(*ReResourceDrainBytesResult) bool) io.Reader {
	cfg := stream.DefaultTransformConfig()
	cfg.MaxLeftover = 1048576

	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReResourceDrain{}.processReject(data, isEOF, pred, emitOut)
	}
...
func (ReResourceDrain) processReject(data []byte, isEOF bool, pred func(*ReResourceDrainBytesResult) bool, emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReResourceDrainBytesResult{}
...
func (r *ReResourceDrainResult) CaptureByIndex(idx int) string {
	switch idx {
	case 0:
		return r.Match
	case 1:
...
// CaptureByIndex returns the capture group value by its 0-based index.
// Index 0 returns the full match, 1+ returns capture groups.
func (r *ReResourceDrainBytesResult) CaptureByIndex(idx int) []byte {
	switch idx {
	case 0:
		return r.Match
	case 1:
...
func (ReResourceDrain) ReplaceAllString(input string, template string) string {
	...
	lastEnd := 0
	var r ReResourceDrainResult

	remaining := input
	offset := 0

	for {
		match, ok := ReResourceDrain{}.FindStringReuse(remaining, &r)
		if !ok {
			break
		}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReResourceDrain) ReplaceAllBytes(input []byte, template string) []byte {
	return ReResourceDrain{}.ReplaceAllBytesAppend(input, template, nil)
}

// ReplaceAllBytesAppend replaces all matches and appends to buf.
// If buf has sufficient capacity, no allocation occurs.
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReResourceDrain) ReplaceAllBytesAppend(input []byte, template string, buf []byte) []byte {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	result := buf[:0]
	lastEnd := 0
	var r ReResourceDrainBytesResult

	remaining := input
	offset := 0

	for {
		match, ok := ReResourceDrain{}.FindBytesReuse(remaining, &r)
		if !ok {
			break
		}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReResourceDrain) ReplaceFirstString(input string, template string) string {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	var r ReResourceDrainResult
	match, ok := ReResourceDrain{}.FindStringReuse(input, &r)
	if !ok {
		return input
	}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReResourceDrain) ReplaceFirstBytes(input []byte, template string) []byte {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	var r ReResourceDrainBytesResult
	match, ok := ReResourceDrain{}.FindBytesReuse(input, &r)
	if !ok {
...
// ReResourceDrainReplaceTemplate holds a pre-compiled replace template for the ReResourceDrain pattern.
// Use CompileReplaceTemplate to create one, then call its Replace methods.
type ReResourceDrainReplaceTemplate struct {
	original string
...
//
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReResourceDrain) CompileReplaceTemplate(template string) (*ReResourceDrainReplaceTemplate, error) {
	tmpl, err := replace.Parse(template)
	if err != nil {
		return nil, err
	}
	...
	return &ReResourceDrainReplaceTemplate{
		original: template,
		segments: resolved,
	}, nil
}
...
func (t *ReResourceDrainReplaceTemplate) String() string {
	return t.original
}

// ReplaceAllString replaces all matches in input using this compiled template.
func (t *ReResourceDrainReplaceTemplate) ReplaceAllString(input string) string {
	var result strings.Builder
	lastEnd := 0
	var r ReResourceDrainResult
...
func (t *ReResourceDrainReplaceTemplate) ReplaceAllBytes(input []byte) []byte {
	return t.ReplaceAllBytesAppend(input, nil)
}

// ReplaceAllBytesAppend replaces all matches and appends to buf.
// If buf has sufficient capacity, no allocation occurs.
func (t *ReResourceDrainReplaceTemplate) ReplaceAllBytesAppend(input []byte, buf []byte) []byte {
	result := buf[:0]
	lastEnd := 0
	var r ReResourceDrainBytesResult
...
func (t *ReResourceDrainReplaceTemplate) ReplaceFirstString(input string) string {
	var r ReResourceDrainResult
	match, ok := ReResourceDrain{}.FindStringReuse(input, &r)
	if !ok {
		return input
...
func (t *ReResourceDrainReplaceTemplate) ReplaceFirstBytes(input []byte) []byte {
	var r ReResourceDrainBytesResult
	match, ok := ReResourceDrain{}.FindBytesReuse(input, &r)
	if !ok {
		return append([]byte{}, input...)
