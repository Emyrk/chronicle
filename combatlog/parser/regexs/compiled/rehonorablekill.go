...
// Code generated by regengo for pattern: (.+[^\s]) dies, honorable kill Rank: (.+[^\s])  \(Estimated Honor Points: (\d+)\)
// DO NOT EDIT.

var reHonorableKillStackPool = sync.Pool{New: func() interface{} {
	stack := make([][3]int, 0, 32)
	return &stack
}}

var reHonorableKillCaptureStackPool = sync.Pool{New: func() interface{} {
	stack := make([]int, 0, 128)
	return &stack
}}

type ReHonorableKill struct{}

var CompiledReHonorableKill = ReHonorableKill{}

// MinMatchLen is the minimum number of bytes any match can have.
const ReHonorableKillMinMatchLen = 61

// MaxMatchLen is the maximum number of bytes any match can have.
// -1 means unbounded (pattern contains * or + quantifiers).
const ReHonorableKillMaxMatchLen = -1

func (ReHonorableKill) MatchLengthInfo() (minLen, maxLen int) {
	return ReHonorableKillMinMatchLen, ReHonorableKillMaxMatchLen
}

func (ReHonorableKill) MatchString(input string) bool {
	l := len(input)
	offset := 0
	stackPtr := reHonorableKillStackPool.Get().(*[][3]int)
	stack := (*stackPtr)[:0]
	defer func() {
		for i := range stack {
			stack[i] = [3]int{0, 0, 0}
		...
		*stackPtr = stack[:0]
		reHonorableKillStackPool.Put(stackPtr)
	}()
	nextInstruction := 1
	goto StepSelect
TryFallback:
...
func (ReHonorableKill) MatchBytes(input []byte) bool {
	l := len(input)
	offset := 0
	stackPtr := reHonorableKillStackPool.Get().(*[][3]int)
	stack := (*stackPtr)[:0]
...
type ReHonorableKillResult struct {
...
type ReHonorableKillBytesResult struct {
...
func (ReHonorableKill) FindStringReuse(input string, r *ReHonorableKillResult) (*ReHonorableKillResult, bool) {
	l := len(input)
	offset := 0
	var captures [8]int
	captureStackPtr := reHonorableKillCaptureStackPool.Get().(*[]int)
	captureStack := (*captureStackPtr)[:0]
	defer func() {
		*captureStackPtr = captureStack[:0]
		reHonorableKillCaptureStackPool.Put(captureStackPtr)
	}()
	stackPtr := reHonorableKillStackPool.Get().(*[][3]int)
	stack := (*stackPtr)[:0]
	defer func() {
...
Ins36:
	{
		captureStack = append(captureStack, captures[:]...)
		stack = append(stack, [3]int{offset, 37, 1})
		goto Ins35
	}
Ins37:
	{
...
Ins68:
	{
		captureStack = append(captureStack, captures[:]...)
		stack = append(stack, [3]int{offset, 69, 1})
		goto Ins67
	}
Ins69:
	{
...
func (recv ReHonorableKill) FindString(input string) (*ReHonorableKillResult, bool) {
	return recv.FindStringReuse(input, nil)
}
func (ReHonorableKill) FindAllStringAppend(input string, n int, s []*ReHonorableKillResult) []*ReHonorableKillResult {
	if n == 0 {
		return s
	}
	result := s
	...
	Ins71:
		{
			captures[1] = offset
			var item *ReHonorableKillResult
			if len(result) < cap(result) {
				result = result[:len(result)+1]
				item = result[len(result)-1]
				if item == nil {
					item = &ReHonorableKillResult{}
...
func (r ReHonorableKill) FindAllString(input string, n int) []*ReHonorableKillResult {
	return r.FindAllStringAppend(input, n, nil)
}
func (ReHonorableKill) FindBytesReuse(input []byte, r *ReHonorableKillBytesResult) (*ReHonorableKillBytesResult, bool) {
	l := len(input)
	offset := 0
	var captures [8]int
	captureStackPtr := reHonorableKillCaptureStackPool.Get().(*[]int)
...
func (recv ReHonorableKill) FindBytes(input []byte) (*ReHonorableKillBytesResult, bool) {
	return recv.FindBytesReuse(input, nil)
}
func (ReHonorableKill) FindAllBytesAppend(input []byte, n int, s []*ReHonorableKillBytesResult) []*ReHonorableKillBytesResult {
	if n == 0 {
		return s
	}
	result := s
	...
	Ins71:
		{
			captures[1] = offset
			var item *ReHonorableKillBytesResult
			if len(result) < cap(result) {
				result = result[:len(result)+1]
				item = result[len(result)-1]
				if item == nil {
					item = &ReHonorableKillBytesResult{}
					result[len(result)-1] = item
				}
			} else {
				item = &ReHonorableKillBytesResult{}
...
func (r ReHonorableKill) FindAllBytes(input []byte, n int) []*ReHonorableKillBytesResult {
...
func (ReHonorableKill) DefaultMaxLeftover() int {
	return 1048576
}
...
func (ReHonorableKill) FindReader(r io.Reader, cfg stream.Config, onMatch func(stream.Match[*ReHonorableKillBytesResult]) bool) error {
	...
	reuseResult := &ReHonorableKillBytesResult{}
			...
			if err == io.EOF {
				...
				if leftover > 0 {
					...
					for searchPos < len(chunk) {
						result, ok := ReHonorableKill{}.FindBytesReuse(chunk[searchPos:], reuseResult)
						if !ok {
							break
						}
						...
						m := stream.Match[*ReHonorableKillBytesResult]{
...
func (ReHonorableKill) FindReaderCount(r io.Reader, cfg stream.Config) (int64, error) {
	var count int64
	err := ReHonorableKill{}.FindReader(r, cfg, func(_ stream.Match[*ReHonorableKillBytesResult]) bool {
		count++
		return true
	})
	return count, err
...
func (ReHonorableKill) FindReaderFirst(r io.Reader, cfg stream.Config) (*ReHonorableKillBytesResult, int64, error) {
	var result *ReHonorableKillBytesResult
	var offset int64
	err := ReHonorableKill{}.FindReader(r, cfg, func(m stream.Match[*ReHonorableKillBytesResult]) bool {
		// Copy the result since buffer will be reused
		result = ReHonorableKill{}.copyBytesResult(m.Result)
...
func (ReHonorableKill) copyBytesResult(src *ReHonorableKillBytesResult) *ReHonorableKillBytesResult {
	if src == nil {
		return nil
	}
	dst := &ReHonorableKillBytesResult{}
	// Copy Match slice
...
//	    })
//	io.Copy(os.Stdout, r)
func (ReHonorableKill) NewTransformReader(r io.Reader, cfg stream.TransformConfig, onMatch func(match *ReHonorableKillBytesResult, emit func([]byte))) io.Reader {
	if cfg.BufferSize == 0 {
	...
	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReHonorableKill{}.processTransform(data, isEOF, onMatch, emitOut)
	}
...
func (ReHonorableKill) processTransform(data []byte, isEOF bool, onMatch func(match *ReHonorableKillBytesResult, emit func([]byte)), emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReHonorableKillBytesResult{}

	for {
		result, ok := ReHonorableKill{}.FindBytesReuse(data[processed:], reuseResult)
		if !ok {
			// No more matches
			if isEOF {
...
//	io.Copy(os.Stdout, r)
func (ReHonorableKill) ReplaceReader(r io.Reader, template string) io.Reader {
	parsed, err := replace.Parse(template)
	if err != nil {
		return &reHonorableKillTransformErrReader{err: err}
	}
	...
	return ReHonorableKill{}.NewTransformReader(r, stream.DefaultTransformConfig(), func(m *ReHonorableKillBytesResult, emit func([]byte)) {
		...
		for _, seg := range resolved {
			...
			case replace.SegmentCaptureIndex:
				capture := (ReHonorableKill{}).getCaptureByIndex(m, seg.CaptureIndex)
				if capture != nil {
...
// reHonorableKillTransformErrReader is a reader that always returns an error.
type reHonorableKillTransformErrReader struct {
	err error
}

func (r *reHonorableKillTransformErrReader) Read(p []byte) (int, error) {
	return 0, r.err
}

// getCaptureByIndex returns the capture group value by its index.
func (ReHonorableKill) getCaptureByIndex(m *ReHonorableKillBytesResult, index int) []byte {
	switch index {
	case 0:
		return m.Match
	default:
...
//	})
func (ReHonorableKill) SelectReader(r io.Reader, pred func(*ReHonorableKillBytesResult) bool) io.Reader {
	cfg := stream.DefaultTransformConfig()
	cfg.MaxLeftover = 1048576

	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReHonorableKill{}.processSelect(data, isEOF, pred, emitOut)
	}
...
func (ReHonorableKill) processSelect(data []byte, isEOF bool, pred func(*ReHonorableKillBytesResult) bool, emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReHonorableKillBytesResult{}
...
//	})
func (ReHonorableKill) RejectReader(r io.Reader, pred func(*ReHonorableKillBytesResult) bool) io.Reader {
	cfg := stream.DefaultTransformConfig()
	cfg.MaxLeftover = 1048576

	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReHonorableKill{}.processReject(data, isEOF, pred, emitOut)
	}
...
func (ReHonorableKill) processReject(data []byte, isEOF bool, pred func(*ReHonorableKillBytesResult) bool, emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReHonorableKillBytesResult{}
...
func (r *ReHonorableKillResult) CaptureByIndex(idx int) string {
	switch idx {
	case 0:
		return r.Match
	case 1:
...
// CaptureByIndex returns the capture group value by its 0-based index.
// Index 0 returns the full match, 1+ returns capture groups.
func (r *ReHonorableKillBytesResult) CaptureByIndex(idx int) []byte {
	switch idx {
	case 0:
		return r.Match
	case 1:
...
func (ReHonorableKill) ReplaceAllString(input string, template string) string {
	...
	lastEnd := 0
	var r ReHonorableKillResult

	remaining := input
	offset := 0

	for {
		match, ok := ReHonorableKill{}.FindStringReuse(remaining, &r)
		if !ok {
			break
		}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReHonorableKill) ReplaceAllBytes(input []byte, template string) []byte {
	return ReHonorableKill{}.ReplaceAllBytesAppend(input, template, nil)
}

// ReplaceAllBytesAppend replaces all matches and appends to buf.
// If buf has sufficient capacity, no allocation occurs.
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReHonorableKill) ReplaceAllBytesAppend(input []byte, template string, buf []byte) []byte {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	result := buf[:0]
	lastEnd := 0
	var r ReHonorableKillBytesResult

	remaining := input
	offset := 0

	for {
		match, ok := ReHonorableKill{}.FindBytesReuse(remaining, &r)
		if !ok {
			break
		}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReHonorableKill) ReplaceFirstString(input string, template string) string {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	var r ReHonorableKillResult
	match, ok := ReHonorableKill{}.FindStringReuse(input, &r)
	if !ok {
		return input
	}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReHonorableKill) ReplaceFirstBytes(input []byte, template string) []byte {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	var r ReHonorableKillBytesResult
	match, ok := ReHonorableKill{}.FindBytesReuse(input, &r)
	if !ok {
...
// ReHonorableKillReplaceTemplate holds a pre-compiled replace template for the ReHonorableKill pattern.
// Use CompileReplaceTemplate to create one, then call its Replace methods.
type ReHonorableKillReplaceTemplate struct {
	original string
...
//
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReHonorableKill) CompileReplaceTemplate(template string) (*ReHonorableKillReplaceTemplate, error) {
	tmpl, err := replace.Parse(template)
	if err != nil {
		return nil, err
	}
	...
	return &ReHonorableKillReplaceTemplate{
		original: template,
		segments: resolved,
	}, nil
}
...
func (t *ReHonorableKillReplaceTemplate) String() string {
	return t.original
}

// ReplaceAllString replaces all matches in input using this compiled template.
func (t *ReHonorableKillReplaceTemplate) ReplaceAllString(input string) string {
	var result strings.Builder
	lastEnd := 0
	var r ReHonorableKillResult
...
func (t *ReHonorableKillReplaceTemplate) ReplaceAllBytes(input []byte) []byte {
	return t.ReplaceAllBytesAppend(input, nil)
}

// ReplaceAllBytesAppend replaces all matches and appends to buf.
// If buf has sufficient capacity, no allocation occurs.
func (t *ReHonorableKillReplaceTemplate) ReplaceAllBytesAppend(input []byte, buf []byte) []byte {
	result := buf[:0]
	lastEnd := 0
	var r ReHonorableKillBytesResult
...
func (t *ReHonorableKillReplaceTemplate) ReplaceFirstString(input string) string {
	var r ReHonorableKillResult
	match, ok := ReHonorableKill{}.FindStringReuse(input, &r)
	if !ok {
		return input
...
func (t *ReHonorableKillReplaceTemplate) ReplaceFirstBytes(input []byte) []byte {
	var r ReHonorableKillBytesResult
	match, ok := ReHonorableKill{}.FindBytesReuse(input, &r)
	if !ok {
		return append([]byte{}, input...)
